	...
	>
		...
		<div class="step-wrapper">
			<div class="step-title">
				...
				<img
					class="step-title-icon"
					src="@sub/assets/imgs/common/finish_status_completed.png"
					alt=""
				/>
			...
			>
				<div class="coal-type-table-wrapper">
					...
					>
						...
						>
							<img
								src="@sub/assets/imgs/common/add_active.png"
								class="add-icon"
							/>
							新增信息</a-button
						>
									...
									>
										<div class="coal-type-select-option">
											<div class="coalTypeName">
												<TextOverflowTooltip
													v-if="isHylg"
													:tipText="item.goodsName"
												>
													<span>{{ item.goodsName }}</span>
												</TextOverflowTooltip>
												<TextOverflowTooltip
													v-else
													:tipText="item.coalType"
												>
...
export default {
	...
	computed: {
		...
		coalTypeInventoryListNotEmpty() {
			...
			list = list.map(item => {
				// 配煤库存信息key
				item.coalTypeInventoryKey = assembleCoalTypeInventory(item, this.isHylg);
				return item;
			});
			return list;
		},
	...
	methods: {
		...
		editCoalTypeTable() {
			...
			this.$nextTick(() => {
				this.$refs.coalTypeForm.validate((valid, obj) => {});
			});
		},
		// 保存配煤煤种信息
		saveCoalTypeTable() {
			...
			this.$refs.coalTypeForm.validate((valid, obj) => {
				...
				dataSource.map((item, index) => {
					let { uuid, coalTypeInventory, quantity, ratio } = item;
					blendCoalTotalQuantity += quantity;
				});
				blendCoalTotalQuantity = Math.round(blendCoalTotalQuantity * 100) / 100;
				// if (this.isCoalBlending && blendCoalTotalQuantity !== coalTotalQuantity) {
				// 	this.$message.error('出煤总量与配煤数量不等，请检查');
				// 	return;
				// }
				// if (this.isWashCoal && coalTotalQuantity > blendCoalTotalQuantity) {
				// 	this.$message.error('已超过掺配总量，请检查');
				// 	return;
				// }
			...
			return new Promise(async (resolve, reject) => {
				...
				try {
					...
					dataSource = dataSource.map((item, index) => {
						let { uuid, coalTypeInventoryKey, quantity, ratio } = item;
						let selectedInventoryItem = this.findCoalTypeInventoryItem(coalTypeInventoryKey);
						if (selectedInventoryItem) {
							item = {
								...item,
...
// 组装配煤库存信息
const assembleCoalTypeInventory = (item, isHylg = false) => {
	let { coalType, goodsName, houseName, goodsAllocationName, inventoryQuantity } = item;
	...
	return `${name}-${houseName}&${goodsAllocationName}-${symbol}`;
};

const assembleCoalTypeProductInventory = item => {
	let { coalType, goodsName, houseName, goodsAllocationName, inventoryQuantity } = item;
	let symbol = inventoryQuantity;
	if (inventoryQuantity < 0) {
		symbol = `负${-inventoryQuantity}`;
	}
	return `${goodsName}-${houseName}&${goodsAllocationName}-${symbol}`;
};
...
const coalTypeFormRules = {
	type: [{ required: true, message: '配煤类型必填' }],
	blendingDate: [{ required: true, message: '请选择配煤日期', trigger: 'change' }],
	coalTypeInventory: [{ required: true, message: '请选择配煤库存信息' }],
	quantity: [{ required: true, message: '请输入配煤数量' }],
	ratio: [{ required: true, message: '请输入占总配煤比例' }],
	coalTotalQuantity: [{ required: true, message: '请输入出煤总量', trigger: ['change', 'blur'] }]
};
</script>
...
.produce-coal-table-wrapper {
	.new-table {
		...
		/deep/ .ant-table-tbody > tr:first-child td:nth-child(4) {
			border-bottom: 1px solid #e5e6eb;
		}
		/deep/ .ant-table-tbody > tr:first-child td:nth-child(5) {
			border-bottom: 1px solid #e5e6eb;
		}
