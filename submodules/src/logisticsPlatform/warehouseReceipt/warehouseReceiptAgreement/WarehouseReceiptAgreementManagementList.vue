	...
	<div>
		...
		<div :class="'table-box ' + (pagination.total > 10 ? 'fixedBottom' : '')">
			<a-table
				...
				:loading="loading"
			>
				<template slot="customTitlesMaterialTexture">{{  isWarehouse  ? '企业名称' : '仓储企业' }}</template>
				<template slot="companyName" 	slot-scope="text, record">
					{{ isWarehouse ? record.initiatorCompanyName : record.receiverCompanyName  }}
				</template>
				...
				>
					...
					<template v-if="!isWarehouse && type == 'rest'">
						<a-space class="space">
							...
							<a
								...
								v-auth="'logisticsStorageCenter:warehouseReceiptManage:agreement:manageAgreement:edit'"
							...
							<a
								...
								v-auth="'logisticsStorageCenter:warehouseReceiptManage:agreement:manageAgreement:sign'"
								v-if="record.status == 'INITIATOR_SIGN'"
							>
								盖章</a
							>

							<!-- 审批中 待接收方确认 待接收方盖章 已生效 失效 驳回 已作废-->
							<a
								href="javascript:;"
								v-if="!['INITIATOR_SIGN', 'DRAFT'].includes(record.status) "
							...
							<a
								href="javascript:;"
								v-auth="'logisticsStorageCenter:warehouseReceiptManage:agreement:manageAgreement:delete'"
								v-if="['INVALID'].includes(record.status) || (record.status =='EFFECTIVE' && record.type =='OFFLINE')"
								@click="del(record)"
								>删除</a
							>
							<a-tooltip
								...
								placement="left"
								v-if="['DRAFT', 'INITIATOR_SIGN'].includes(record.status)"
							...
							>
								<template slot="title">
									...
									<a
										href="javascript:;"
										v-if="record.status == 'INITIATOR_SIGN'"
										v-auth="'logisticsStorageCenter:warehouseReceiptManage:agreement:manageAgreement:cancel'"
					...
					<template v-if="isWarehouse">
						<a-space class="space">
							...
							<a
								href="javascript:;"
								v-auth="'logisticsStorageCenter:warehouseReceiptManage:agreement:manageAgreement:confirm'"
...
const restColumns = [
	...
	{ dataIndex: 'receiverCompanyName', slots: { title: 'customTitlesMaterialTexture' }, scopedSlots: { customRender: 'companyName' }  },
	{ title: '协议类型', dataIndex: 'typeText' },
	{ title: '签订日期', dataIndex: 'signDate' },
	{ title: '协议期限', dataIndex: 'effectiveDate' },
...
const adminColumns = [
	{ title: '协议编号', dataIndex: 'serialNo' },
	{ dataIndex: 'initiatorCompanyName', title: '存货人'  },
	{ dataIndex: 'receiverCompanyName', title: '仓储企业'  },
	{ title: '协议类型', dataIndex: 'typeText' },
	{ title: '签订日期', dataIndex: 'signDate' },
	{ title: '协议期限', dataIndex: 'effectiveDate' },
	{ title: '签订状态', dataIndex: 'statusText', scopedSlots: { customRender: 'statusText' }, fixed: 'right', },
...
export default {
	...
	computed: {
		...
		statusData() {
			...
			return [
					...
					{ label: this.type == 'admin' ? '待发起方盖章' : '待我方盖章', value: 'INITIATOR_SIGN',  num: 0},
		...
		searchList() {
			if (this.isWarehouse || this.type == 'admin') {
				return [
					{
						decorator: ['serialNo'],
					...
					},
					{
						...
						type: 'input',
						placeholder: this.isWarehouse ?  '请输入企业名称' : '请输入存货人或仓储企业名称'
					},
					{
						decorator: ['type'],
	...
	mounted() {
		// TextEncoderStream
		this.columns = this.type =='admin' ? adminColumns : restColumns
		this.getList();
	...
	methods: {
		...
		gotoDetail(record) {
			let path =
				this.type == 'admin'
					...
					: '/center/logisticsPlatform/warehouseReceipt/warehouseReceiptAgreement/agreeManageDetail';
			this.$router.push({
				path,
				query: {
					id: record.bid
		...
		gotoEdit(record) {
			let path = record.type == 'OFFLINE'
				? '/center/logisticsPlatform/warehouseReceipt/warehouseReceiptAgreement/addOfflineManage'
				: '/center/logisticsPlatform/warehouseReceipt/warehouseReceiptAgreement/addOnlineManage';
			this.$router.push({
				path,
				query: {
					id: record.bid
