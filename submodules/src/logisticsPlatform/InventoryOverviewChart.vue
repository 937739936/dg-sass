<template>
	<a-spin :spinning="loading">
			...
			<div class="actions">
				<div class="search-wrap">
					<div class="radio-wrap">
						<span
							@click="change('all')"
							:class="activeKey == 'all' ? 'active' : ''"
							>全部</span
						>
						<span
							@click="change('week')"
							:class="activeKey == 'week' ? 'active' : ''"
							>本周</span
						>
						<span
							@click="change('month')"
							:class="activeKey == 'month' ? 'active' : ''"
							>本月</span
						>
						<span
							@click="change('lastMonth')"
							:class="activeKey == 'lastMonth' ? 'active' : ''"
							>上月</span
						>
						<span
							@click="change('lastThreeMonths')"
							:class="activeKey == 'lastThreeMonths' ? 'active' : ''"
							>近三个月</span
						>
						<span
							@click="change('lastSixMonths')"
							:class="activeKey == 'lastSixMonths' ? 'active' : ''"
							>近六个月</span
						>
					</div>
					<sl-range-picker
		...
		<InventoryOverviewPie ref="pie" :source='source' />
	</a-spin>
</template>
<script>
...
import InventoryOverviewPie from '@sub/logisticsPlatform/InventoryOverviewPie';
	...
	methods: {
		setData(data) {
			this.$refs.pie.setData({
				...
				inventoryPieChartVO: data.inventoryPieChartVO
			});
			let inData = [],
				outData = [],
				total = [],
			...
			data.lineChartVO.forEach(item => {
				...
				outData.push(item.outInventory), total.push(item.totalInventory);
		...
		init() {
			...
			this.myEcharts.setOption({
				...
				tooltip: {
					...
					formatter: params => {
						...
						let html = params.map((item) => {
							return `
								<div class='row ${item.seriesType}-mark' style="--color:${item.color}">
									<span class='label'>${item.seriesName}</span>
									<span class='value'>${(
									item.value || 0
								).toNumberString()}</span></div>
							`
						})
                ...
                <span class='time'>${params[0].name}</span>  
								${html.join("")}
                <div class='line'></div>
                <div class='row'><span class='label'>期初库存</span><span class='value'>${(
									this.lineChartData.opening[params[0].dataIndex] || 0
								).toNumberString()}</span></div>
              </div>`;
					}
				},
				legend: {
					...
					data: [
						...
						{
							name: '库存吨位',
							icon: `path://M0 1C0 0.447715 0.447715 0 1 0H13C13.5523 0 14 0.447715 14 1C14 1.55228 13.5523 2 13 2H1C0.447715 2 0 1.55228 0 1Z`
						}
					]
				},
				dataZoom: [
