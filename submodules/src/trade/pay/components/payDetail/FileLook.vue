  ...
  <div>
    ...
    >
      <video width="520" controls autoplay>
        <source :src="encodeURI(previewurl)" type="video/ogg" />
        <source :src="encodeURI(previewurl)" type="video/mp4" />
        <source :src="encodeURI(previewurl)" type="video/webm" />
        <object data="movie.mp4">
          <embed width="520" :src="encodeURI(previewurl)" />
        </object>
  ...
  methods: {
    ...
    async fileLook(data) {
      ...
      if (ext == 'avi' || ext == '3gp' || ext == 'mp4' || ext == 'mkv') {
        this.previewurl = await this.$RsaDecrypt.generateFileUrl(url);
