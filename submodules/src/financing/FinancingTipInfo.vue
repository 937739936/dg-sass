<template>
    <span>
        ...
        <a-tooltip placement="topLeft" trigger="hover" overlayClassName="financing_inner"  @visibleChange="(e) => visibleChange(e, item)"  :overlayStyle="{zIndex:9999}">
            <template slot="title">
                <a-spin v-if="!tipShowText">
                    <a-icon slot="indicator" type="sync" style="font-size: 16px; color: #fff" spin />
                </a-spin>
                <span v-else v-html="tipShowText"> </span>
            </template>
            ...
            <span class="status" v-if="type == 'rest'" :class="item.status">{{filterCodeByValueName(item.status,'financingStatusDict') || item.status}}</span> 
            <span class="status" v-else :class="item.status">{{filterCodeByValueNameAdmin(item.status,'financingStatusDict') || item.status}}</span> 
    ...
    export default {
        ...
        methods:{
            visibleChange(visible, items) {
                if(visible) {
                    this.tipShowText = '';
                    this.API_GetFinancingStatusTip({
