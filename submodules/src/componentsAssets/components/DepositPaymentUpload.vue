		...
		>
			...
			>
				<div class="special-label">
					<label :class="handleType == 'detail' ? 'ant-form-item-no-colon' : 'ant-form-item-no-colon ant-form-item-required'"
						>付款信息</label
					>
				</div>
				<a-row :gutter="30">
				...
				<a-row :gutter="30">
					<a-col :span="12">
						...
						>
							<img
								src="@sub/assets/imgs/assets/money-icon.png"
								alt=""
								class="moneyIcon"
							/>
							<a-input
					...
					<div class="df">
						...
						>
							<img
								class="del"
								@click="del(index)"
								src="@sub/assets/imgs/assets/del2.png"
								alt=""
							/>
							<img
								v-if="isPdf(item.path)"
								src="@sub/assets/imgs/assets/pdf4.jpg"
								:alt="item.text"
						...
						>
							...
							>
								<img
									src="@sub/assets/imgs/assets/upload.png"
									alt=""
								/>
							</div>
						</a-upload>
...
export default {
	...
	inject: {
		...
		getBankInfoParent: { form: 'getBankInfoParent', default: null }
	},
	data() {
		return {
			getPopupContainer,
	...
	methods: {
		...
		async getBankInfo() {
			if (this.getBankInfoParent) {
				let res = await this.getBankInfoParent();
				if (res.success) {
					this.info = res.data;
		...
		async open() {
			...
			this.$nextTick(() => {
				this.form.setFieldsValue({
					paymentCompanyName: this.info.paymentCompanyName,
					collectionCompanyName: this.info.collectionCompanyName
				});
		...
		async editOpen(item, index) {
			if (this.handleType !== 'detail') {
				await this.getBankInfo();
			}
			this.editIndex = index;
			this.visible = true;
			this.realFileList = [item];
			let marginVoucherVO = item.marginVoucherVO;
			this.$nextTick(() => {
				this.form.setFieldsValue({
					paymentDate: marginVoucherVO.paymentDate,
					paymentCompanyName: marginVoucherVO.paymentDate,
					paymentAccountNo: marginVoucherVO.paymentAccountNo,
					paymentBankName: marginVoucherVO.paymentBankName,
					collectionAmount: marginVoucherVO.collectionAmount,
					collectionCompanyName: marginVoucherVO.collectionCompanyName,
					collectionAccountNo: marginVoucherVO.collectionAccountNo,
					collectionBankName: marginVoucherVO.collectionBankName,
					fileInfo: item,
					uploadFiles: this.realFileList
				});
			});
		},
		//上传前校验
		...
		fileChange(e) {
			...
			if (file.status == 'done') {
				...
				obj.path = obj.fileUrl;
				obj.realUrl = obj.path;
				obj.url = obj.path;
				obj.uid = file.uid;
				obj.type = this.type;
				obj.attachId = '';
		...
		submit() {
			this.form.validateFields((err, values) => {
				if (!err) {
					...
					values.paymentCompanyUscc = this.info.paymentCompanyUscc;
					values.collectionCompanyUscc = this.info.collectionCompanyUscc;
					delete values.uploadFiles;
					const marginVoucherVO = Object.assign({}, values);
					const obj = this.realFileList[0];
					obj.marginVoucherVO = marginVoucherVO;
