...
exports.getMessageListMap = getMessageListMap;
...
// 订单详情
var orderDetail = function orderDetail(o) {
	if (!o) return;
	var url = '';
	...
	if (o && o.id && o.companyId) {
		// orderType : BUY采购订单
		// orderType : SELL 销售订单
		if (o.orderType == _constants['default'].orderTypeDict.BUY) {
			url = '/center/contract/buy/online/detail?id='.concat(o.id, '&type=BUY');
		}

		if (o.orderType == _constants['default'].orderTypeDict.SELL) {
			url = '/center/contract/sell/online/detail?id='.concat(o.id, '&type=SELL');
		}
	}
...
var receiveDetail = function receiveDetail(o, type) {
	var url = ''; // type : 1 发货详情
	// type : 2 收货详情
	...
	if (o && o.deliverId) {
		if (type == 1) {
			url = '/center/receive/send/detail?deliverId='.concat(o.deliverId, '&from=release');
		} else if (type == 2) {
			url = '/center/receive/accept/detail?deliverId='.concat(o.deliverId, '&from=receive');
		}
	}
...
var payDetail = function payDetail(o) {
	var url = '';
	if (!o) return;
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.id && o.orderId) {
		url = '/center/fund/pay/record/detail?id='
			.concat(o.id, '&orderId=')
			.concat(o.orderId, '&type=overview&orderType=')
			.concat(o.orderType);
	}
...
var goodsTransferDetail = function goodsTransferDetail(o) {
	var url = '';
	if (!o) return;
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.goodsTransferNo && o.orderSerialNo) {
		url = '/center/transfer/send/detailnew?no='.concat(o.goodsTransferNo, '&orderNo=').concat(o.orderSerialNo);
	}

	return url;
}; // 收款确认

var collectConfirm = function collectConfirm(o) {
	var url = '';
	if (!o) return;
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.id) {
		url = '/center/fund/collect/confirm?id='.concat(o.id);
	} else {
...
var settleDetail = function settleDetail(o) {
	var url = '';
	if (!o) return;
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.statementId) {
		url = '/center/settle/buy/onlinedetail?id='.concat(o.statementId);
	}

	return url;
}; // 业务监控详情

var monitoringDetail = function monitoringDetail(o) {
	if (!o) return;
	var url = '';
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.businessLineNo) {
		url = '/center/businessline/detail?businessLineNo='.concat(o.businessLineNo);
	}

	return url;
}; // 退款详情

var refundDetail = function refundDetail(o) {
	if (!o) return;
	var url = '';
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.id) {
		url = '/center/fund/refund/detail?id='.concat(o.id);
	}

	return url;
};

var suppleAgreementSign = function suppleAgreementSign(o) {
	if (!o) return;
	var url = '';
	o = Object.keys(o).length ? JSON.parse(o) : {};
	...
	if (o && o.id) {
		url = '/center/contract/agreement/stamp?id='.concat(o.id, '&isInitiator=').concat(isInitiator);
	}

	return url;
};

var suppleAgreementDetail = function suppleAgreementDetail(o) {
	if (!o) return;
	var url = '';
	o = Object.keys(o).length ? JSON.parse(o) : {};

	if (o && o.id) {
		url = '/center/contract/agreement/detail?id='.concat(o.id);
	}
...
function getMessageListMap(obj) {
	var result = [
		{
			type: 'user01',
			ifShowBtn: false
		...
		{
			type: 'user05',
			ifShowBtn: false // ifShowBtn:true,
		...
		_toConsumableArray(
			statementTypeList.map(function (item) {
				return {
					type: item,
					ifShowBtn: true,
		...
		[
			...
			{
				type: 'PAYMENT_PAYER_PLATFORM_AUDITING_REJECT',
				// 付款: 平台运营驳回
				ifShowBtn: true,
				url: payDetail(obj),
				selectKey: '/center/fund/pay/record/list',
			...
			{
				type: 'PAYMENT_PAYER_RISK_CONTROL_REJECT',
				// 付款: 平台风控驳回
				ifShowBtn: true,
				url: payDetail(obj),
				selectKey: '/center/fund/pay/record/list',
			...
			{
				type: 'PAYMENT_PAYER_RISK_CONTROL_APPROVE_NO_OA',
				// 付款: 平台风控审批通过
				ifShowBtn: true,
				url: payDetail(obj),
				selectKey: '/center/fund/pay/record/list',
			...
			{
				type: 'PAYMENT_PAYER_RISK_CONTROL_APPROVE_NEED_OA',
				// 付款: 平台风控审批通过
				ifShowBtn: true,
				url: payDetail(obj),
				selectKey: '/center/fund/pay/record/list',
			...
			{
				type: 'PAYMENT_RECEIVER_RISK_CONTROL_APPROVE_NO_OA',
				// 收款款: 平台风控审批通过
				ifShowBtn: true,
