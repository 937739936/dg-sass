<template>
	<div class="login">
		<div class="logo">
			<HeaderComNoMenu></HeaderComNoMenu>
		</div>
		...
		>
			<div class="center">
				<h3>{{ systemConfig.pageLogin.title }}</h3>
				<a-tabs
					:defaultActiveKey="defaultActiveKey"
					@change="changeTabValue"
				>
					<a-tab-pane
						tab="验证码登录"
						:key="2"
						...
						>
							...
							<a-row>
								<a-form-item>
									...
									<!-- <a-icon style="cursor:pointer" :type="iconType" @click.native="type = type == 'password' ? 'text' : 'password';iconType =  type == 'password' ? 'eye-invisible' : 'eye'" class="s-eye" /> -->
								</a-form-item>
							</a-row>
							<a-button
								style="width: 100%; height: 40px"
				...
				<div class="edit-account">
					...
					<p @click="$router.push('/findpass')">忘记密码</p>
				</div>
				<div class="login-agreement">
					<span>登录即代表您同意</span>
					<a @click.prevent="$refs.agreeModal.showModal()">{{ systemConfig.pageLogin.userServiceAgreement }}</a>
					<a @click.prevent="$refs.privacyModal.showModal()">{{ systemConfig.pageLogin.privacyProtectionPolicy }}</a>
				</div>
			</div>
...
<script>
import { API_LOGINBYPWD, API_LOGINBYCODE, API_SENDLOGINCODE } from 'api/account';
...
export default {
	...
	mounted() {
		if (this.VUEX_ST_PERSONALLINFO.id) {
			setTimeout(() => {
		...
		this.getvisitorId();
		// 每次登录清空钢材报表缓存的信息
		localStorage.removeItem('wpsSession');
	},
	methods: {
		...mapMutations({
		...
		loginByCode() {
			this.form.validateFields(['mobile', 'code'], (err, values) => {
				if (!err && this.validateMobile(values.mobile) && this.validateCode(values.code)) {
					this.values = values;
					this.toLoginByCode();
					// tencentCaptcha(this.toLoginByCode)
				}
			});
		},
		toLoginByCode() {
		...
		jumpPage(path) {
			...
			if (redirectUrl) {
				if (redirectUrl.indexOf('/center/admin/invoice')) {
					storage.session.set('loginType', 'invoiceTools');
				} else {
					storage.session.set('loginType', 'normal');
				}
				this.$router.push(`${redirectUrl}${window.location.search}`);
			} else {
				storage.session.set('loginType', 'normal');
				this.$router.push(this.VUEX_ST_BACKURL || path);
			}
		},
		// 账户密码登录
		loginByPass() {
			this.form.validateFields(['password', 'account'], async (err, values) => {
				...
				if (!err) {
					this.values = values;
					this.toLoginByPass();
					// tencentCaptcha(this.toLoginByPass)
				}
			});
		},
		async toLoginByPass() {
