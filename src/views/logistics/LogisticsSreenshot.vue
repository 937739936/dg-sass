...
export default {
  ...
  methods:{
    getTrailRecord() { // 获取轨迹信息 传参运单号
      API_getSoaTrailRecordTrain({id: this.$route.query.waybillId}).then(res => {
        if (res.success) {
          this.tabSource = res.data
          this.selectBatch = res.data[0]
          if(this.selectBatch?.trajectoryPointInfoList?.length){
            let num = 0
            this.selectBatch.trajectoryPointInfoList.map(item=>{
              num+=item.length
            })
            document.querySelector('.content-right').style.height = `${71*num}px`
          }
          this.setMap()
