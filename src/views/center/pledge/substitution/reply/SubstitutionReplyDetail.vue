		...
		<a-card :bordered="false">
			<div class="methods-wrap">
				<span
					...
					>还款赎货详情</span
				>
				<span style="font-size: 14px">状态：{{ detailData.statusText }}</span>
			</div>
			<a-form
				:label-col="{ span: 9 }"
				:wrapper-col="{ span: 15 }"
				...
				<div class="s-card-content">
					...
					<a-row class="row">
						<a-col span="8">
							...
							<a-row>
								<a-form-item label="赎货方">
									{{ detailData.financier }}
								</a-form-item>
							</a-row>
							<a-row>
								<a-form-item label="还款类型">
									{{ detailData.repayTypeText }}
								</a-form-item>
							</a-row>
						</a-col>
						<a-col span="8">
							<a-row>
								<a-form-item label="货押融资编号">
									<a
										target="_new"
										@click="$router.push('/center/financing/financingPledgeDetail?id=' + detailData.financingApplyId + '')"
										>{{ detailData.financingApplyNo }}</a
									>
								</a-form-item>
							</a-row>
							<a-row>
					...
					<h2 style="margin-top: 10px">本次还款信息</h2>
					<a-row class="row">
						<a-col span="8">
							<a-row>
...
import { API_PledgeReplyDetail, API_PledgeReplyDetaildownloadFileAll, API_PledgeReplyDetaildownloadFile } from 'api';
...
export default {
	...
	computed: {},
	mounted: function () {
		API_PledgeReplyDetail({ repayApplyId: this.$route.query.id }).then(res => {
			if (res.success) {
	...
	methods: {
		downAll() {
			API_PledgeReplyDetaildownloadFileAll({
				id: this.$route.query.id
			}).then(res => {
				comDownload(res, undefined, `${this.detailData.repayTypeText}-${this.detailData.serialNo}.zip`);
			});
		},
		downPDF(record) {
			API_PledgeReplyDetaildownloadFile({
