<template>
    <div class="contentBox">
        <div class="content" v-if="goodTransferInfo">
            <p class="title">货转凭证信息</p>
            <!-- 运输方式为动力煤船运-->
            <div class="partBox" v-if="editFlag && contractInfo.upContract.coalType == 'STEAM_COAL' && deliverInfo.deliverList.filter(item => item.transferType == CONSTANTS.despatchModeDict.SHIP).length>0">
                ...
                <template v-if="deliverInfo.deliverList">
                    <span>批次号</span>
                    <a-table :pagination="false" :columns="deliverColumns" :data-source="deliverInfo.deliverList.filter(item => item.transferType == CONSTANTS.despatchModeDict.SHIP)"  :scroll="{x:true}" rowKey="index" :row-selection="rowSelectionOne"></a-table>
                </template>
                <div v-else>
                    暂无数据
                </div>
                <Upload :receivalVO="receivalVO" @uploadFiles="getUploadFiles" type="GOOD_TRANSFER_CERTIFICATION" btnText="选择文件" :selectedRows="selectedRowsOne" mode="transfer"></Upload>
            ...
            <div v-else>
                ...
                <!-- 附件展示 -->
                <a-table :pagination="false" :columns="noFileName ? noFileNameFilesColumns : filesColumns" :data-source="filterLockFile(goodTransferInfo.list)"  :scroll="{x:true}" rowKey="path">
                    <template slot="type" slot-scope="type">
                        {{CONSTANTS.fileType[type]}}
                    </template>
                    <template slot="name" slot-scope="name,items">
    ...
    export default({
        ...
        watch: {
            'goodTransferInfo': function () {
                if (this.editFlag && this.contractInfo.upContract.coalType == 'STEAM_COAL' && this.deliverInfo.deliverList.filter(item => item.transferType == this.CONSTANTS.despatchModeDict.SHIP).length>0) { // 编辑情况下，给表格数据初始化
