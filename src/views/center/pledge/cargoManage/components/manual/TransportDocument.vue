<template>
    <div class="contentBox">
        <!-- 下游 -->
        <div class="">
            <div class="title">运输凭证信息<span class="redTips" v-if="editFlag">必须存在附件</span></div>
            ...
            <div class="content">
                <div>
                        ...
                        <a-form :form="baseForm" >
                            <a-row>
                                <a-col :span="12" class="row">
                                    <a-form-item label="运输方式" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                        <a-select
                                            placeholder="请选择运输方式"
                                            @change="changeTransport"
                                                ...
                                                >
                                            <a-select-option
                                                v-for="(item, index) in [{name:'汽运',value:'AUTOMOBILE'},{name:'火运',value:'TRAIN'},{name:'船运',value:'SHIP'}]"
                                                :key="index"
                                                :value="item.value">
                                                {{ item.name }}
                                            </a-select-option>
                            ...
                            <template v-if="baseForm.getFieldValue('transType') == 'SHIP'">
                                ...
                                <a-row>
                                    <a-col :span="12" class="row">
                                        <a-form-model-item label="装货港" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入装货港"
                                                v-decorator="[
                                                    'shipLoadingPortName',
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="卸货港" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入卸货港"
                                                v-decorator="[
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="货物数量(吨)" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入货物数量"
                                                v-decorator="[
                                                    `deliverQuantity`,
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="装货日期" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-date-picker
                                                placeholder="请输入装货日期"
                                                format="YYYY-MM-DD"
                                                v-decorator="[
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="收货日期" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-date-picker
                                                placeholder="请输入收货日期"
                                                format="YYYY-MM-DD"
                                                v-decorator="[
                                ...
                                <div>
                                    ...
                                    <div style="text-align:right;margin-bottom:10px;">
                                        <a-button ghost type="primary" @click="addShip">新增</a-button>
                                    </div>
                                    <a-table :pagination="false" :columns="shipEditColumns" :data-source="shipDataSource"  :scroll="{x:true}" rowKey="id">
                                        <template slot="action" slot-scope="action,items,index" >
                                            <a style="margin-right:10px;" href="javascript:;" @click="editShip(items,index)">编辑</a>
                                            <a href="javascript:;">删除</a>
                                            <a style="margin-left:10px;" href="javascript:;" @click="openGuiji(items,index)" v-if="items.id">查看轨迹</a>
                                        </template>
                                    </a-table>
                                </div>
                            </template>
                            <template v-if="baseForm.getFieldValue('transType') == 'TRAIN'">
                                ...
                                <a-row>
                                    <a-col :span="12" class="row">
                                        <a-form-model
                                            ...
                                            :rules="rules"
                                            :model="params">
                                            <a-form-model-item prop="shipperName" label="托运人" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                                <a-select
                                                ...
                                                <a-select-option
                                                    ...
                                                    :key="item.name"
                                                    :title="item.name">{{item.name}}</a-select-option>
                                                </a-select>
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="运单号" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                ...
                                                v-decorator="[
                                                    `serialNo`,
                                                    {
                                                        rules: [
                                                            ...
                                                            { pattern: /^\w{12}$/, message:'请录入正确的12位运单号'}
                                                        ],
                                                    },
                                                ]"
                                                >
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-model-item label="发货数量(吨)" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入发货数量"
                                                v-decorator="[
                                                    `deliverQuantity`,
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-model-item label="车数" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入车数"
                                                v-decorator="[
                                                    `trainNum`,
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="发货日期" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-date-picker
                                                placeholder="请输入发货日期"
                                                format="YYYY-MM-DD"
                                                v-decorator="[
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="发站" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-select
                                                showSearch
                                                ...
                                                v-decorator="[
                                                    'deliveryStation',
                                                    {
                                                        rules: [{ required: true, message: `发站必填` }],validateTrigger:'blur'
                                                    }
                                                ]"
                                                >
                                            <a-select-option v-for="(items, index) in placeData" :key="index" :value="items.name">{{items.name}}</a-select-option>
                                        </a-select>
                                        </a-form-item>
                                    </a-col>
                                    <a-col :span="12" class="row">
                                        <a-form-item label="到站" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-select
                                                showSearch
                                                :disabled="hasserialNo"
                                                mode="multiple"
                                                ...
                                                v-decorator="[
                                                    'arriveStation',
                                                    {
                                                        rules: [{ required: true, message: `到站必填` }],validateTrigger:'blur'
                                                    }
                                                ]"
                                                >
                                                <a-select-option v-for="(items, index) in placeData" :key="index" :value="items.name">{{items.name}}</a-select-option>
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-model-item label="铁路计划号" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                ...
                                <div>
                                    <p>车皮信息<span style="color:red" v-if="!hasserialNo">（至少录入一个车皮信息）</span></p>
                                    <div style="text-align:right;margin-bottom:10px;">
                                        <a-button type="primary" @click="openTrainGuiji()" v-if="deliverInfo&&deliverInfo.id">查看轨迹</a-button>
                                        <a-button ghost type="primary" style="margin-left:20px;" @click="addTrain" v-if="!hasserialNo" v-show="baseForm.getFieldValue('serialNo')">新增</a-button>
                                    </div>
                                    <a-table :pagination="false" :columns="hasserialNo ? trainColumns : trainEditColumns" :data-source="trainDataSource"  :scroll="{x:true}" rowKey="id">
                                        <template slot="action" slot-scope="action,items,index" >
                                            <a style="margin-right:10px;" href="javascript:;" @click="editTrain(items,index)">编辑</a>
                                            <a href="javascript:;" @click="deleteTrain(items,index)">删除</a>
                            ...
                            <template v-if="baseForm.getFieldValue('transType') == 'AUTOMOBILE'">
                                ...
                                <a-row>
                                    <a-col :span="12" class="row">
                                        <a-form-item label="发货数量(吨)" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入发货数量"
                                                v-decorator="[
                                                    `deliverQuantity`,
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-item label="车数" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入车数"
                                                v-decorator="[
                                                    `trainNum`,
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-model-item label="发货地址" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入发货地址"
                                                v-decorator="[
                                                    `deliverAddr`,
                                    ...
                                    <a-col :span="12" class="row">
                                        <a-form-model-item label="收货地址" :colon=false :label-col="{ span: 6 }" :wrapper-col="{ span: 14}">
                                            <a-input
                                                placeholder="请输入收货地址"
                                                v-decorator="[
                                                    `receiveAddr`,
                    ...
                    <template v-if="!editFlag">
                        ...
                        <template v-if="deliverInfo.transType == 'SHIP'">
                            ...
                            <a-table :pagination="false" :columns="shipEditColumns" :data-source="shipDataSource"  :scroll="{x:true}" rowKey="path">
                                ...
                                <template slot="action" slot-scope="action,items,index" >
                                    ...
                                    <a href="javascript:;" @click="openGuiji(items,index)" v-if="items.id">查看轨迹</a>
                                </template>
                            </a-table>
                        </template>
                        <template v-if="deliverInfo.transType == 'TRAIN'">
                            ...
                            <a-table :pagination="false" :columns="trainColumns" :data-source="trainDataSource"  :scroll="{x:true}" rowKey="path">
                ...
                <div>
                    ...
                    <template v-if="editFlag||editFile">
                        <template  v-if="baseForm.getFieldValue('transType') == 'AUTOMOBILE'">
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_CREDENTIALS" btnText="上传运输凭证"></Upload>
                        </template>
                        <template  v-if="baseForm.getFieldValue('transType') == 'SHIP'">
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_CREDENTIALS" btnText="上传运输凭证"></Upload>
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_WEIGHT" btnText="上传称重凭证"></Upload>
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_OTHER_CREDENTIALS" btnText="上传其他凭证"></Upload>
                        </template>
                        <template  v-if="baseForm.getFieldValue('transType') == 'TRAIN'">
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_BIG_TICKET" btnText="上传大票"></Upload>
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_CAR_NUMBER_TABLE" btnText="上传车号表"></Upload>
                            <Upload @uploadFiles="getUploadFiles" type="MANUAL_DELIVER_OTHER_CREDENTIALS" btnText="上传其他凭证"></Upload>
                        </template>
                        <span style="font-family: PingFangSC-Regular;font-size: 12px;color: #C8CCD5;">单个文件最大支持100M，支持多个上传</span>
                        <a-table :pagination="false" :columns="editFilesColumns" :data-source="fileListDataSource.filter(item=>item.delFlag == 0)"  :scroll="{x:true}" rowKey="path">
                            <template slot="type" slot-scope="type">
                                {{CONSTANTS.fileType[type]}}
                            </template>
                            <template slot="name" slot-scope="name,items">
                    ...
                    <!-- 情展示模块 -->
                    <template v-if="!editFlag&&!editFile">
                        <a-table  :pagination="false" :columns="filesColumns" :data-source="fileListDataSource.filter(item=>item.delFlag == 0)"  :scroll="{x:true}" rowKey="path">
                            <template slot="type" slot-scope="type">
                                {{CONSTANTS.fileType[type]}}
                            </template>
                            <template slot="name" slot-scope="name,items">
        ...
        <a-modal
            ...
            <a-form :form="shipVisibleForm"  :label-col="{span: 6}" :wrapper-col="{span: 15 }">
            ...
            <template slot="footer">
                <a-button key="back" @click="shipVisible=false">
                取消
                </a-button>
                <a-button key="submit" type="primary" @click="okShip">
                确定
                </a-button>
                <a-button key="submit1" type="primary" @click="okShip('more')" v-if="editShipFlag == null">
                确定并继续添加
                </a-button>
            </template>
        ...
        <a-modal
            ...
            @ok="okTrain"
            @cancel="()=>{trainVisible=false}">
            <a-form :form="trainVisibleForm"  :label-col="{span: 6}" :wrapper-col="{span: 15 }">
...
<script>
    import { API_STATION,API_getDeliverGetRpcShip,API_getDeliverInfoTrains,API_getApiTrainDeliverShipper,API_saveTrainDeliverShipper,API_getRouteInfo,API_GETTRAINRECORD} from "api";
    ...
    export default({
        ...
        methods: {
            ...
            openGuiji(items){
                ...
                window.open('/logistics/LogisticsDetailShip?source=ASSET_MANUAL&mmsi=' + items.identifierNo + '&shipName=' + items.shipName + '&type=realLocation&deliveryId=' + this.deliverInfo.id + '&from=yunkong')
            },
            openTrainGuiji(){
                ...
                this.baseForm.validateFields((error,values)=>{
                    if (!error) {
                        ...
                        let obj = {
                            ...
                            deliveryStation:(values.deliveryStation && values.deliveryStation.join) ? values.deliveryStation.join(',') : values.deliveryStation,
                            arriveStation:(values.arriveStation && values.arriveStation.join) ? values.arriveStation.join(',') : values.arriveStation,
