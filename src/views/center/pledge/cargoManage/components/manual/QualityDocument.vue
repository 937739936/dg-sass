    ...
    <div class="contentBox">
        <div class="content">
            ...
            <template v-if="editFlag||editFile">
                <p class="title">数质量凭证信息<span class="redTips" v-if="editFlag">必须存在附件</span></p>
                <div class="partBox">
                    <template  v-if="VUEX_MANUAL_ASSET_OBJ.transportMode == 'AUTOMOBILE'">
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_WEIGHT_NOTES" btnText="上传磅单"></Upload>
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_WEIGHT_NOTES_DETAIL" btnText="上传磅单明细"></Upload>
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_TEST_CREDENTIALS" btnText="上传化验凭证"></Upload>
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_OTHER_CREDENTIALS" btnText="上传其他凭证"></Upload>
                    </template>
                    <template  v-if="VUEX_MANUAL_ASSET_OBJ.transportMode == 'SHIP'">
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_TEST_CREDENTIALS" btnText="上传化验凭证"></Upload>
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_WEIGHT" btnText="上传称重凭证"></Upload>
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_HARBOR_TRANSFER_VOUCHER" btnText="上传港口货转凭证"></Upload>
                        <Upload @uploadFiles="getUploadFiles" type="MANUAL_RECEIVE_OTHER_CREDENTIALS" btnText="上传其他凭证"></Upload>
                    </template>
            ...
            <template v-if="!editFlag&&!editFile">
                ...
                <template>
                    <a-table :pagination="false" :columns="filesColumns" :data-source="fileListDataSource.filter(item=>item.delFlag == 0)"  :scroll="{x:true}" rowKey="index">
                        <template slot="type" slot-scope="type">
                            {{CONSTANTS.fileType[type]}}
                        </template>
                        <template slot="name" slot-scope="name,items">
