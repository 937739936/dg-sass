<template>
    <div class="contentBox">
        <div class="content invo-tabs">
            ...
            <div style="text-align:right;margin-bottom:10px;" v-if="editFlag">
                <a-space>
                    <a-button ghost type="primary" @click="addInvo('addSell?invoiceType=1&contractType=DOWN&contractNo=')">新增贸易发票</a-button>
                    <a-button ghost type="primary" @click="addInvo('addFreight?invoiceType=2&contractType=DOWN&contractNo=')">新增运费发票</a-button>
                </a-space>
            </div>
            <a-tabs defaultActiveKey="a">
                <a-tab-pane key="a" tab="贸易发票" style="flex-direction: column;" class="first-key">
                    <a-table :pagination="false" :columns="tradeInvoiceColumns" :data-source="tradeInvoiceList" :scroll="{x:true}" rowKey="index">

                        <div slot="action" slot-scope="action, items,index">
                            <a href="javascript:;" @click="getResult(items)" style="margin-right:8px">查验结果</a>
                            <a-popconfirm
                                v-if="editFlag"
                                title="确定删除?"
                                okText="确定"
                ...
                <a-tab-pane key="b" tab="运费发票" style="flex-direction: column;">
                    <a-table :pagination="false" :columns="freightInvoiceColumns" :data-source="transInvoiceList" :scroll="{x:true}" rowKey="index">

                        <div slot="action" slot-scope="action, items,index">
                            <a href="javascript:;" style="margin-right:8px" @click="getResult(items)">查验结果</a>
                            <a-popconfirm
                                v-if="editFlag"
                                title="确定删除?"
                                okText="确定"
        ...
        <a-modal :visible="visible" :footer="null" @cancel="()=>{visible=false}" width="80%">
            <div class="modalContent" v-if="invoiceResult">
                ...
                <table cellspacing="0" cellpadding="0" class="modalTable" style="margin-bottom: 20px;" :scroll="{x:true}">
                    ...
                    <tr v-for="(item,index) in invoiceResult.invoiceItemList" :key="index">
                        <td colspan="5">
...
<script>
    ...
    import { API_GetInvoiceResult,API_InvoiceDelete } from 'api'
    // import comDownload from '@sub/utils/comDownload.js';
    import { Empty } from 'ant-design-vue';
    // import _ from 'lodash';
    export default({
        name: 'Invoice',
        props: ['invoiceInfo','editFlag','lineId','contractInfo','isDai'],
        components: {
                ...
                if (this.contractInfo && this.contractInfo.contractNo && this.$route.query.id) {
                    this.$router.push({
                          ...
                          query: {
                            ...
                            ref:this.isDai ? 'accountsReceivableDaiManualDetail':'accountsReceivableManualEdit'
                          }})
