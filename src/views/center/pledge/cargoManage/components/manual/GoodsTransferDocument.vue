<template>
    <div class="contentBox">
        <div class="content">
            <p class="title">货转凭证信息<span class="redTips" v-if="editFlag">必须存在附件</span></p>
            <!-- 运输方式为动力煤船运-->
            <div class="partBox" v-if="editFlag||editFile">

                <Upload @uploadFiles="getUploadFiles" type="MANUAL_GOOD_TRANSFER_CERTIFICATION" btnText="上传货转证明"></Upload>
                ...
                <a-table :pagination="false" :columns="editFilesColumns" :data-source="fileListDataSource.filter(item=>item.delFlag == 0)"  :scroll="{x:true}" rowKey="path">
                    ...
                    <template slot="quantity" slot-scope="name,items">
                        <a-input type="number" v-model="items.quantity" :disabled="items.editFlag == 0"/>
                    </template>
                    <template slot="openTime" slot-scope="name,items">
                        <a-date-picker
                            type="date"
...
<script>
    ...
    export default({
        ...
        methods: {
            ...
            openTimeChange(item,v){

                item.openTime = v
            },
            deliverQuantityChange(item,v){
                // debugger
                setTimeout(()=>{
                    item.quantity = v.target.value
                },400)
