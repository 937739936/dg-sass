<template>
    <div class="contentBox">
        <div class="content" v-if="settleInfo">
            <p class="title">结算单材料信息</p>
            <p class="sub-title">附件信息</p>
            <!-- 编辑模块 -->
            <template v-if="editFlag">
                <Upload @uploadFiles="getUploadFiles" type="PAYABLE_VOUCHER" btnText="上传付款凭证" :receivalVO="receivalVO"></Upload>
                <Upload @uploadFiles="getUploadFiles" type="SETTLEMENT" btnText="上传结算对账单" :receivalVO="receivalVO"></Upload>
            ...
            <!-- 详情模块 -->
            <template v-if="!editFlag">
                <a-table :pagination="false" :columns="noFileName ? noFileNameFilesColumns : filesColumns" :data-source="filterLockFile(settleInfo.list)" :scroll="{x:true}" rowKey="index">
                    <template slot="type" slot-scope="type">
                        {{CONSTANTS.fileType[type]}}
                    </template>
                    <template slot="name" slot-scope="name,items">
