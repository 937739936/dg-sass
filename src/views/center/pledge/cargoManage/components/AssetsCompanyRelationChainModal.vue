...
<div>
    ...
    >
        ...
        >
            ...
            >
                <div class="company">
                    ...
                    />
                    <div class="company tc" v-if="item.list">
                    ...
                    <a-tooltip v-else placement="bottom" :title="item.name">
                        <p class="tc ellipsis" @click="stop($event, index)">
                            {{ item.name }}
                        </p>
                    </a-tooltip>
                </div>
        ...
        <div class="" v-if="panelType == 'contract'">
                    ...
                    <a-col :span="12">
                        <a-row :gutter="20">
                            ...
                            <a-col
                                ...
                                "
                                >{{
                                    curCompany.status == "2" ? "双签" : "单签"
                                }}</a-col
                            >
                        </a-row>
                    </a-col>
                    <!-- 双签状态 -->
                    <a-col :span="12" v-if="curCompany.status == '2'">
                        <a-row :gutter="20">
                            <a-col :span="8">双签版合同获得日期</a-col>
                            <a-col :span="16">{{
                                curCompany.doubelSignRecvDate
                            }}</a-col>
                        </a-row>
                    </a-col>
                </a-row>
        ...
        <div v-if="panelType == 'invo'">
            ...
            <a-table :pagination="false" :columns="invoColumns" :data-source="curInvoList" :scroll="{x:true}" rowKey="id">
    ...
    <ChooseInvoice ref="ChooseInvoice" @chooseFinInvo="chooseFinInvo" />
</div>
</template>
<script>
import {
...
import ChooseInvoice from "./common/ChooseInvoice";
export default {
    ...
    methods: {
            ...
            API_FullBusinessLineDetailV1(id).then((res) => {
                if (res.success) {
                    ...
                    if (this.companyChain[0].list) {
                        ...
                        this.curFirsContract =
                            this.contractChain[0].contractList[0];
                    }
        ...
        selectCompany(e) {
            ...
            let left =
                e.clientX -
                businessLine.getBoundingClientRect().left -
                this.activedCoverMoveBaseWidth / 3 +
                businessLine.scrollLeft;
            if (left <= 0) {
                left = 0;
            }
            ...
            let activedCoverIndex = Math.floor(
                left / this.activedCoverMoveBaseWidth
            );
            activedCoverIndex =
                activedCoverIndex >= this.companyChain.length * 2
                    ? this.companyChain.length * 2 - 1
                    : activedCoverIndex;
        ...
        handleActived(data) {
            ...
            let upContract =
                this.contractChain[this.activedCoverIndex - 1] || {};
            ...
            if (this.activedCoverIndex === 1) {
                upCompany = this.curFirstCompany
                    ? this.curFirstCompany
                    : upCompany;
                upContract = this.curFirsContract
                    ? this.curFirsContract
                    : upContract;
            }
            if (this.activedCoverIndex === 0) {
                upCompany = {};
                upContract = {};
                curCompany = this.curFirstCompany
                    ? this.curFirstCompany
                    : curCompany;
                curContract = this.curFirsContract
                    ? this.curFirsContract
                    : curContract;
            }
            const params = {
                upCompany,
                upContract: upContract.contract
                    ? upContract.contract
                    : upContract,
                curCompany,
                curContract: curContract.contract
        ...
        setActivedCoverLeft(index) {
            ...
            this.activedCoverLeft =
                this.activedCoverIndex * this.activedCoverMoveBaseWidth;
        },
        okInc() {
            this.$emit('addFin',this.panelType == 'contract' ? this.curCompany : {
        ...
        chooseContract(params){
            API_GetReceivablefullBusinessLine({
                ...
                this.curCompany = {
                    ...
                    basePrice:
                        type == "DOWN" ? _item.contractPrice : _item.basePrice,
                    quantity:
                        type == "DOWN"
                            ? _item.contractQuantity
                            : _item.quantity,
                    amount:
                        type == "DOWN" ? _item.contractAmount : _item.amount,
                    signDate:
                        type == "DOWN"
                            ? _item.contractSignTime
                            : _item.signDate,
                    endDate: type == "DOWN" ? null : _item.endDate,
                    status: type == "DOWN" ? _item.status : _item.signStatus,
                    doubelSignRecvDate:
                        type == "DOWN"
                            ? _item.doubleSignTime
                            : _item.doubelSignRecvDate,
                };
            });
        },
    },
