...
<a-row class="template">
  <a-row type="flex"  class="title select-template">
    <p>{{typeText}}<span class="red-sim">（请注意条款中甲乙方与买卖方对应关系，建议统一用买方、卖方进行描述。）</span></p>
    <a-button v-if="!disabled && !flag" style="margin: 0 20px;" type="primary" icon="heart" @click.native="visible2 = true;name='';">收藏</a-button>
    <a-button v-if="!disabled && !flag" type="primary" icon="plus-circle" @click.native="selectTemplate">选择已有模板</a-button>
  </a-row>
    ...
    <div @click.stop style="background:#fff; margin-top:120px"  @move.stop>
      <div style="background:#FFF;width:800px;" class="content">
        <a-row style="width:80%; margin: 0 auto;margin-top:20px">
          <a-input @change="getTemplateList" v-model="params.name" placeholder="请输入模板名称"></a-input>
        </a-row>
        <a-row v-if="templatList && templatList.length" class="w-e-text">
          <a-row v-for="(items, index) in templatList" :key="index" style="margin: 10px 0;cursor:pointer" @click.native="addTemplateValue(items.content)">
            <h3>{{items.name}}<a-button style="margin-left:20px;" @click.stop="deleteTemplate(items.id, items.name)">删除</a-button></h3>
        ...
        <a-row v-else type="flex" justify="center" style="font-size:24px; color:#999;padding:20px 0;">暂无数据</a-row>
      </div>
  ...
  <div class="s-modal" v-if="visible2" @click="visible2=false;">
    <div @click.stop style="background:#fff; margin-top:120px"  @move.stop>
      <a-row class="content" @click.native.stop>
        <a-row>
          <a-input style="width: 90%; margin: 0 auto;margin-top:20px" placeholder="请输入模板名称" v-model="name"></a-input>
        </a-row>
        <a-row class="btn">
          <a-button type="primary" @click="visible2=false">取消</a-button>
          <a-button type="primary" style="margin-left: 20px" @click="handleOk">确定</a-button>
        </a-row>
      </a-row>
    </div>
  </div>
...
import { API_TEXTTEMPLATELIST, API_SAVETEXTTEMPLATE, API_DELETETEMPLATE } from 'api'
export default {
  data(){
    return {
      visible: false,
  ...
  watch: {
    ...
    problemList(data){
        this.problem = []
        this.editorContent = this.editorContent.replace(/<\/?mgc.*?>/g,'').replace(/style="background-color: rgb\(255, 255, 0\);"/g,'') //每次检测关键词时，清空原有的高亮标签，防止标签多层嵌套
        data.map(item=>{
          if(this.editorContent.includes(item)){
            this.problem.push(item)
            const regExp = new RegExp(item, 'g')
            this.editorContent = this.editorContent.replace(regExp,`<mgc style="display:inline-block;background-color:yellow">${item}</mgc>`)
          }
        })
  ...
  methods: {
    ...
    getTypeName(){
      // 模板不同，对应富文本标题不同
      const { STEAM_COAL_013, COKING_COAL_005, COKING_008,SEMI_COKE_002,INJECTION_COAL_002,SMOKELESS_COAL_002, STEAM_COAL_1004, COKING_COAL_1004, COKING_1004,SEMI_COKE_1004,INJECTION_COAL_1004,SMOKELESS_COAL_1004,STEAM_COAL_1003, COKING_COAL_1003, COKING_1003,SEMI_COKE_1003,INJECTION_COAL_1003,SMOKELESS_COAL_1003} = this.CONSTANTS.contractTemplateDict;
      ...
      if(this.value == 'qualityExamineWayHtmlStr'){
        this.typeText = (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_1001)?'质数量的验收':'质量和数量验收标准及方法'
        if (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_010) {
          this.typeText = '数量和质量的结算依据';
        }
        if (BUY_PRESSURE_LIST.includes(this.contractTemplate) || SELL_PRESSURE_LIST.includes(this.contractTemplate) || SELL_PURCHASE_LIST.includes(this.contractTemplate)) {
          this.typeText = '质量和数量验收';
        }
      }
      if(this.value == 'makePriceWayHtmlStr'){
        this.typeText = (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_1001)?'合同价格':'合同定价办法';
        if (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_010) {
          this.typeText = '价格';
        }
        if (BUY_PRESSURE_LIST.includes(this.contractTemplate) || SELL_PRESSURE_LIST.includes(this.contractTemplate) || SELL_PURCHASE_LIST.includes(this.contractTemplate)) {
      ...
      if(this.value == 'settleWayHtmlStr'){
        this.typeText = (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_1001)?'结算方式':'结算付款方式'
        if (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_010) {
      ...
      if(this.value == 'otherAppointHtmlStr'){
        this.typeText = (this.contractTemplate === this.CONSTANTS.contractTemplateDict.STEAM_COAL_1001)?'其他事项':'其他约定'
        if (BUY_PRESSURE_LIST.includes(this.contractTemplate) || SELL_PRESSURE_LIST.includes(this.contractTemplate)) {
          this.typeText = '其他约定';
        }
      }
      if(this.value == 'downstreamSignHtmlStr'){
        this.typeText = '下游签约信息描述'
        if (SELL_PRESSURE_LIST.includes(this.contractTemplate) || SELL_PURCHASE_LIST.includes(this.contractTemplate) ) {
