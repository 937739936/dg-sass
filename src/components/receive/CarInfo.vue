<template>
	...
	<div id="carInfo">
		...
		<div class="train-times-wrap">
			<a-table :dataSource="dataSource" :columns="columns" :pagination="false" :locale="{emptyText: '暂无数据'}">
				<template slot="operation" slot-scope="text, record, index">
					<a-space>
						<a href="javascript:;" v-if="(release&&record.type=='HAND')||receive" @click="editCarInfo(record,index)">编辑</a>
						<a-popconfirm
							v-if="release"
							title="确定删除该车辆信息?"
							okText="确定"
			...
			<p class="add-wrap" v-if="release">
				<a-space size="middle">
					<span @click="popCarList" v-if="popCar.show"><a-space><a-icon type="plus" />选择已有发运车辆</a-space></span>
					<span @click="editCarInfo"><a-space><a-icon type="plus" />手动添加</a-space></span>
				</a-space>
			</p>
		</div>
				...
				class="modal-car-info">
					<a-form-model
						...
						<a-form-model-item
							...
							<div>
								<a-input
									autocomplete="off"
									:disabled="!release"
									@blur="() => {$refs.plateNumber.onFieldBlur()}"
									@change="() => {$refs.plateNumber.onFieldChange()}"
						...
						<a-form-model-item
							...
							label="发货量(吨)">
							<a-input :disabled="!release" v-model="carInfoform.deliverQuantity"/>
						</a-form-model-item>
						<a-form-model-item
							prop="deliverDate"
							label="发车时间">
						...
						<a-form-model-item
							prop="ticketNo"
							label="运单号">
							<a-input :disabled="!release" v-model="carInfoform.ticketNo"/>
						</a-form-model-item>
					</a-form-model>
				</a-modal>
			<CarList ref="carList" @getCarList="getCarList" />
		</div>
	</div>
</template>

<script>
	import plateNumberJson from '../../assets/json/licensePlateNumber.json'
	import {filterCodeByTextName} from '@sub/utils/globalCode.js'
	import { dispatchDriverSaveOrUpdate } from "@/api/dispatch";
			...
			plateRules(){
				return {
					plateNumber: [
						...
						{ pattern: /^(?:[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领 A-Z]{1}[A-HJ-NP-Z]{1}(?:(?:[0-9]{5}[DF])|(?:[DF](?:[A-HJ-NP-Z0-9])[0-9]{4})))|(?:[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领 A-Z]{1}[A-Z]{1}[A-HJ-NP-Z0-9]{4}[A-HJ-NP-Z0-9 挂学警港澳]{1})$/, message: '请输入正确的车牌号', trigger: ['change', 'blur']}
					],
					deliverQuantity: [
						{ required: this.release, message: '请输入发货量', trigger: ['change', 'blur'] },
						{ pattern: /(^((\d{0,2})|([1]\d{0,2})|0)(\.\d{0,2})?$)|(^([2][0]{0,2})(\.[0]{0,2})?$)/, message:'发货量为数字，最大200，最多两位小数', trigger: ['change', 'blur']}
					],
					deliverDate: [
						{ required: this.release, message: '请选择发车时间', trigger: ['change', 'blur'] }
					],
					arriveDate: [
						{ required: this.receive, message: '请选择到站时间', trigger: ['change', 'blur'] }
					]
				}
			}
		},
		...
		methods:{
			...
			getCarList(selectedCarNoList, selectedCarNoData) {
				...
				let selectedCarNoDataNew=selectedCarNoData.filter(item=>{
					return !dataSource.some(items=>{return items.id==item.id})
				})
...
.modal-car-info{
	.plate-number-container{
		...
		border-radius: 5px;
		.plate-number-province, .plate-number-number{
			display: flex;
			flex-wrap: wrap;
		}
		.plate-number-item{
  ...
  ::v-deep.ant-form-inline .ant-form-item-with-help{margin:0}
}
</style>
