<template>
    ...
    <div id="trainInfo">
        <div class="title">
            ...
            <a-button v-if="params.status == 2 || params.status == 3 || params.status == 4"  type="primary" ghost class="trans-track" @click.native="jumpToTail()"><a-icon type="car" />查看运输轨迹</a-button>
        </div>
        <div class="train-times-wrap">
            <a-table :dataSource="dataSource" :columns="columns" :pagination="false" :locale="{emptyText: '暂无数据'}" rowKey="serialNo">
                <template slot="operation" slot-scope="text, record">
                    <a href="javascript:;" class="edit-btn" v-if="record.businessType!='OTHER'" @click="showDeliverInfo(record)">查看运单</a>
                    <span class="line" v-if="datas.vehicleFlag">|</span>
                    <a href="javascript:;" class="edit-btn" @click="showTrainTable(record)" v-if="datas.vehicleFlag">查看车号表</a>
                </template>
            </a-table>
        </div>
        <div class="modal-wrap">
            ...
            >
                <div>
                    <p style="overflow:hidden;padding-bottom: 5px;"><span style="float: left;">需求号：{{modalInfo.demandNo}}</span><span style="float: right;">运单号：{{modalInfo.serialNo}}</span></p>
                    <table cellspacing="0" cellpadding="0" class="modalTable">
                        ...
                        <tr>
                            ...
                            <td colspan="7" rowspan="2">{{modalInfo.consignorName}}</td>
                            <td colspan="2">经办人</td>
                            <td colspan="3"></td>
                            <td colspan="2">货位</td>
                            <td colspan="6"></td>
                        ...
                        <tr>
                            <td colspan="2" rowspan="2">名称</td>
                            <td colspan="7" rowspan="2">{{modalInfo.consigneeName}}</td>
                            <td colspan="2">经办人</td>
                            <td colspan="3"></td>
                            <td colspan="2">施封号</td>
                            <td colspan="6"></td>
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                <a-checkbox disabled :default-checked="Boolean(modalInfo.loadingToDoor)"  style="margin-right:5px;"></a-checkbox>上门装车
                            <td colspan="3"></td>
                            <td colspan="3">费目</td>
                        ...
                        <tr>
                            <td colspan="3">
                                <a-checkbox disabled :default-checked="Boolean(modalInfo.unloadingToDoor)"  style="margin-right:5px;"></a-checkbox>上门卸车</td>
                            <td colspan="3"></td>
                            <td colspan="3"></td>
                            <td colspan="2"></td>
                            <td colspan="2"></td>
                        ...
                        <tr>
                            <td colspan="6">
                                <a-checkbox disabled :default-checked="Boolean(modalInfo.transportSafeValue)"  style="margin-right:5px;"></a-checkbox>保价运输
                        ...
                        <tr>
                            <td colspan="3" rowspan="3">
                                <span>增值税发票类型</span>
                                <a-checkbox disabled :default-checked="Boolean(modalInfo.vatInvoiceType == 1)"  style="margin-right:5px;"></a-checkbox>普通票<br/>
                                <a-checkbox disabled :default-checked="Boolean(modalInfo.vatInvoiceType == 2)"  style="margin-right:5px;"></a-checkbox>专用票
                            </td>
                        ...
                        <tr>
                            <td colspan="9" rowspan="3" style="height: 74px;">{{modalInfo.shipperRemember}}</td>
                            <td colspan="14" rowspan="3" style="height: 74px;">{{modalInfo.carrierRemember}}</td>
                        </tr>
                    </table>
            ...
            >
                <div>
                    <div style="display: flex;flex-direction: row;justify-content: space-between;;margin-bottom: 20px;font-size:16px;font-weight:bold;">
                        <a-button type="primary" @click="exportXls" style="position: absolute;left:100px;top:10px;" v-if="exportFlag">导出</a-button>
                        <span>整列运输运单-车辆附表</span>
                        <span>运单号码【{{modalInfo.serialNo}}】共【{{modalInfo.waybillVehicleInfoVO?modalInfo.waybillVehicleInfoVO.length:0}}】车</span>
                    </div>
                    <div style="display: flex;flex-direction: row;margin-bottom: 20px;justify-content: space-between;font-weight:bold;">
                        ...
                        <span>日期：{{modalInfo.departureTime}}</span>
                    </div>
                    <table cellspacing="0" cellpadding="0" class="modalTable" style="margin-bottom: 20px;" v-if="modalInfo.waybillVehicleInfoVO && modalInfo.waybillVehicleInfoVO.length > 0">
                        <tr>
                            <th colspan="2">序号</th>
                            <th colspan="3">需求号</th>
                        ...
                        <tr v-for="(item,index) in modalInfo.waybillVehicleInfoVO" :key="index">
                            <td colspan="2">{{index+1}}</td>
                            <td colspan="3">{{item.demandId}}</td>
                    ...
                    <div v-if="!modalInfo.waybillVehicleInfoVO">暂无数据</div>
                </div>
            </a-modal>
        </div>
    </div>
...
<script>
    ...
    import {API_ExportTrainTable} from 'api'
    export default {
        name : "TrainInfoNew",
        props:['datas', 'params','exportFlag'],
        created(){
            this.dataSource = []
            if (this.datas) {
                this.dataSource.push(this.datas)
            }
        },
        watch:{
            datas(){
        ...
        data(){
            return{
                columns:[
                    { title: '运单号', dataIndex:'serialNo' },
                    { title: '车种', dataIndex:'carType'},
                    { title: '车号', dataIndex:'carNumber' },
                    { title: '票重（吨）', dataIndex:'weight'},
                    { title: '操作', scopedSlots: { customRender: 'operation' }, dataIndex:'operation' }
                ],
                trainModalIsShow: false,
        ...
        methods:{
            ...
            jumpToTail(){
                window.open('/logistics/LogisticsDetailTrainNew?waybillId=' + this.params.waybillId)
            },
            exportXls() { // 导出车号表
                API_ExportTrainTable({id: this.params.waybillId}).then(res => {
...
<style lang="less" scoped>
    ...
    ::v-deep.ant-modal-body{overflow: hidden}
</style>
