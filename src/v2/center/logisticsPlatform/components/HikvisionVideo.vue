<template>
	...
	>
      ...
      <br /> -->
			...
			<div
				class="plug-content"
				v-if="plugStatus == 'Fail'"
			>
				<div class="error-tip-content">
					<img
						class="warning-icon"
						src="@/v2/assets/imgs/logisticsPlatform/plug_warning_icon.png"
						alt=""
					/>
					<span class="warning-text">{{ playText }}</span>
				</div>
				<div class="plug-list-content">
					<!-- <img class="warning-icon" src="@/v2/assets/imgs/logisticsPlatform/plug_warning_icon.png" alt="">
          <span class="warning-text">{{ playText }}</span> -->
					<div class="plug-list">
						<div class="plug-item">
							<img
								class="plug-icon"
								src="@/v2/assets/imgs/logisticsPlatform/browser_plug_icon.png"
								alt=""
							/>
							<span class="plug-title">浏览器播放插件</span>
						</div>
						<div style="height: 30px"></div>
						<div class="plug-item">
							<img
								class="plug-icon"
								src="@/v2/assets/imgs/logisticsPlatform/video_plug_icon.png"
								alt=""
							/>
							<span class="plug-title">视频播放插件</span>
						</div>
					</div>
					<a-button type="primary">
						<a
							href="https://admin.shdatalink.com/plugin-download/VideoWebPlugin.zip"
							download="VideoWebPlugin.zip"
							>一键下载</a
						>
					</a-button>
				</div>
				...
				<span
					v-if="playMode == 0"
					class="remark"
					>注:使用插件播放，可展示更清晰、流畅的监控画面效果</span
				>
			</div>
			<div
				class="plug-content"
			...
			>
				<img
					class="plug-status-icon"
					src="@/v2/assets/imgs/logisticsPlatform/videoPlug_status_icon.png"
					alt=""
				/>
				<div
					class="plug-status-text"
...
export default {
	...
	data() {
		const plugStatus = this.isToInstallPlug == true ? PlugStatus.Fail : PlugStatus.UnStarted;
		console.log('this.isToInstallPlug:', this.isToInstallPlug);
		const toInstall = plugStatus == PlugStatus.Fail ? true : false; // 是否去安装
		return {
	...
	computed: {
		playText: function () {
			let text = '插件未启动';
			switch (this.plugStatus) {
				case PlugStatus.UnStarted:
					text = '插件未启动';
					break;
				case PlugStatus.Initializing:
					text = '插件未启动\n正在尝试启动，请稍候...';
					break;
				case PlugStatus.Fail:
					text = '插件启动失败，请检查插件是否安装。如未安装，请下载安装以下两个插件：';
					break;
				case PlugStatus.Success:
					text = '插件启动成功';
					break;
				case PlugStatus.UnSupport:
					text = '仅支持windows电脑播放';
					break;
				default:
					break;
	...
	methods: {
		...
		// 初始化+预览
		createdVideo() {
			// this.plugStatus = PlugStatus.Initializing;
			if (this.os !== 'window') {
				// 电脑不是window 不支持
				this.plugStatus = PlugStatus.UnSupport;
				return;
			}
			this.initPlugin(() => {
				// 初次进入页面时覆盖问题
			...
			this.oWebControl = new WebControl({
				...
				cbConnectSuccess: () => {
					...
					this.oWebControl
						...
						.then(
							() => {
								this.plugStatus = PlugStatus.Success;
								// 启动插件服务成功
							...
							() => {
								this.plugStatus = PlugStatus.Fail;
								// 启动插件服务失败
							}
						);
				},
				cbConnectError: () => {
					...
					if (this.initCount < 3) {
						...
						if (this.os == 'window') {
							this.plugStatus = PlugStatus.Fail;
							// this.playText = '插件启动失败，请检查插件是否安装（需要安装视频播放插件和浏览器视频播放插件）！<br/>（如已下载安装插件，请刷新页面尝试播放视频）。';
						} else {
							this.plugStatus = PlugStatus.Fail;
							// this.playText ="仅支持windows电脑播放"
						}
					}
				},
