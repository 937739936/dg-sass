<template>
  <a-modal
    ...
    :footer="null"
  >
    <div class="title" >地址及围栏详情</div>
    <div class="main">
      <div class="map" id="circleMap" :class="isEdit ? '' : 'main2'"></div>
      <div class="mapCon" v-show="isEdit">
        <a-form :form="form" :colon="false">
          <a-form-item label="站台名称"  class="special">
            {{info.platFormName}}
          </a-form-item>
          <div class="" style="margin-top:10px">
            <a-form-item label="站台地址" style="display:inline-block">
              <a-cascader
            ...
            <a-form-item label=""   style="display:inline-block;margin-left:20px">
              <a-select
                showSearch
                allowClear
                style="width:424px"
          ...
          <div style="margin-top:10px">
            <a-form-item label="围栏半径" style="display:inline-block">
              <a-input-number
        ...
        <div class="footer">
          <div style="margin-bottom:20px">
             <a-button type="primary" @click="affirm">保存</a-button>
          </div>
           <a-button @click="isEdit = false" class="cancel" >取消</a-button>
         
        </div>
      </div>
      <div class="mapCon mapCon2" v-show='!isEdit'>
        <div style="color:rgba(0, 0, 0, 0.80)">
          <p style="margin-bottom:10px;color: rgba(0, 0, 0, 0.40);"><span>站台名称：</span> <span style="margin-left:14px;color:rgba(0, 0, 0, 0.80)">{{info.platFormName}}</span> </p>
          <p style="margin-bottom:10px;color: rgba(0, 0, 0, 0.40);"><span>站台地址：</span> <span style="margin-left:14px;color:rgba(0, 0, 0, 0.80)">{{info.area}}  {{info.address}}</span> </p>
          <p><span style="color: rgba(0, 0, 0, 0.40);">围栏半径：</span> <span style="margin-left:14px;color:rgba(0, 0, 0, 0.80)">{{info.radius}}km</span> </p>
        </div>
        <div class="footer" style="line-height:80px">
...
import {
  API_deliverGetPortSearchHistorys,
...
china.children.forEach(el => {
  ...
  el.children && el.children.forEach(el2 => {
    el2.value = String(el2.label)
     el2.children && el2.children.forEach(el3 => {
...
export default {
    ...
    PortSearch() {
      ...
      if (circle.remove) {
        ...
        if (
          !data.some(
            (item) =>
              `${item.originPortLat}${item.originPortLon}` ==
  ...
  methods: {
    ...
    //加载弹框
    async show(circle, flag) {
      this.isEdit = flag


      const newInfo = cloneDeep(circle)

      newInfo.radius = newInfo.electronicFenceRadius

      newInfo.lon = newInfo.longitude
      newInfo.lat = newInfo.latitude



      newInfo.platFormName = newInfo.name
      newInfo.name = newInfo.addressKeywords || newInfo.address

      this.handleCircleSearch( newInfo.name)
      ...
      // Object.assign(circle, { name:  circle.address })
      ...
      this.visible = true;
      this.circle = { ...this.circle, ...newInfo };
      ...
      //有详细地址渲染表单地址
      if (newInfo.lon && newInfo.lat && newInfo.address && newInfo.name) {
        //如果当前值再记录中，不进行添加，否则添加新值
        if (
          !this.PortSearch.some(
            (item) =>
              `${item.originPortLat}${item.originPortLon}` ==
              `${newInfo.lat}${newInfo.lon}`
          )
        ) {
        ...
        this.$nextTick(() => {
          this.form.setFieldsValue({
            circle: `${newInfo.lat}${newInfo.lon}`,
            name: newInfo.address,
          });
        });
      }
      /** 回显 */
      ...
      const areaArr = this.info.area ?  this.info.area.split('/') : ['北京市', '北京市', '东城区']
      this.form.setFieldsValue({
          sendA: areaArr,
        });
      //绘图
    ...
    async onValuesChange(props, values) {
      ...
      if (values.hasOwnProperty("circle")) {
        //如果选中港口
        if (values.circle) {
          let circlePort = this.PortSearch.find(
            (item) =>
              `${item.originPortLat}${item.originPortLon}` == values.circle
          );

          if (
            circlePort.originPortLon != circle.lon ||
            circlePort.originPortLat != circle.lat
          ) {
            // 替换经纬度
            this.circle = {
              ...circle,
            ...
            if(circlePort.pname) {
              const sendA = [circlePort.pname, circlePort.cityname, circlePort.adname]
              /** 回显站台地址 省市区 */
   ...
   async initMap(flag) {
    ...
    setMarkerLabel() {
      ...
      if (markerLabel) {
        ...
        markerLabel.setContent(
          `<div class="markerLabel markerLabelRight">${circle.address}</div>`
        );
      } else {
    ...
    handleMarker(circle) {
      ...
      timeout3 = setTimeout(() => {
        if (
          circle.lon == circlePosition.lon &&
          circle.lat == circlePosition.lat
        ) {
          const params = {
