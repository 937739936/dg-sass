...
import {
	...
	signSaveAgreementServe,
	getAgreementServeHashList,
	autoSignAgreementServe
...
export default {
	...
	methods: {
		...
		autoSignature() {
			...
			autoSignAgreementServe({ id: this.$route.query.id })
				.then(res => {
					if (res.success) {
						this.$message
							.success('签署完成')
		...
		step1(obj) {
			return getAgreementServeHashList({
				id: this.$route.query.id,
				cert: obj.cert
			});
		},
		step2() {
			return signSaveAgreementServe({
				id: this.$route.query.id
			});
		},
		signApply() {
