...
export default {
	...
	methods: {
		...
		autoSignature() {
			...
			autoSingTransfer({ id: this.$route.query.id })
				.then(res => {
					if (res.success) {
						this.$message
							.success('签署完成')
							.then(() => this.$router.push('/center/logisticsPlatform/warehouseReceipt/warehouseReceiptTransfer/list'));
		...
		step1(obj) {
			return getTransferHashList({
				id: this.$route.query.id,
				cert: obj.cert
			});
		},
		step2() {
			return signSaveTransfer({
				id: this.$route.query.id
			});
		},
		signApply() {
		...
		submitSign(cfcaSealList, certModel) {
			if (certModel == 'TRUST') {
				...
				sign.call(
					...
					'/center/logisticsPlatform/warehouseReceipt/warehouseReceiptTransfer/list',
					true
				);
			}
		},
