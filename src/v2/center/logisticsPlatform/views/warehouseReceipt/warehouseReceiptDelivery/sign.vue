		...
		<a-card :bordered="false">
			...
			>
				<a-tab-pane
					...
					:tab="item.fileTypeDesc + item.warehouseReceiptNo"
				></a-tab-pane>
			</a-tabs>
			<div
				class="content"
...
import {
	...
	API_deliveryUKeySignUpdate,
	API_deliveryUKeyGetPDFHashList,
	API_deliveryAutoSignature
...
export default {
	...
	methods: {
		async getDetail() {
			...
			const res = await API_warehouseReceiptDeliveryDetail(params);
			this.detailData = res.data;
			this.signList = res.data.waitSignAttachmentList || [];
			this.currentPdf = this.signList.length ? this.signList[0].path : '';
		},
		goBack() {
			this.$router.push('/center/logisticsPlatform/warehouseReceipt/warehouseReceiptDelivery/auditList');
		},
		...
		autoSignature() {
			...
			API_deliveryAutoSignature({ id: this.$route.query.id })
				.then(res => {
					if (res.success) {
						this.$message.success('签署完成').then(() => this.goBack());
		...
		step1(obj) {
			return API_deliveryUKeyGetPDFHashList({
				id: this.$route.query.id,
				cert: obj.cert
			});
		},
		step2() {
			return API_deliveryUKeySignUpdate({
				id: this.$route.query.id
			});
		},
		signApply() {
		...
		submitSign(cfcaSealList, certModel) {
			if (certModel == 'TRUST') {
				...
				sign.call(
					...
					'/center/logisticsPlatform/warehouseReceipt/warehouseReceiptDelivery/auditList',
					true
				);
			}
		},
		async downAll() {
			const res = await API_warehouseReceiptDeliveryDownload({ id: this.$route.query.id, type: 2 });
		...
		async confirmCancel() {
			...
			const res = await API_deliveryAudit_receipt(params);
