		...
		>
			...
			<div style="margin-top: 20px">
				<a-table
					...
					:pagination="false"
					:rowKey="record => record.bizContractNo"
					:loading="loading"
				>
				</a-table>
			</div>
...
const relationOrderListColumns = [
	{ title: '仓储合同编号', dataIndex: 'bizContractNo', fixed: 'left' },
	{ title: '合同状态', dataIndex: 'statusDesc' },
	{ title: '签订日期', dataIndex: 'signDate' },
	{ title: '生效日期', dataIndex: 'effectiveDate',customRender: (t,r) => `${r.effectiveStartDate}至${r.effectiveEndDate}`},
	{ title: '仓储企业', dataIndex: 'warehouseOwnerCompanyName', customRender: (t,r) => t || r.warehouseCompanyName},
	{ title: '承租方名称', dataIndex: 'warehouseTenantCompanyName' },
];
	...
	computed: {
		rowSelection() {
			...
			return {
				...
				onSelect: record => {
					t.selectedRowKeys = [record.bizContractNo];
	...
	methods: {
		...
		onClickRow(record) {
			return {
				on: {
					click: () => {
						this.selectedRowKeys = [record.bizContractNo];
