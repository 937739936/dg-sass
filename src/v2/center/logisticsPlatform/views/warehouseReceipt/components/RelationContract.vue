...
import { getWarehouseReceiptContractList } from '@/v2/center/logisticsPlatform/api/warehouseReceipt';
...
const columns2 = [
	...
	{ title: '收货人', dataIndex: 'consigneeCompanyName', customRender: t => t || '-' },
	{
		...
		dataIndex: 'startDate',
		customRender: (txt, record) => {
			return record.startDate ? `${record.startDate} - ${record.startDate}` : '';
		}
	},
	{ title: '签订日期', dataIndex: 'signDate' },
	{ title: '运输方式', dataIndex: 'transportModeDesc' },
	...
	computed: {
		rowSelection() {
			...
			return {
				...
				selectedRowKeys,
				onSelect: record => {
					t.selectedRowKeys = [record.orderContractId];
					t.$forceUpdate();
					t.selected = record;
				}
			};
	...
	methods: {
		...
		getRelationOrderList(data) {
			...
			const params = {
				...
				type: this.type === 'IN' ? 'BUY' : 'SELL',
				contractType: this.contractType
			};
			// getWarehouseReceiptContractList
			// getRelOrderContractList
			getWarehouseReceiptContractList(params)
				.then(res => {
					if (res.success) {
						...
						if (this.relateOrderNo) {
							const resultIds = result.records.filter(item => (item.paperContractNo || item.contractNo) == this.relateOrderNo)[0]
								?.id;
