...
export default {
	...
	methods: {
			...
			this.signList = [
				{
					...
					name: this.detailData.warehouseReceiptNo
				}
			];
			this.currentPdf = this.detailData.fileUrl;
		},
		goBack() {
		...
		autoSignature() {
			...
			autoSingOpen({ id: this.$route.query.id })
				.then(res => {
					if (res.success) {
						this.$message
							.success('签署完成')
							.then(() => this.$router.push('/center/logisticsPlatform/warehouseReceipt/warehouseReceiptOpen/list'));
		...
		submitSign(cfcaSealList, certModel) {
			if (certModel == 'TRUST') {
				...
				sign.call(
					...
					'/center/logisticsPlatform/warehouseReceipt/warehouseReceiptOpen/list',
					true
				);
			}
		},
		async downAll() {
			const res = await API_getCommonDownload(this.detailData.fileUrl);
			const name = `电子仓单-${this.detailData.warehouseReceiptNo}`;
			const item = this.detailData.warehouseReceiptAttachmentList.find(el => el.fileType == 'WAREHOUSE_RECEIPT') || {
				name: '电子仓单.pdf'
			};
