<template>
	<div>
		...
		>
			...
			<!-- 关联合同的 榜单明细 -->
			<!-- <div v-if="ticketDetailDataSource && ticketDetailDataSource.length">
				<div class="tip-box">
					<div class="spread-box">
						<span
							v-show="!isSpread1"
							class="btn"
							@click="isSpread1 = true"
							>展开
							<img
								class="icon"
								src="@/v2/assets/imgs/contract/arrow-down.png"
					...
					<div v-if="isSpread1">
						<p
							v-for="(item, index) in ticketDetailDataSource"
							:key="index"
						>
							<span class="tip-title">{{ item.title }}</span
							>：可支持格式为{{ item.acceptTip }}的附件，支持多张，单个附件大小不得超过100M的文件
					...
					<div v-else>
						<p
							v-for="(item, index) in [ticketDetailDataSource[0]]"
							:key="index"
						>
							<span class="tip-title">{{ item.title }}</span
							>：可支持格式为{{ item.acceptTip }}的附件，支持多张，单个附件大小不得超过100M的文件
				...
				<a-table
					class="new-table"
					:columns="columns"
					:data-source="ticketDetailDataSource"
					:rowKey="(record, index) => record.id"
					style="margin-top: 30px; margin-bottom: 10px"
					:pagination="false"
				>
...
// import { commonAttachFilesUpload,API_UPLOAD} from "v2/api/upload";
import { commonUpload, API_UPLOAD_WATER_MARk, API_UPLOAD_STATION } from '@/v2/api';
// /common/upload
...
export default {
	props: ['transportMode', 'handleType', 'isCoreCompany', 'isManager', 'isRelation'],
	data() {
		return {
			columns,
			delVisible: false,
