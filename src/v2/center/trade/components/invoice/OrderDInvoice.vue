	...
	<div>
		...
		<div class="table-top">
			...
			<a-button
				...
				@click="exportInvoice(1)"
				>下载发票附件</a-button
			>
			<a-button
				type="primary"
				:disabled="hasSelected"
				@click="exportInvoice(2)"
				>导出发票四要素</a-button
			>
		</div>
		<a-table
		...
		>
			...
			>
				<a-space>
					...
					<a
						target="_blank"
						:href="BASE_NET + `api/invoice/common/pdf?id=${record.id}`"
						>PDF</a
					>
				</a-space>
			</span>
...
import { API_EXPORT_INVOICE, API_InvoiceDetailNew } from '@/v2/center/trade/api/invoice';
