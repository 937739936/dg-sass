<template>
	...
	<div id="trainInfo">
		<div class="title">
			...
			<a-button
				v-if="params.status == 2 || params.status == 3 || params.status == 4"
				type="primary"
				ghost
				class="trans-track"
				@click.native="jumpToTail()"
				><a-icon type="car" />查看运输轨迹</a-button
			>
		</div>
			...
			>
				...
				>
					<a
						href="javascript:;"
						class="edit-btn"
						v-if="record.businessType != 'OTHER'"
						@click="showDeliverInfo(record)"
						>查看运单</a
					>
					<span
						class="line"
						v-if="datas.vehicleFlag"
						>|</span
					>
					<a
						href="javascript:;"
						class="edit-btn"
						@click="showTrainTable(record)"
						v-if="datas.vehicleFlag"
						>查看车号表</a
					>
				</template>
			...
			>
				<div>
					...
					<table
						...
						v-if="modalInfo.waybillVehicleInfoVO && modalInfo.waybillVehicleInfoVO.length > 0"
					>
						<tr>
							<th colspan="2">序号</th>
							<th colspan="3">需求号</th>
						...
						<tr
							v-for="(item, index) in modalInfo.waybillVehicleInfoVO"
							:key="index"
						>
...
import { API_ExportTrainTable } from '@/v2/center/trade/api/receive';
export default {
	name: 'TrainInfoNew',
