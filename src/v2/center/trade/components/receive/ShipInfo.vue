<template>
	<div id="shipInfo">
		<div class="title">
			...
			<span
				class="remarks"
				v-if="freightPayType != filterCodeByTextName('下游支付', 'freightPayTypeDict') && !disabled"
				>(至少录入一个船舶信息)</span
			>
		</div>
			...
			>
				...
				>
					<a
						href="javascript:;"
						class="edit-btn"
						@click="editShipInfo(record, index)"
						>编辑</a
					>
							...
							>
								<a-input
									...
									v-decorator="[
										'voyageNo',
										{
											rules: [{ pattern: /^.{0,30}$/, message: '最大输入30个字符' }]
										}
									]"
								/>
							</a-form-item>
...
export default {
	...
	methods: {
		...
		handleStartPortSelect(val, type) {
