		...
		>
			...
			<div class="table-box">
				...
				>
					...
					>
						<template v-if="text"> {{ text | formatMoney(2) }}元/吨 </template>
						<template v-else>
							{{ items.basePriceDesc || '-' }}
						</template>
					</span>
...
import { API_GETSETTLECONTRAT, API_GETSETTLEAPPLYCHECK } from '@/v2/center/trade/api/settle';
import { selectOnlineContractColumns } from '../columns/columns.js';
...
const searchList = [
	...
	},
	{
		...
		allowClear: true,
		placeholder: '请选择煤种',
		options: filterCodeByKey('goods_type')
	},
	{
...
export default {
	...
	computed: {
		...
		columns() {
			let columns = selectOnlineContractColumns;
			let dataIndex = '';
			switch (this.type) {
				case 'buy':
					dataIndex = 'buyerName';
	...
	methods: {
		async handleSubmit() {
			...
			let orderId = this.selectedRowKeys[0];
			//选中合同校验合同是否可结算
			...
			if (res.success && !res.data) {
				this.$router.push({
					path: `/center/settle/${this.type}/onlineadd`,
					query: {
						orderId
					}
				});
