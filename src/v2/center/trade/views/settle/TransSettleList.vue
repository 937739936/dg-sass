		...
		<a-card :bordered="false">
			<div class="s-title-btn">
				...
				<a-button
					type="primary"
					@click="settleAdd"
					v-auth="'dgChain:settle:transSettle:bl'"
							...
							<template>
								<a-space>
									<a
										...
										@click="toDetail(item)"
										v-auth="'dgChain:settle:transSettle:ck'"
										>查看</a
									>
									<a
										...
										v-auth="'dgChain:settle:transSettle:edit'"
										>修改</a
									>
									<a
										href="javascript:void(0)"
										@click="deleteSettle(item)"
										v-auth="'dgChain:settle:transSettle:delete'"
										>删除</a
									>
								</a-space>
							</template>
			...
			>
				<template slot="footer">
					<a-button @click="downPdf(BASE_NET + pdfShowUrl)">下载PDF</a-button>
					<a-button
						type="primary"
...
export default {
	...
	methods: {
		...
		downloadSettleFiles(item) {
			...
			API_DownloadSettleFiles({ statementId: item.id }).then(res => {
		...
		deleteIn(item) {
			API_transport_settle_delete({ statementId: item.id }).then(res => {
				if (res.success) {
					if (res.data) {
		...
		editSettle(item) {
			this.$router.push({
				path: '/center/settle/transport/edit',
				query: {
					statementId: item.id
				}
			});
