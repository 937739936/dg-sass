	...
	<div>
		...
		>
			...
			<a-row>
				<a-col
					:span="8"
					v-if="editableLadingInfo.warehouseToReceivable"
				>
					<a-row
						type="flex"
						justify="start"
						...
						<a-form-item label="实际提货单位">
							<a-input
								...
								v-decorator="[
									'actualLadingUnit',
									{
										rules: [{ required: true, message: `实际提货单位必填` }, { validator: actualLadingUnitValidator }],
										validateTrigger: ['blur']
									}
								]"
							/>
						...
						<a-form-item label="实际收货单位">
							<a-input
								...
								v-decorator="[
									'actualReceivingUnit',
									{
										rules: [{ required: true, message: `实际收货单位必填` }, { validator: actualLadingUnitValidator }],
										validateTrigger: ['blur']
									}
								]"
							/>
...
export default {
	...
	methods: {
		...
		actualLadingUnitValidator(rule, value, callback) {
			if (!value) {
				callback();
				return;
			}
			const actualLadingUnit = this.form.getFieldValue('actualLadingUnit');
			const actualReceivingUnit = this.form.getFieldValue('actualReceivingUnit');
			if (actualLadingUnit == actualReceivingUnit) {
				callback('实际提货单位与实际收货单位相同，请核验');
				return;
			}
			callback();
		},
					...
					async (err, values) => {
						...
						if (err) {
							...
							try {
								...
								//   return;
								// }
								if (this.currentPlace?.stationId) {
