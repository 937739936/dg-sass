...
export default {
	name: 'MyEditor',
	props: ['content', 'placeholder', 'id', 'sensitiveWordsList', 'maxLength'],
	...
	watch: {
		content: {
			handler(val) {
				if (val && this.editor && !this.isChange) {
		...
		onChange(editor) {
			...
			this.debounce(() => {
				...
				if (editor.isEmpty()) {
					...
					this.$emit('change', this.filterContent(elem));
				}
			}, 800);
		},
		onBlur(editor) {
			...
			if (editor.isEmpty()) {
				...
				this.createContentHtmlNode();
				const elem = document.querySelector(`#${this.id}`);
				const innerHtml = this.filterContent(elem);
				if (innerHtml.replace(/<[^>]+>/g, '').length > this.maxLength) {
					this.$message.warn(`输入内容长度不能超过${this.maxLength}字符`);
				}
				this.$emit('blur', innerHtml);
			}
		}
	},
	mounted() {},
