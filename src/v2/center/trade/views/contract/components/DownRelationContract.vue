			...
			<div class="table-box">
				...
				>
					...
					<template
						...
						slot-scope="text, items"
					>
						<p v-if="contractType === 'on'">{{ text ? text + '吨' : '-' }}</p>
						<p v-else>{{ items.contractQuantity ? items.contractQuantity + '吨' : '-' }}</p>
					</template>
					<template
						slot="basicPrice"
						slot-scope="text, items"
					>
						<p v-if="contractType === 'on'">{{ items.followTheMarket ? '随行就市' : text ? text + '元/吨' : '-' }}</p>
						<p v-else>{{ items.followTheMarket ? '随行就市' : items.contractPrice ? items.contractPrice + '元/吨' : '-' }}</p>
					</template>
					<template
...
const relationOrderListColumnsBuy = [
	...
	{ title: '基准价格', dataIndex: 'basicPrice', align: 'center', scopedSlots: { customRender: 'basicPrice' } },
	{
		title: '品名',
		dataIndex: 'goodsName',
		customRender: text => {
...
export default {
	...
	computed: {
		...
		//电子合同，线下合同运输方式不同，电子合同没有其他
		transTypeList() {
			let list = [];
			//线下合同采购有运输方式有无
			if (this.contractType === 'off' && this.type === 'buy') {
				list = transTypeList;
			} else {
				list = transTypeList.filter(item => {
		...
		tab1() {
			return this.type === 'buy' ? '电子采购合同' : '电子销售合同';
		},
		tab2() {
			return this.type === 'buy' ? '线下采购合同' : '线下销售合同';
		},
		//电子合同，线下合同表头不同
	...
	props: ['type', 'relateOrderNo'], // type=buy是关联采购合同，type=sell是关联销售合同
	data() {
		return {
			getPopupContainer,
	...
	methods: {
		getRelationOrderList(data) {
			...
			if (this.searchModalParamsRelation.contractSignTimeStart && this.searchModalParamsRelation.contractSignTimeEnd) {
				this.searchModalParamsRelation.contractSignTimeStart = this.searchModalParamsRelation.contractSignTimeStart + ' 00:00:00';
				this.searchModalParamsRelation.contractSignTimeEnd = this.searchModalParamsRelation.contractSignTimeEnd + ' 23:59:59';
			}
			this.loading = true;
		...
		//外部引用方法打开弹框
		showRelationOrderList(list) {

			if(list) {
					this.currentSelectContractList = list || [];
			}	
		...
		resetValuesRelation() {
			...
			this.searchList = this.contractType === 'on' ? this.searchListOn : this.searchListOff;
