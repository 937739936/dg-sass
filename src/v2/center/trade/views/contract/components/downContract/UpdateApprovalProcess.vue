...
import { getOfflineContractAuditChainAndOperator, updateOfflineContractAuditChainRel } from '@/v2/center/trade/api/downcontract';
...
export default {
	...
	methods: {
		async show(data) {
			...
			this.visible = true;
			const params = {
				offlineContractNo: data.contractNo,
				industryType: 'COAL'
			};
		...
		getoaauditcodelist() {
			API_GETOAAUDITCODELIST().then(res => {
				if (res.success) {
					...
					const params = {
						offlineContractNo: this.info.contractNo,
						industryType: 'COAL'
					};
					getOfflineContractAuditChainAndOperator(params).then(res => {
						if (res.success) {
		...
		submit() {
			this.form.validateFields((err, values) => {
				if (!err) {
					...
					updateOfflineContractAuditChainRel({
