    ...
    okBtnText="合同完结"
		>
			...
			<template v-if="!contractFinishInfo.validPass">
              ...
              <div v-if="record.errorType =='AMOUNT_ALL_EQUALS_ZERO'">发票金额、结算单金额、{{payAmountDes}}缺失，请补充</div>
    ...
    goDetail(item) {
      ...
      if(item.validType == 'CONTRACT') {
        ...
        if(this.contractFinishInfo.orderType == 'BUY') {
          // 卖方未签署
          if(item.errorType == 'CONTRACT_SELL_NOT_DOUBLE_SIGN' ||  (item.errorType == 'CONTRACT_NOT_EXEC_TING' && this.contractFinishInfo.contractType =='ONLINE')) {
          ...
          // 线下
          if(item.errorType == 'CONTRACT_NOT_EXEC_TING' && this.contractFinishInfo.contractType =='OFFLINE') {
        ...
        if(this.contractFinishInfo.orderType == 'SELL') {
          ...
          // 买方未签署
          if(item.errorType == 'CONTRACT_BUY_NOT_DOUBLE_SIGN' || (item.errorType == 'CONTRACT_NOT_EXEC_TING' && this.contractFinishInfo.contractType =='ONLINE')) {
            path = '/center/contract/sell/online/detail'
            query = { 
              id: item.params.id,
              type: this.contractFinishInfo.orderType,
              initiatorUscc: item.params.initiatorUscc,
           ...
           // 线下
          if(item.errorType == 'CONTRACT_NOT_EXEC_TING' && this.contractFinishInfo.contractType =='OFFLINE') {
            path = '/center/contract/sell/offline/detail'
            query = { 
              id: item.params.id,
              type: 'sell',
              initiatorUscc: item.params.initiatorUscc,
      ...
      if(item.validType == 'RECEIPT') {
        path = '/center/ladingbill/receipt/detail'
        query = { 
          id: item.params.id,
        }
      }
      ...
      if(item.validType == 'STATEMENT') {
        ...
        if(this.contractFinishInfo.contractType == 'ONLINE') {
          ...
          if(this.contractFinishInfo.orderType == 'BUY') {
            path = '/center/settle/buy/onlinedetail'
            query = { 
              id: item.params.id,
            }
          }
          if(this.contractFinishInfo.orderType == 'SELL') {
            path = '/center/settle/sell/onlinedetail'
            query = { 
              id: item.params.id,
            }
          }
          ...
          if(this.contractFinishInfo.orderType == 'BUY') {
            path = '/center/settle/buy/offlinedetail'
            query = { 
              id: item.params.id,
            }
          }
          ...
          if(this.contractFinishInfo.orderType == 'SELL') {
            path = '/center/settle/sell/offlinedetail'
            query = { 
              id: item.params.id,
            }
          }
      ...
      if(item.validType == 'INVOICE') {
        ...
        if(item.params.invoiceType == 'TRADE_INVOICE') {
          if(this.contractFinishInfo.orderType == 'BUY') {
            path = '/center/invoice/buy/detail'
            query = { 
              id: item.params.id,
              industryType: 'COAL',
            }
          } else {
            path = '/center/invoice/sell/detail'
            query = { 
              id: item.params.id,
              industryType: 'COAL',
            }
          ...
          path = '/center/invoice/freight/detail'
          query = { 
            id: item.params.id,
            industryType: 'COAL',
          }
