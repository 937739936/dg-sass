<template>
	<div>
		<div class="slTitleAssis">
			...
			<a-button class="relBtn" @click="relationInvoice" type="primary" ghost>关联发票</a-button>
		</div>
		<a-table
			:pagination="false"
		...
		>
			<div slot="title">
				<span>选择发票</span>
				<a class="addBtn" @click="addInvoice">
					<a-icon
						type="plus-circle"
			...
			<div class="totalRowBox" v-if="invoiceTotal.total">
				<span>已选：{{invoiceTotal.total}}张发票</span>
				<a-space :size="20" class="totalRow">
					<span>价税合计<em>{{invoiceTotal.totalAmountTotal | formatMoney(2)}}</em></span>
					<span>归属价税合计<em>{{invoiceTotal.currentContractSplitedAmountTotal | formatMoney(2)}}</em></span>
				</a-space>
			</div>
			<div class="footer">
				<a-space :size="20">
...
const transColumns = [
	...
	{ title: '印花税税额(元)', dataIndex: 'stampTaxFlagAmount', align: 'center',customRender: text => text && text.toLocaleString() },
	{ title: '含印花税合计(元)', align: 'center', dataIndex: 'stampTaxFlagTotalAmount',customRender: text => text && text.toLocaleString() },
	{ title: '拆分到本合同金额(元)', dataIndex: 'currentContractSplitedAmount', align: 'center',customRender: text => text && text.toLocaleString() },
...
const transColumnsDrawer = [
	...
	{ dataIndex: 'taxExcludedAmount',slots: { title: 'customTitle2' },
		customRender: text => text && text.toLocaleString(), width: 150 },
	{ title: '价税合计(元)',
		dataIndex: 'totalAmount',
		customRender: text => text && text.toLocaleString(), width: 130 
		
	},
	{ title: '是否包含印花税', dataIndex: 'stampTaxFlag', align: 'center', customRender: text => ['否', '是'][text - 1], width: 180  },
	{ title: '印花税税额(元)', dataIndex: 'stampTaxFlagAmount', align: 'center',customRender: text => text && text.toLocaleString(), width: 180  },
	{ title: '含印花税合计(元)', align: 'center', dataIndex: 'stampTaxFlagTotalAmount',customRender: text => text && text.toLocaleString(), width: 180  },
	{ title: '拆分到本合同金额(元)', dataIndex: 'currentContractSplitedAmount', align: 'center',customRender: text => text && text.toLocaleString(), width: 200  },
...
export default {
	...
	methods: {
		...
		addInvoice() {
			const path = '/center/invoice/freight/add'
			const query = {
				type: 'freight',
