<template>
	<div>
		...
		>
			...
			>
				<p>业务线货款回款金额(元)</p>
				<p>{{ statisticsData.businessLineDownStreamCollectionAmount | formatMoney(2) }}</p>
			</div>
		</div>
		<a-form
			:form="payApplyForm"
...
export default {
	...
	methods: {
		getTime(value, dateString) {
			this.$emit('getPlanPayDate', dateString + ' 23:59:59');
		},
		init() {
			this.getBankInfo();
		},
			...
			if (data) {
				...
				this.$nextTick(() => {
					...
					setTimeout(() => {
						...
						if (data.planPayDate) {
							this.$emit('getPlanPayDate', data.planPayDate + ' 23:59:59');
						}
					}, 0);
				});
			}
		...
		// 获取资金来源
		async getFundingSource(despatchTypeList) {
			return new Promise(async (resolve, reject) => {
				...
				const obj = {
					...
					despatchTypeList: despatchTypeList,
				}
				if (despatchTypeList?.length) {
					obj.payType = this.selectFundingSource?.payType
				}
				const res = await API_PaymentCreateBankProductItem(obj)
				if (!res.success) {
					reject(false);
				}
				this.fundingSourceList = res.data || [];
				//如果订单的业务类型为其他，选择时资金来源只能选择自有资金
				if (['OTHER', 'SELF_OPERATION'].includes(this.contractInfo?.businessType)) {
					this.fundingSourceList = res.data.filter(item => {
				...
				// 货物信息选中后回调
				if ((this.payApplyForm.getFieldValue('payType') !== null && this.payApplyForm.getFieldValue('payType') !== undefined) && despatchTypeList) {
					if (!this.fundingSourceList?.length) {
						reject(false)
					}
					this.fundingSourceList.forEach(item => {
						if (item.payType == this.payApplyForm.getFieldValue('payType')) {
							this.selectFundingSource = item;
						}
					});
					// 选择“自有资金”、“供应链资金”、“池保理”时，展示“付款金额”、"付款日期"字段；
					this.showGoods =
						['0', 'product-supply-chain'].includes(this.payApplyForm.getFieldValue('payType')) ||
		...
		// 资金来源改变
		async fundingSourceChange(value) {
			if (this.showBusinessline && !this.businessLineNo) {
		...
		fundingSourceOrPayTypeChange() {
			...
			const hasPaymentType = typeof paymentType !== 'undefined' && paymentType !== null;
			const hasFundingSource = typeof fundingSource !== 'undefined' && fundingSource !== null;
			// 付款类型选择-预结算付款”且资金来源选择“自有资金”或“供应链资金”或“池保理”时,展示对应的发货信息或货转信息
			const showGoods = hasPaymentType && hasFundingSource && paymentType === 'PRE_SETTLEMENT' && this.showGoods;
			this.$emit('getGoodsInfo', showGoods);
			// 付款类型选择-结算付款”且资金来源选择“自有资金”或“供应链资金”或“池保理”时,展示对应的上游结算单
			const showSettle = hasPaymentType && hasFundingSource && paymentType === 'SETTLEMENT' && this.showGoods;
		...
		async getBankInfo() {
			const res = await API_PaymentCreateReceiveAccount({
				uscc: this.contractInfo.sellerUscc
			})
			if (!res.success) return
