<template>
	<div>
		...
		<a-space v-if="settelTotal.total" :size="20" class="totalRow">
			<span>结算单数量<em>{{settelTotal.total}}</em></span>
			<span>已结算数量<em>{{settelTotal.settelQuantityTotal | formatMoney(2)}}</em>&nbsp;吨</span>
			<span>已结算金额<em>{{settelTotal.settleAmountTotal | formatMoney(2)}}</em>&nbsp;元</span>
		</a-space>
	</div>
</template>
...
import {
	API_PaymentCreateStatement
} from '@/v2/center/trade/api/pay';
const columns = [
	{ title: '结算单编号', dataIndex: 'serialNo',scopedSlots: {
	...
	methods: {
		async init() {
			const res = await API_PaymentCreateStatement({
				serialNo: this.settleType === 'up' ? this.$route.query.serialNo : this.downSerialNo,
			  contractType:  this.settleType === 'up' ? this.$route.query.contractType : this.downContractType,
			})
		...
		jumpPage(record) {
			...
			const path = this.$route.query.contractType === 'TRANSPORT' ? `/center/settle/transport/detail` : (this.settleType === 'up'? `/center/settle/buy/${contractType}detail` : `/center/settle/sell/${contractType}detail`)
