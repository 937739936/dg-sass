<template>
	<div
		...
		:style="currentStatusIsNeedTip ? { cursor: 'pointer' } : {}"
	>
		<a-tooltip
			placement="top"
			trigger="hover"
		...
		>
			<template
				v-if="currentStatusIsNeedTip"
				slot="title"
			>
				<a-spin v-if="isTipLoading">
					<a-icon
						slot="indicator"
						type="sync"
...
export default {
	...
	computed: {
		currentStatusIsNeedTip() {
			return this.listNeedTipStatus.includes(this.status);
		},
		...
		isTipLoading() {
			return this.statusTipInfoNotEmpty.isTipLoading;
		},
		statusTip() {
			return this.statusTipInfoNotEmpty.statusTip;
		}
	},
	methods: {
		visibleChange(visible) {
			this.getPaymentStatusTip(visible, { paymentStatus: this.status, paymentNo: this.paymentNo });
		}
	}
};
</script>
