	...
	>
		...
		>
			...
			<a-row>
				...
				>
					<a-form-item
						v-if="this.source == this.CONSTANTS.shyhUscc"
						label="本次付款金额"
						style="width: 100%"
					>
						<a-input-number
							...
							v-decorator="[
								'payAmount',
								{
									rules: [
										...
										{ pattern: /^[1-9]\d*(\.\d{1,2})?$/, message: '金额为正数，最多两位小数' }
									]
								}
							]"
					...
					>
						...
						<template v-if="isShowGoodsValue && !(isdetail && type !== 'additionalPayment')">
							<a
								href="#"
								style="margin: 0 4px"
							...
							<a-popover :title="null">
								<template
									slot="content"
									v-if="type === 'additionalPayment'"
								>
									<p>本次付款金额≤max【本次转让金额*（有票融资比例-无票融资比例），本次转让金额*有票融资比例-首笔付款金额】</p>
								</template>
								<template
									slot="content"
									v-else
...
export default {
	...
	computed: {
			...
			return {
				...
				onSelectAll: (selected, selectedRows) => {
					if (!selectedRows.length) {
						that.goodsReceiveSelectedRowKeys = [];
						that.goodsSelectedRowKeys = [];
						that.receiveNoList = [];
						that.resultDetail = [];
						this.$emit(
							'selectedChange',
					...
					that.goodsSelectedRowKeys = selectedRows.map(el => el.id);
					selectedRows.forEach(el => {
						if (el.receiveRecordsList && el.receiveRecordsList.length) {
							el.receiveRecordsList.forEach(el2 => {
								that.goodsReceiveSelectedRowKeys.push(el2.id);
								that.receiveNoList.push(el2.receiveNo);
								that.resultDetail.push({ id: el2.id, deliverBatchId: el2.deliverId });
							});
						} else {
							that.resultDetail.push({ deliverBatchId: el.id });
						}
					});
		...
		goodsColumns() {
			if (
				(this.editDetail && this.editDetail.businessType == 'WAREHOUSE_RECEIPTS_PLEDGE') ||
				(this.orderDetail && this.orderDetail.businessType == 'WAREHOUSE_RECEIPTS_PLEDGE')
			) {
				return [
					...
					{ title: '付款节点', dataIndex: 'payNode', customRender: text => info[text] },
					{ title: '货转凭证', dataIndex: 'goodsTransferFlag' },
	...
	methods: {
		...
		onSelectGoods(record, selected) {
			...
			if (!record.yspzUrl) {
				if (
					!(
						record.transType == this.CONSTANTS.transportModeDict.SHIP &&
						this.$route.query.coalType == this.CONSTANTS.coalTypeDict.STEAM_COAL
					) &&
					!(
						(record.transType == this.CONSTANTS.transportModeDict.TRAIN ||
							record.transType == this.CONSTANTS.transportModeDict.AUTOMOBILE_AND_TRAIN) &&
						this.$route.query.coalType == this.CONSTANTS.coalTypeDict.STEAM_COAL
					)
				) {
					// 非动力煤船运校验 非动力煤火运、汽运和火运中火运
			...
			if (record.receiveRecordsList && record.receiveRecordsList.length > 0) {
				if (selected) {
					record.receiveRecordsList.forEach(item => {
						...
						this.resultDetail.push({ id: item.id, deliverBatchId: item.deliverId });
						// }
					});
				} else {
					record.receiveRecordsList.forEach(item => {
		...
		onSelectGoodsReceive(record, selected, selectedRows) {
			...
			if (selected) {
				this.goodsData.map(item => {
					item.receiveRecordsList.map(i => {
						if (i.receiveNo == record.receiveNo) {
							this.goodsSelectedRowKeys.push(item.id);
							this.resultDetail.push({ id: i.id, deliverBatchId: item.id });
						}
					});
				});
			} else {
		...
		// 上海银行 [本次付款金额] 取值 [本次转让金额]
		syncPaymountValueIfNeed() {
			if (this.source == this.CONSTANTS.shyhUscc) {
				let amount = this.modalFileForm.getFieldValue('thistransferAmount');
				this.modalFileForm.setFieldsValue({
			...
			this.modalFileForm.validateFields(['thistransferAmount'], (error, res) => {
				if (!error) {
					...
					if (this.$route.query.additionalPaymentEdit || this.$route.query.isAdditionalPaymentCreat) {
						API_paymentAdditionalCalMaxPayAmount({
							serialNo: this.$route.query.mainPaymentNo || this.$route.query.serialNo,
