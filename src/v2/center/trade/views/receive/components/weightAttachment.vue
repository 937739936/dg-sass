		...
		>
			...
			>
				...
				>
					<template slot="title">
						<p>
							<span>到站时间：{{ item.arriveDate }}</span>
							<span style="margin-left: 3px">车牌号：</span>
							<span
								v-for="(plat, i2) in item.plateNumberList"
								:key="i2"
								>{{ plat.plateNumber }} {{ i2 == item.plateNumberList.length - 1 ? '' : '、' }}
							</span>
						</p>
					</template>
					<p class="omit">
						...
						<span
							v-for="(plat, i2) in item.plateNumberList"
							:key="i2"
							>{{ plat.plateNumber }} {{ i2 == item.plateNumberList.length - 1 ? '' : '、' }}</span
						>
					</p>
				</a-tooltip>
			</div>
...
const columns2 = [
	{ title: '车牌号', dataIndex: 'plateNumber', scopedSlots: { customRender: 'plateNumber' }, width: 210 },
	{ title: '操作', dataIndex: 'action', scopedSlots: { customRender: 'action' }, width: 100 }
];
...
import { receiveCommonUpload, receiveOcr } from '@/v2/center/trade/api/receive';
	...
	methods: {
		...
		async handleUpload(files, t) {
			...
			try {
				...
				const list = ocrRes.result.ocrResult || [];

				/** 日期去重 */

				list.forEach(el => {
					const arriveDateList = this.vehicleInfoList.map(el2 => el2.arriveDate);
					if (!arriveDateList.includes(el.arriveDate)) {
						const arr = el.arriveDate.split('-');
						...
						const item = this.vehicleInfoList.find(el2 => el2.arriveDate == el.arriveDate);
						/** 去重 */
						const currentPlatList = el.plateNumberList.map(el2 => {
							return {
								plateNumber: el2
							};
						});
			...
			this.vehicleInfoList.forEach(el => {
				el.plateNumberList.forEach((el2, i) => {
					if (!reg.test(el2.plateNumber)) {
						tip = `${el.arriveDate}下面的第${i + 1}列车牌号不正确`;
						flag2 = true;
					}
				});
			});
		...
		confirmDate(e) {
			...
			// 判断当前日期是否已经存在
			const str = `${arr[1]}月${arr[2]}日`;
			const flag = this.vehicleInfoList.some(el => el.arriveDate.includes(this.dateTime));
			if (flag) {
				this.$message.error('当前日期已经存在');
				return;
			}

			if (this.vehicleInfoList[this.currentWeightIndex]) {
				this.vehicleInfoList[this.currentWeightIndex].arriveDate = this.dateTime;
				this.vehicleInfoList[this.currentWeightIndex].showDate = str;
			}
