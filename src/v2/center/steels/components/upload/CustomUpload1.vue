				...
				>
					...
					<a-row class="row">
						<a-col :span="24">
							...
							>
								<!-- :action="(uploadType == 'receive' || uploadType == 'deliver') ?  : actionMark" -->
								<a-upload
									:headers="headers"
...
<script>
import { API_GETCURRENTENV, API_DeleteAttach, API_GetDownloadRAR } from '@/v2/api';
...
export default {
	...
	methods: {
		...
		//                 t.previewImage = API_GETCURRENTENV(file.response.fileUrl)
		//             }else{
		...
		beforeUpload(file) {
			const allowFormat = ['png', 'jpeg', 'jpg', 'gif', 'pdf', 'doc', 'docx', 'xlsx', 'xls', 'rar', 'zip'];
			let isAllowFormat = false;
			let ext = file.name.split('.')[file.name.split('.').length - 1].toLowerCase();
			if (allowFormat.indexOf(ext) == -1) {
				this.$message.error('仅支持jpg，jpeg，png，gif，pdf，docx，doc，xlsx，xls，rar，zip的格式');
				isAllowFormat = false;
			} else {
				isAllowFormat = true;
			}
