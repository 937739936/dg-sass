	...
	<div class="otherForm">
		...
		<a-form :form="form">
			...
			>
				...
				<!-- 仓库 -->
				<!-- <span slot="customTitles7"><i style="display:inline-block;color:red;margin-right:10px;position:relative;top:2px">*</i>仓库</span>
                <template slot="entrepot" slot-scope="text,record">
                    <a-form-item >
                        <a-input placeholder="请输入仓库" :maxLength='100' v-model.trim="record.entrepot" :disabled="!editable"> </a-input>
                    </a-form-item>
                </template> -->
				<!-- 预设含税单价（元/吨）-->
	...
	methods: {
		...
		save() {
			const flag2 = this.selected.every(el => {
				//  不指定规格
				if (this.isSpecial) {
					el.mainId = `${el.materialName}_${el.materialTexture}_${el.placeOfOrigin}`;
				} else {
