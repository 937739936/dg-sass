		...
		<a-form :form="form">
			...
			>
				...
				>
					<a-form-item>
						<a-select
							...
							:disabled="!editable"
							@change="getTheoreticalWeight(record)"
				...
				<span slot="customTitles10"
					><i style="display: inline-block; color: red; margin-right: 10px; position: relative; top: 2px">*</i>预提件数</span
				>
				<template
				...
				<span slot="customTitles11">
					<span>
						<i style="display: inline-block; color: red; margin-right: 10px; position: relative; top: 2px">*</i>预提捆包号</span
					>
				</span>
				<template
					slot="baleNo"
				...
				>
					<a-form-item>
						<a-input
							...
							:disabled="!editable"
							@blur="itemChange('currentApplyQuantity', record)"
							:key="record.id"
						></a-input>
					</a-form-item>
				</template>
...
import { templateDownload } from '@/v2/center/steels/api/orderApply';
import { getOriginPlace, getMaterialCategory, getTakeBillMaterialCategory, importPurchase } from '@/v2/center/steels/api';
const previewColumns = [
	...
	// 件数
	{ key: 'currentApplyPieceQuantity', dataIndex: 'currentApplyPieceQuantity', title: '件数' },
	// 捆包号
	...
	// 数量
	{ key: 'currentApplyQuantity', dataIndex: 'currentApplyQuantity', title: '数量（吨）', with: 200 },
	// 仓库
	// 含税单价
...
export default {
	...
	computed: {
		...
		columns() {
			...
			const columns = [
				...
				// 数量
				{ key: 'currentApplyQuantity', slots: { title: 'customTitles5' }, scopedSlots: { customRender: 'currentApplyQuantity' } },
				// 计量方式
				...
				{ title: '备注', dataIndex: 'remark', slots: { title: 'remark' }, scopedSlots: { customRender: 'remark' }, width: 150 },
	...
	watch: {
		...
		materialNameArr: {
			handler(v) {
				...
				v.forEach(el => {
					const newArr = (el.materialName && el.materialName.split(',')) || [];
		...
		contractDetails: {
			handler(v) {
				...
				this.getMaterialCategory(v.contractId);
			},
			deep: true,
			immediate: true
		}
	...
	methods: {
		...
		async getOriginPlace(name, item) {
			...
			if (this.steelType == 'FINISHED_STEEL' && specialSteelTypeArr.includes(item.materialName) && this.appointSpec == '1') {
				item.theoreticalWeight = '';
			} else {
				item.theoreticalWeight = '/';
			}
		...
		changePlaceOfOrigin(item) {
			...
			if (info.theoreticalWeight) {
				...
				if (this.steelType == 'FINISHED_STEEL' && specialSteelTypeArr.includes(item.materialName)) {
					// 判断件数  判断是否指定规格
					if (item.currentApplyPieceQuantity && item.currentApplyPieceQuantity !== '/' && this.appointSpec == '1') {
						item.currentApplyQuantity = (item.theoreticalWeight * item.currentApplyPieceQuantity).toFixed(3);
					}
				}
			} else {
				item.theoreticalWeight = '/';
		...
		itemChange(name, record) {
			...
			// 判断件数
			if (name == 'currentApplyPieceQuantity') {
				if (!record.currentApplyPieceQuantity) {
				...
				let reg = /^[0-9]*[1-9][0-9]*$/;
				if (record.currentApplyPieceQuantity !== '/') {
					if (!reg.test(record.currentApplyPieceQuantity)) {
						record.currentApplyPieceQuantity = '';
						this.$message.error('件数请输入/或者数字');
						return;
					}
					if (record.currentApplyPieceQuantity < 1 || record.currentApplyPieceQuantity > 99999999999999) {
						record.currentApplyPieceQuantity = '';
						this.$message.error('件数请输入/或者大于等于1的小于99999999999999正整数');
						return;
					}
		...
		async getMaterialCategory(id) {
			...
			const res = await getTakeBillMaterialCategory(id, params);
			const materialNameList = res.data || [];
		...
		handleOk() {
			...
			this.previewList.forEach(el => {
				...
				if (el.baleNo !== '/' && el.currentApplyPieceQuantity > 1) {
		...
		getTheoreticalWeight(item) {
			const newInfo = this.materialNameArr.find(el => el.materialName.includes(item.materialName)) || {};
