	...
	<div>
					...
					<a-form-item label="提单有效期">
						<a-range-picker
							...
							v-decorator="[
								'validity',
								{
									rules: [{ type: 'array', required: true, message: '交提货期限必填' }]
								}
							]"
						/>
					</a-form-item>
		...
		<div>
			...
			<a-button
				...
				block
				@click="showModal"
				v-if="['upload', 'add'].includes(typeModel)"
			>
...
export default {
	...
	data() {
		return {
			...
			takeDelivery: {},
			showPreviewBtn: true,
			acceptFormat: '.jpg, .jpeg, .png, .gif, .pdf, .docx, .doc, .xlsx, .xls',
