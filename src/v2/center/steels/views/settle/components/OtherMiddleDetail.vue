	...
	<div class="settle-apply-form">
		<template>
			...
			<div class="table-wrap">
				<a-table
					:rowSelection="{ selectedRowKeys: transferIds, onChange: onSelectGoodsTransferChange, getCheckboxProps }"
...
export default {
	...
	computed: {
		hasStatementId() {
			return this.$route.query.statementId ? true : false;
		},
		jsColumns() {
			return appJsColumns;
		}
	...
	methods: {
		...
		async getDetail() {
			...
			this.goodsTransferData = res.data.transferReleaseList;
		...
		async getEditDetail() {
			...
			res.data.transferReleaseList.map(i => {
		...
		onSelectGoodsTransferChange(selectedRowKeys, selectedRow) {
			...
			API_SteelsStatementDetailParticular({
				transferReleaseId: this.transferIds,
		...
		getCheckboxProps() {
			const flag = this.$route.query.statementId ? true : false;
			return {
				props: {
					disabled: flag
				}
		...
		handleChangeQuantity(value, index, key) {
