	...
	<div class="otherForm">
		<a-form :form="form">
			...
			>
				...
				>
					<a-form-item
						:validate-status="getFieldStatus('settlementAmount', record, record.errorShow['settlementAmount']).validStatus"
					>
						<a-input
							:key="record.id"
							placeholder="请输入结算单价（元/吨)"
							v-model.trim="record.settlementAmount"
							:disabled="!editable"
							@blur="itemChange('settlementAmount', record)"
						></a-input>
					</a-form-item>
				</template>
			</a-table>
	...
	created() {
		...
		if (this.steelType === 'SCRAP_STEEL') {
			this.columns = [
				...
				{ title: '品种', dataIndex: 'materialName', scopedSlots: { customRender: 'materialName' } },
		...
		this.columns = [
			...this.columns,
			{
				...
				key: 'deliveryAddress',
				scopedSlots: { customRender: 'deliveryAddress' }
			},
			{ key: 'receiveQuantity', slots: { title: 'customTitles1' }, scopedSlots: { customRender: 'receiveQuantity' } },
			{ key: 'settlementAmount', slots: { title: 'customTitles2' }, scopedSlots: { customRender: 'settlementAmount' } }
		];
	},
	methods: {
		updateData(data) {
			...
			this.selected.forEach(item => {
				(item.receiveQuantity = item.receiveQuantity || ''), (item.settlementAmount = item.settlementAmount || '');
				item.errorShow = {
					materialName: false,
					specs: false,
					materialTexture: false,
		...
		itemChange(name, record) {
			...
			if (name === 'settlementAmount') {
				if (record[name]) {
					...
					if (!regs2.test(record[name]) || Number(record[name]) > 10000000) {
						this.getFieldStatus(name, record, record.errorShow[name]);
						this.$message.error('不超过1000万，最多两位小数');
						record.errorShow[name] = true;
					} else {
						record.errorShow[name] = false;
					}
