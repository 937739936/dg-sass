<template>
	<div class="add-box">
		...
		<a-card
			...
			:bordered="false"
		>
			<div class="invoice-title">发票新增-发票上传</div>
			<!-- 上传方式 -->
			<div class="upload-box">
				...
				>
					...
					>
						<img
							src="@/v2/assets/imgs/invoicetools/four_icon.png"
							alt=""
			...
			<div
				class="no-data-box"
				v-if="!specialTaskList.length && !isFour"
			>
				<img
					src="@/v2/assets/imgs/invoicetools/empty-img-simple.png"
					alt=""
					style="width: 110px"
				/>
				<p style="color: rgba(0, 0, 0, 0.25); margin-top: 5px; padding-bottom: 100px">暂无数据</p>
			</div>
			<FourElements
				v-if="isFour"
				ref="fourElements"
			...
			>
				...
				<div class="excel-box">
					<div class="excel-box-content">
						...
						<div class="excel-box-upload">
							...
							<a-upload
								...
								@change="handleChange"
							>
								<a-button style="color: #4682f3; border: 1px solid #4682f3"> 选择文件</a-button>
							</a-upload>
					...
					<div class="tpl">
						...
						<a
							class="download-tpl"
							:href="publicPath + 'files/invoice/发票上传模板.xlsx'"
						>
...
import DelModal from '../components/TipModal.vue';

import moment from 'moment';
import Breadcrumb from '../components/Breadcrumb.vue';
import FourElements from '../components/FourElements.vue';
import { nextFourFactor } from '../api';
...
export default {
	...
	methods: {
		...
		next(item) {
			...
			const path = item.uploadType == 'PICTURE' ? '/invoice/discern/pictureInvoice' : '/invoice/discern/excelInvoice';
			// const path =  '/invoice/discern/excelInvoice'
		...
		// 四要素下一步
		async next2() {
			const values = await this.$refs.fourElements.save();
			if (!values) {
				return;
			}
			const params = {
			...
			const res = await nextFourFactor(params);
			this.$router.push({
				path: '/invoice/discern/fourInvoice',
				query: {
					taskId: res.data
				}
			});
			...
			try {
				...
				res.data.records.forEach(el => {
					if (el.currentStatus == 'SPLIT' || el.currentStatus == 'OCR') {
						...
						el.uploadProgress2 = el.uploadProgress;
					}
