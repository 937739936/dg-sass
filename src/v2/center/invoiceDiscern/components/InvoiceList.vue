		...
		>
			...
			>
				<div class="column-flex">
					<p>￥{{ items.amount | formatMoney(2) }}</p>
				</div>
			</template>
			<template
				slot="action"
...
import { getInvoiceToolsList, exportInvoiceTool, delInvoice, invoiceStatistics } from '@/v2/center/invoiceDiscern/api';
...
export default {
	...
	methods: {
			...
			exportInvoiceTool({
				...
				.then(res => {
					comDownload(
						res,
						undefined,
						'发票-' + this.VUEX_ST_COMPANYSUER.companyName + '-' + moment().format('yyyy-MM-DD') + '.xls'
					);
				})
				.finally(() => {});
		},
		...
		async invoiceStatistics() {
			...
			const res = await invoiceStatistics(params);
