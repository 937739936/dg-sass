<template>
	<div>
		<div class="modalContent">
			...
			<div>
				<p>
					<span class="c8">发票代码：</span><span>{{ info.invoiceVO.code }}</span>
				</p>
				<p>
					<span class="c8">发票号码：</span><span>{{ info.invoiceVO.no }}</span>
				</p>
				<p>
					<span class="c8">开票日期：</span><span>{{ info.invoiceVO.issuedDate }}</span>
				</p>
				<p>
					<span class="c8">校验码：</span><span>{{ info.invoiceVO.checkCode }}</span>
				</p>
				<p>
					<span class="c8">机器编号：</span><span>{{ info.invoiceVO.machineCode }}</span>
				</p>
			</div>
			<table
				cellspacing="0"
			...
			>
				...
				<tr
					v-for="(item, index) in invoiceItemVOList"
					:key="index"
				>
					<td colspan="5">
				...
				<tr>
					...
					<td
						...
						class="c8"
					>
						<span class="c8">（小写）</span
						><span
							class="blue"
							style="display: inline-block; margin-left: 20px"
							>¥{{ info.invoiceVO.amountTax }}</span
						>
					</td>
				</tr>
				<tr style="height: 180px">
					...
					>
						...
						<p style="margin-top: 20px">
							<span class="c8"><span>纳税人识别号</span></span
							>{{ info.invoiceVO.sellerUscc }}
						</p>
...
export default {
	...
	computed: {
		invoiceItemVOList() {
			const list = this.info.invoiceItemVOList || [];
			const obj = {
				name: '详见销货清单',
				quantity: 0,
				amount: 0,
			...
			list.forEach(el => {
				...
				obj.tax += el.tax ? Number(el.tax) : 0;
				obj.taxRate = el.taxRate;
			});
			obj.amount = obj.amount.toFixed(2);
			obj.quantity = obj.quantity.toFixed(2);
			obj.tax = obj.tax.toFixed(2);
