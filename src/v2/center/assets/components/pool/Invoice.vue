<template>
	<div class="contentBox">
		...
		<Attachment
			...
			v-if="invoiceInfo && invoiceInfo.upInvoiceDetailList"
		></Attachment>
		<a-modal
			:visible="visible"
			:footer="null"
...
export default {
	name: 'Invoice',
	props: ['invoiceInfo', 'editFlag', 'selectFinancedInvoice'],
	components: {
		ChooseInvoice,
		Attachment
	},
	...
	methods: {
		...
		dealEditData() {
			...
			if (!this.list.length && this.invoiceInfo.list) {
				this.list = this.invoiceInfo.list;
			}
			let list = this.list;
			if (list && list[0]) {
		...
		addInvo(type) {
			console.log(this.VUEX_POOL_ASSET_OBJ);
