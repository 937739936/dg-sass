								...
								>
									...
									>
										<a-date-picker
											...
											:disabled="elecContract || poolContractEdit || (contractInfo && contractInfo.whetherEditAllBoo === false)"
							...
							<template>
								...
								>
									<template
										...
										v-if="contractInfo && contractInfo.whetherEditAllBoo !== false"
									>
										<a
									...
									>
										<a-form-item label="回款账号">
											<a-select
												...
												:disabled="poolContractEdit || (contractInfo && contractInfo.whetherEditAllBoo === false)"
												v-decorator="[
													'acctBankId',
													{
		...
		>
			...
			<a-row>
				<a-table
					...
					:pagination="true"
					:scroll="{ x: 1000 }"
					:rowKey="record => record.id + record.parentOrderNo"
...
import { API_COMPANYACCOUNTLIST, API_GetPoolOrderList, API_GetPoolOrderDetail } from '@/v2/center/assets/api/index.js';
	...
	computed: {
		...
		rowSelection() {
			...
			return {
				...
				onSelect: record => {
					t.selectedRowKeys = [record.id + record.parentOrderNo];
