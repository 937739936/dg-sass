<template>
	<div class="contentBox">
		...
		>
			...
			<div
				class="partBox"
				v-if="editFlag && this.assetsConfig.TRANSFER_SHIP_CREDENTIALS"
			>
...
export default {
	...
	watch: {
		goodTransferInfo: function () {
			this.checkAssetsConfig(this.goodTransferInfo);
			if (this.editFlag && this.assetsConfig.TRANSFER_SHIP_CREDENTIALS) {
				// 编辑情况下，给表格数据初始化
	...
	methods: {
		...
		checkAssetsConfig(data) {
			...
			this.assetsConfig.TRANSFER_SHIP_CREDENTIALS =
				pici && (_.find(list, { item: 'TRANSFER_SHIP_CREDENTIALS' }) || {}).status == 1;
			this.$forceUpdate();
		},
		checkAssetsSubmit(goodTransferInfo) {
			var fileList = goodTransferInfo.list;
			let list = goodTransferInfo.bankProductAssetConfigList || [];
			var obj = {};

			if (this.assetsConfig.TRANSFER_SHIP_CREDENTIALS) {
				var o = _.find(list, { item: 'TRANSFER_SHIP_CREDENTIALS' });
				if (o.required == 1) {
					if (!fileList.some(i => i.type == 'GOOD_TRANSFER_CERTIFICATION' && i.delFlag == 0)) {
						obj.errorStr = '货转模块-缺少:' + o.itemDesc;
					}
				}
			}
		...
		onSubmit() {
			var checkRes = this.checkAssetsSubmit(this.goodTransferInfo);
