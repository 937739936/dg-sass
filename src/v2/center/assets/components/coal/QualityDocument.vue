		...
		>
			...
			<template v-if="editFlag">
				...
				<div
					class="partBox"
					v-if="
						assetsConfig.RECEIVE_AUTOMOBILE_POUND ||
						assetsConfig.RECEIVE_AUTOMOBILE_POUND_DETAIL ||
						assetsConfig.RECEIVE_AUTOMOBILE_TEST ||
						assetsConfig.RECEIVE_AUTOMOBILE_OTHER
					"
				>
					<div class="littleTitle">
						...
						<div class="typeBox">
							<a-button
								v-if="assetsConfig.RECEIVE_AUTOMOBILE_POUND"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_AUTOMOBILE_POUND_DETAIL || receivalVO.paymentType == 'product-shanxi-bank'"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_AUTOMOBILE_TEST"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_AUTOMOBILE_OTHER"
								type="primary"
					...
					<a-table
						...
						:data-source="recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.AUTOMOBILE)"
						:scroll="{ x: true }"
						rowKey="index"
				...
				<div
					class="partBox"
					v-if="assetsConfig.RECEIVE_TRAIN_WEIGHT || assetsConfig.RECEIVE_TRAIN_TEST || assetsConfig.RECEIVE_TRAIN_OTHER"
				>
					<div class="littleTitle">
						<p>单据类型</p>
						<div class="typeBox">
							<a-button
								v-if="assetsConfig.RECEIVE_TRAIN_WEIGHT"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_TRAIN_TEST"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_TRAIN_OTHER"
								type="primary"
					...
					<a-table
						:pagination="false"
						:columns="deliverColumns"
						:data-source="recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.TRAIN)"
						:scroll="{ x: true }"
						rowKey="index"
				...
				<div
					class="partBox"
					v-if="assetsConfig.RECEIVE_SHIP_WEIGHT || assetsConfig.RECEIVE_SHIP_TEST || assetsConfig.RECEIVE_SHIP_OTHER"
				>
					<div class="littleTitle">
						<p>单据类型</p>
						<div class="typeBox">
							<a-button
								v-if="assetsConfig.RECEIVE_SHIP_WEIGHT"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_SHIP_TEST"
								type="primary"
							...
							>
							<a-button
								v-if="assetsConfig.RECEIVE_SHIP_OTHER"
								type="primary"
					...
					<template v-if="recvInfo.recvList">
						...
						<a-table
							:pagination="false"
							:columns="deliverColumns"
							:data-source="recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.SHIP)"
							:scroll="{ x: true }"
							rowKey="index"
							:row-selection="rowSelectionTwo"
						></a-table>
...
export default {
	...
	data() {
		return {
			...
			filesColumns: [
				...
				{ title: '关联批次号', dataIndex: 'batchNo', key: 'batchNo' }
			], // 附件详情表头
			...
			editFilesColumns: [
				...
				{ title: '转换文件名', dataIndex: 'transferName', key: 'transferName' },
				{ title: '收货编号', dataIndex: 'receiveNo', key: 'receiveNo' },
				{ title: '关联发货批次号', dataIndex: 'batchNo', key: 'batchNo' },
	...
	mounted() {
		...
		this.checkAssetsConfig(this.recvInfo);
	},
	computed: {
		rowSelectionOne() {
			const t = this;
	...
	methods: {
		...
		checkAssetsConfig(data) {
			...
			let autoPici = data.recvList.filter(item => item.transferType == this.CONSTANTS.despatchModeDict.AUTOMOBILE).length > 0;

			this.assetsConfig.RECEIVE_AUTOMOBILE_POUND =
				autoPici && (_.find(list, { item: 'RECEIVE_AUTOMOBILE_POUND' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_AUTOMOBILE_OTHER =
				autoPici && (_.find(list, { item: 'RECEIVE_AUTOMOBILE_OTHER' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_AUTOMOBILE_TEST =
				autoPici && (_.find(list, { item: 'RECEIVE_AUTOMOBILE_TEST' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_AUTOMOBILE_POUND_DETAIL =
				autoPici && (_.find(list, { item: 'RECEIVE_AUTOMOBILE_POUND_DETAIL' }) || {}).status == 1;

			let trainPici = data.recvList.filter(item => item.transferType == this.CONSTANTS.despatchModeDict.TRAIN).length > 0;

			this.assetsConfig.RECEIVE_TRAIN_WEIGHT = trainPici && (_.find(list, { item: 'RECEIVE_TRAIN_WEIGHT' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_TRAIN_TEST = trainPici && (_.find(list, { item: 'RECEIVE_TRAIN_TEST' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_TRAIN_OTHER = trainPici && (_.find(list, { item: 'RECEIVE_TRAIN_OTHER' }) || {}).status == 1;

			let shipPici = data.recvList.filter(item => item.transferType == this.CONSTANTS.despatchModeDict.SHIP).length > 0;

			this.assetsConfig.RECEIVE_SHIP_WEIGHT = shipPici && (_.find(list, { item: 'RECEIVE_SHIP_WEIGHT' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_SHIP_TEST = shipPici && (_.find(list, { item: 'RECEIVE_SHIP_TEST' }) || {}).status == 1;
			this.assetsConfig.RECEIVE_SHIP_OTHER = shipPici && (_.find(list, { item: 'RECEIVE_SHIP_OTHER' }) || {}).status == 1;
			this.$forceUpdate();
		},
		checkAssetsSubmit(recvInfo) {
			var fileList = recvInfo.attachList;
			let list = recvInfo.bankProductAssetConfigList || [];
			var obj = {};
			...
			for (var key in this.assetsConfig) {
				...
				if (this.assetsConfig[key] && o.required == 1) {
					if (!fileList.some(i => i.type == o.assetAttachType && i.delFlag == 0)) {
						obj.errorStr = '数质量凭证模块-缺少:' + o.itemDesc;
						break;
					}
				}
			}
		...
		onSubmit() {
			var checkRes = this.checkAssetsSubmit(this.recvInfo);
			if (checkRes && checkRes.errorStr) {
				return checkRes;
			}
			return this.recvInfo || { attachList: [] };
		}
	}
};
</script>
