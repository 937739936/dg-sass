...
<script>
import { API_GetInvoiceResult } from '@/v2/center/assets/api/index.js';
import ChooseInvoice from './ChooseInvoice.vue';
import { Empty } from 'ant-design-vue';
import { mapGetters } from 'vuex';
export default {
	name: 'Invoice',
	props: ['invoiceInfo', 'editFlag'],
	components: {
		ChooseInvoice
	},
	watch: {
		...
		list: {
			immediate: true,
			handler(v) {
				if (this.newInvoiceInfo) {
					this.newInvoiceInfo.list = v;
				} else {
					this.newInvoiceInfo = { list: v };
				}
				this.dealEditData();
	...
	methods: {
		...
		dealEditData() {
			if (!this.newInvoiceInfo) return;

			this.list = this.newInvoiceInfo.list;
			let list = this.list;

			if (list && list[0]) {
				this.icount = list.length;
				this.itotal = list.reduce((pre, cur) => pre + (cur.totalAmount || 0), 0).toFixed(2);
				this.isplit = list.reduce((pre, cur) => pre + (cur.currentContractSplitedAmount || cur.splitAmount || 0), 0).toFixed(2);
			}
		},
		...
		addInvo(type) {
			...
			this.$refs.ChooseInvoice.show(this.VUEX_POOL_ASSET_OBJ.contractId, this.VUEX_POOL_ASSET_OBJ.contractType, this.keys);
		},
		chooseFinInvo(data) {
			this.list = data;
			if (this.list.length) {
