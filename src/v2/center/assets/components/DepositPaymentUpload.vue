...
import { API_CYGetPaymentAccountVoByPaymentId } from '@/v2/center/trade/api/pay';
...
export default {
	...
	methods: {
		...
		getBankInfo() {
			return API_CYGetPaymentAccountVoByPaymentId({
				paymentId: this.orderDetail.paymentId
			}).then(res => {
				if (res.success) {
					this.info = res.data;
		...
		async open() {
			return new Promise(async resolve => {
				...
				this.$nextTick(() => {
					this.form.setFieldsValue({
						paymentCompanyName: this.info.paymentCompanyName || this.orderDetail.buyerName,
						collectionCompanyName: this.info.collectionCompanyName || this.orderDetail.sellerName
					});
		...
		async handleUpload(file) {
			...
			try {
				...
				obj.url = obj.fileUrl;
				(obj.attachId = ''), (obj.name = file.name);
