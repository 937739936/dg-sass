		...
		>
			...
			<template v-if="editFlag">
				...
				<div
					...
					v-if="
						recvInfo.recvList &&
						recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.AUTOMOBILE).length > 0
					"
				>
					<div class="littleTitle">
						...
						<template v-if="receivalVO && receivalVO.industryType == 'STEEL'">
							<div class="typeBox">
								<a-button
									type="primary"
									:ghost="fileTypeOne != 'UP_RECEIVE_VOUCHER'"
									@click="
										() => {
											fileTypeOne = 'UP_RECEIVE_VOUCHER';
										}
									"
									>上游收货凭证</a-button
								>
					...
					<a-table
						...
						:data-source="
							recvInfo.recvList && recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.AUTOMOBILE)
						"
						:scroll="{ x: true }"
						rowKey="index"
				...
				<div
					...
					v-if="
						recvInfo.recvList &&
						recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.TRAIN).length > 0
					"
				>
					<div class="littleTitle">
						<p>单据类型</p>
					...
					<a-table
						...
						:columns="deliverColumns"
						:data-source="
							recvInfo.recvList && recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.TRAIN)
						"
						:scroll="{ x: true }"
						rowKey="index"
