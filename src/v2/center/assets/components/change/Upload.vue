...
export default {
	...
	methods: {
		...
		handlePreview(file) {
			// 添加与修改时返回格式不同
			if (file.response) {
				this.$refs.imageViewer.showFile(file.response.data);
			}
			if (file.url) {
		...
		checkFormatType() {
			if (this.mode == 'quality1' || this.mode == 'quality2') {
				// 数质量凭证模块修改文件类型
				this.myFormat = qualityFormat;
				if (this.type == 'RECEIVE_WEIGHT_NOTES_DETAIL') {
					// 磅单明细
					this.myFormat = qualityWEIGHTFormat;
				}
			} else if (this.mode === 'excelHsb') {
				this.myFormat = ['xls', 'xlsx'];
			} else {
		...
		addFileType() {
			...
			} else if ((this.mode == 'quality1' || this.mode == 'quality2') && this.selectedRows.length == 0) {
				// 数质量凭证模块下附件上传时要选批次
