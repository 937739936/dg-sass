	...
	<div class="contentBox">
		...
		>
						...
						<a-form-item label="结算单号">
							<a-input
								...
								v-decorator="[
									`serialNo`,
									{
										rules: [{ required: true, message: `结算单号必填`, whitespace: true }]
									}
								]"
							>
							</a-input>
						...
						<a-form-item label="结算日期">
							<a-date-picker
								...
								v-decorator="[
									`statementTime`,
									{
										rules: [{ required: true, message: `结算日期必填` }]
									}
								]"
							>
							</a-date-picker>
			...
			<Upload
				...
				type="MANUAL_STATEMENT"
				btnText="上传结算单"
			></Upload>
			<a-table
				:pagination="false"
				:columns="editFilesColumns"
				:data-source="settleFileList"
				:scroll="{ x: true }"
				rowKey="path"
			>
				<template
