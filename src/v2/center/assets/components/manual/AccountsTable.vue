		...
		<div class="content">
			...
			<template v-if="editFlag || editFile">
				...
				<Upload
					...
					type="MANUAL_PAYMENT_ACCOUNTING_TABLE"
					:uploadText="isPool ? '核算表截图' : '核算表'"
					:btnText="isPool ? '上传核算表截图' : '上传核算表'"
					:allowFormat="isPool ? ['png', 'jpeg', 'jpg', 'gif', 'pdf'] : null"
				></Upload>
				<Upload
					v-if="isPool"
					@uploadFiles="getUploadFiles"
				...
				<a
					v-if="isPool"
					:href="publicPath + 'files/核算表模板-平安银行池保理.xls'"
...
export default {
	...
	props: ['editFlag', 'accountInfo', 'editFile', 'isPool'],
	components: {
		Upload
	},
	watch: {
