	...
	<div class="contentBox">
		...
		>
			...
			<template v-if="editFlag">
				...
				<div
					class="partBox"
					v-if="recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.AUTOMOBILE).length > 0"
				>
					<div class="littleTitle">
						...
						<div class="typeBox">
							...
							<a-button
								type="primary"
								:ghost="fileTypeOne != 'DOWN_RECEIVE_VOUCHER'"
								@click="
									() => {
										fileTypeOne = 'DOWN_RECEIVE_VOUCHER';
									}
								"
								>上传下游收货凭证</a-button
							>
				...
				<div
					class="partBox"
					v-if="recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.TRAIN).length > 0"
				>
					<div class="littleTitle">
						<p>单据类型</p>
						<div class="typeBox">
				...
				<div
					...
					v-if="
						contractInfo &&
						contractInfo.upContract &&
						contractInfo.upContract.coalType == 'STEAM_COAL' &&
						recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.SHIP).length > 0
					"
				>
					<div class="littleTitle">
						<p>单据类型</p>
				...
				<div
					class="partBox"
					v-if="
						!(contractInfo && contractInfo.upContract && contractInfo.upContract.coalType == 'STEAM_COAL') &&
						recvInfo.recvList.filter(item => item.transferType == CONSTANTS.despatchModeDict.SHIP).length > 0
					"
				>
					<div class="littleTitle">
						...
						<div class="typeBox">
							...
							<a-button
								v-if="!isChange"
								type="primary"
								:ghost="fileType != 'RECEIVE_HARBOR_TRANSFER_VOUCHER'"
								@click="
									() => {
										fileType = 'RECEIVE_HARBOR_TRANSFER_VOUCHER';
									}
								"
								>港口货转凭证</a-button
							>
...
export default {
	...
	data() {
		return {
			...
			fileTypeOne: 'DOWN_RECEIVE_VOUCHER', // 汽运默认附件类型为磅单
