			...
			<div>
				...
				<template v-if="editFlag">
					...
					<a-table
						...
						:data-source="invoiceInfo.upInvoiceDetailList && invoiceInfo.upInvoiceDetailList.filter(item => item.delFlag == 0)"
						:scroll="{ x: true }"
						rowKey="index"
					>
						<template
				...
				<template v-if="!editFlag">
					<a-table
						...
						:data-source="invoiceInfo.upInvoiceDetailList.filter(item => item.delFlag == 0)"
						:scroll="{ x: true }"
						rowKey="index"
					>
						<template
...
export default {
	...
	methods: {
		...
		getResult(items) {
			...
			API_GetInvoiceResult({ invoiceId: items.id, industryType: 'STEEL' }).then(res => {
				if (res.success) {
		...
		getUploadFiles(data) {
			// 上传文件 获取附件数据
			this.invoiceInfo.upInvoiceDetailList = this.invoiceInfo.upInvoiceDetailList || [];
			data.forEach(item => {
