					...
					<div class="s-card-content">
						...
						>
							...
							<a-col span="8">
								...
								>
									<a-col span="9">资产MD5</a-col>
									<a-col span="15">{{ item.receivalVO.md5Hex }}</a-col>
								</a-row>
							</a-col>
						</a-row>
					</div>
							...
							>
								<div class="tabs-bar">
									...
									<div
										...
										@click="tabChange(1)"
										v-if="item.receivalVO.paymentType != 'receivable-shanmei-down'"
									>
										<div class="tabs-icon">
											<img
									...
									<div
										...
										@click="tabChange(3)"
										v-if="['receivable-shanmei-down', 'product-huaneng-downReceivable'].includes(item.receivalVO.paymentType)"
									>
										<div class="tabs-icon">
											<img
							...
							<a-col span="20">
								...
								<div v-show="activeIndex == 0">
									<template v-if="item.receivalVO.assetType === 'ACCOUNTS_RECEIVABLE_DOWN_MANUAL'">
										<ContractManual
											v-if="item.receivalVO.poolReuse != 1"
											:editFlag="false"
								...
								<template v-if="item.receivalVO.assetType === 'ACCOUNTS_RECEIVABLE_DOWN_MANUAL'">
									...
									<SettleInfo
										:editFlag="false"
										:contractInfo="item.contractInfo"
										:settlesInfo="item.manualStatement"
										:receivalVO="item.receivalVO"
										:isJr="true"
										v-if="activeIndex == 8"
										ref="settle"
									...
									<TransferInfo
										ref="newTransfer"
										:manualTransfer="item.manualTransfer"
										:editFlag="false"
										:isJr="true"
										v-if="activeIndex == 3 && ['receivable-shanmei-down'].includes(item.receivalVO.paymentType)"
									></TransferInfo>
									<InvoiceInfo
										:isJr="true"
										v-if="activeIndex == 6"
										:paymentType="item.receivalVO.paymentType"
...
<script>
import { API_AuditReceivableJRDownload } from '@/v2/center/assets/api/index.js';
...
// import  from "@/v2/center/assets/components/manual/Invoice.vue"
import SettleInfo from '@/v2/center/assets/views/receivable/components/SettleInfo.vue';
import InvoiceInfo from '@/v2/center/assets/views/receivable/components/InvoiceInfo.vue';
	...
	methods: {
		...
		getDetail() {
			if (this.defaultDetailData?.length) {
				this.currentStatus = this.defaultDetailData[0].receivalVO || {};
				// 默认第一个是实时时间
				if (this.defaultDetailData.length == 1) {
					this.detailData = [this.defaultDetailData[0]];
				} else if (this.defaultDetailData.length > 1) {
					this.detailData = this.defaultDetailData.slice(1);
					this.showSTime = true;
				} else {
					this.detailData = [];
				}
