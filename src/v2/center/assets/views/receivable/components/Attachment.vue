<template>
	<div class="attachment">
		...
		<div class="tips">
			<div style="color: #000">
				<p>可支持格式为xls，xlsx的附件，单个附件大小不得超过100M的文件。</p>
			</div>
		</div>
		<a-table
			class="new-table"
		...
		>
			...
			>
				<div class="name-box">
					<template v-for="(item, index) in record.fileList">
						<div
							v-if="item.delFlag == 0"
							class="name"
							:key="index"
...
import { commonUpload, API_UPLOAD_WATER_MARk, API_GETCURRENTENV } from 'api';
...
export default {
	...
	data() {
		return {
			isSpread: false,
			columns,
			dataSource: [{ type: 'UP_INVOICE_DETAIL', id: 'UP_INVOICE_DETAIL', fileList: [], typeName: '发票明细表' }],
			delVisible: false,
			currentIndex: 0,
			currentItem: {},
			previewImg: '',
	...
	methods: {
		init(info) {
			// 线下合同
			const contractAttachment = info.upInvoiceDetailList || [];
			this.$nextTick(() => {
		...
		save() {
			...
			const list = this.dataSource[0].fileList;

			list.forEach(el => {
				el.type = 'UP_INVOICE_DETAIL';
				el.delFlag = el.delFlag || 0;
			});
