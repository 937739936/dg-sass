		...
		>
			...
			<template v-if="currentStatus.status">
				<a-row
					class="row"
					:gutter="40"
					style="margin-left: 0; margin-bottom: 20px; margin-top: 20px"
				>
...
<script>
import { API_GetAccountsManualDetail, API_AuditReceivableJRDownload } from '@/v2/center/assets/api/index.js';
...
export default {
	...
	methods: {
		...
		tabChange(index) {
			...
			this.$router.push('/center/assets/receivable/manual/detailJR?id=' + this.$route.query.id + '&activeIndex=' + index);
		}
	}
};
</script>
