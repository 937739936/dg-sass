		...
		<a-card :bordered="false">
			...
			<div :class="'table-box ' + (pagination.total > 10 ? 'fixedBottom' : '')">
				...
				>
					...
					>
						<a
							v-auth="'asset:sellerConfirm:view'"
						...
						<!-- 状态为"待确权"时 -->
						<a
							v-auth="'asset:sellerConfirm:sign'"
							v-if="items.status == 'TO_BE_SIGN'"
							href="javascript:;"
...
import { API_GetConfirmAssetsSellList, API_JudgeConfirm } from '@/v2/center/assets/api/index.js';
...
export default {
	...
	data() {
		return {
			...
			url: {
				list: API_GetConfirmAssetsSellList
			},
			searchList,
			defaultParams: {
				status: 'TO_BE_SIGN'
			...
			API_JudgeConfirm({ assetId: id }).then(res => {
				if (res.success) {
					if (res.data) {
						...
						this.$router.push('/center/assets/confirmRights/sellstamp?id=' + id);
					}
				}
			});
		},
