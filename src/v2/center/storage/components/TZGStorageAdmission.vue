<!-- 我的仓储-泰州港-出入场记录 -->
<template>
	<div class="storage-admission-tzg">
		<a-table
			:rowKey="
				(record, index) => {
					return index;
...
import { API_getWarehouseHarborInListPage, API_getWarehouseHarborMyInExportXls } from '@/v2/center/storage/api';
import { filterCodeByValueName } from '@sub/utils/globalCode.js';
export default {
	name: 'StorageAdmissionTZG',
	data() {
		return {
			dataSource: [],
			columns: [
	...
	methods: {
		...
		getList(data) {
			...
			API_getWarehouseHarborInListPage(this.params).then(resp => {
				if (resp.success) {
					let obj = resp.result || {};
		...
		exportXls(data) {
			return {
				func: API_getWarehouseHarborMyInExportXls(data),
				name: '泰州港入场详情'
			};
		}
	}
