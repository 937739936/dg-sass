					...
					<a-row class="invoice-item">
						...
						<div class="invoice-desc">
							<div class="invoice-desc-left">
								...
								<p>
									<span>数量</span>
									<span class="invoice-attr"
										>{{ item.list.length && formateNumber(computedAmount(item.list, 'quantity'), 4) }}{{ unit(index) }}</span
									>
								</p>
								<p>
									<span>发票代码</span>
							...
							<div class="invoice-desc-right">
								<p>
									<span>关联合同数量</span>
									<span class="relation-count">{{ formateNumber(qualityAndAmount[`quality_${index}`], 4) || 0 }}</span>
									<span class="grid-font-color">{{ unit(index) }}</span>
								</p>
								<p>
									<span>关联至合同金额价税合计</span>
						...
						<div class="table-wrap">
							<a-table
								...
								class="new-table"
							>
								<span slot="splitQuantity">关联至合同数量({{ unit(index) }})</span>
								<span
									slot="contractNo"
								...
								>
									<a-form-item>
										<a-input-number
											...
											v-decorator="[
												'splitQuantity_' + index + '_' + record.key,
												{
													rules: [
...
<script>
import { columnsInvoiceRelationContract } from './table';
...
export default {
	...
	methods: {
		...
		amountAndQuality(index) {
			this.$nextTick(() => {
				...
				for (const key in formValues) {
					if (key.indexOf(`splitQuantity_${index}`) >= 0 && formValues[key]) {
						result[`quality_${index}`] += Number(formValues[key]);
					}
			...
			for (let i = 0; i < cloneList.length; i++) {
				for (let j = 0; j < cloneList[i].invoiceContractSplitList.length; j++) {
					if (cloneList[i].invoiceContractSplitList[j]) {
						...
						cloneList[i].invoiceContractSplitList[j].splitQuantity =
							values[`splitQuantity_${i}_${cloneList[i].invoiceContractSplitList[j]['key']}`];
					}
				}
			}
			return cloneList;
					...
					API_INVOICE_SAVE({
						...
						if (res.success) {
							if (res.data.success) {
								...
								this.$router.replace('/center/admin/invoice/in/list');
							} else {
		...
		batchRelation() {
			...
			this.$refs.invoiceContractSplitList.showModal(list.filter(item => item.show));
		},
		relationContractAdd(index) {
			...
			this.$nextTick(() => {
				...
				const parentElem = document.querySelector('#invoice-list');
		...
		getChangeItemFormContractList(list, no) {
			return list.filter(item => item.no === no)[0];
		},
		getInvoiceList(list) {
