	...
	<div class="wrap">
		...
		<div class="info-desc marin-top30">
			...
			<div class="info-wrap">
				...
				<p>开户银行/账号：{{ invoiceData.purchaserBankAndNumber }}</p>
				<p>联系方式：{{ invoiceData.purchaserAddressAndPhone }}</p>
			</div>
		</div>
		<div class="info-desc marin-top30">
			...
			<div class="info-wrap">
				...
				<p>开户银行/账号：{{ invoiceData.salerBankAndNumber }}</p>
				<p>联系方式：{{ invoiceData.salerAddressAndPhone }}</p>
			</div>
		</div>
		<div class="info-desc marin-top30">
			...
			<div class="info-table">
				...
				></a-table>
				<p class="count">
					<span>数量：{{ formateNumber(quantityCount('invoiceItemList', 'quantity'), 4) }}{{ unit }}</span>
					<span>价税合计：￥{{ formateNumber(invoiceData.totalAmount) }}</span>
				</p>
			</div>
		</div>
		<div class="info-desc marin-top30">
			...
			<div class="info-table">
				...
				<p class="count">
					<span>已关联合同数量：{{ formateNumber(quantityCount('invoiceContractRelList', 'splitQuantity'), 4) }}{{ unit }}</span>
					<span>已关联合同金额：￥{{ formateNumber(quantityCount('invoiceContractRelList', 'splitAmount'), 2) }}元</span>
				</p>
			</div>
		</div>
		<div
...
export default {
	...
	methods: {
		formateNumber,
		quantityCount(item, tag) {
			if (this.invoiceData[item]) {
				return this.invoiceData[item].reduce((pre, cur) => {
					return pre + cur[tag];
				}, 0);
			}
		},
