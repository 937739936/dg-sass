<template>
  <div>
    <div class="method-btn-wrap">
      <div class="invoice-add-button" type="primary" icon="plus" @click="add" v-auth="'kitInvoice:deliverInvoice:list:add'"> 运费发票 </div>
      <div class="invoice-export-button" icon="export" :loading="loadingExport" @click="exportList" v-auth="'kitInvoice:deliverInvoice:list:export'">导出数据</div>
    </div>
    <div class="search-form search-form-no-margin-left">
      ...
      <FormMoreAndCheckboxItem class="trans-list-input" ref="sellerNameListStr" v-on:change="handleItemChange" label="销售方" title="sellerNameListStr" :list="sellerList" placeholder="请输入销售方"/>
      <FormMoreAndCheckboxItem class="trans-list-input" ref="commissionItemNameListStr" v-on:change="handleItemChange" label="商品名称" title="commissionItemNameListStr" :list="goodsList" placeholder="请输入商品名称"/>
      <div class="filter-more-list" v-if="showMoreFilter">
        <FormNoInputItem ref="no" v-on:change="handleItemChange" label="发票号码" title="no" placeholder="请输入发票号码"/>
    ...
    <a-table
      ...
      :scroll="{x: 1200}"
    >
      <span slot="hasAttachment" slot-scope="text, scoped">
        {{scoped.hasAttachment ? '有附件' : '无附件'}}
      </span>
      <span slot="action" slot-scope="text, scoped">
        <a-space>
          <a @click="detail(scoped)" v-if="scoped.state == 'NORMAL'" v-auth="'kitInvoice:deliverInvoice:list:detail'">查看</a>
          <a @click="edit(scoped)" v-if="scoped.state == 'NORMAL'" v-auth="'kitInvoice:deliverInvoice:list:edit'">编辑</a>
          <a @click="deleteItem(scoped)" v-auth="'kitInvoice:deliverInvoice:list:delete'">删除</a>
          <a-dropdown v-if="scoped.state == 'NORMAL'">
            <a class="ant-dropdown-link">
              更多<a-icon type="down" />
            </a>
            <a-menu slot="overlay">
              <a-menu-item v-auth="'kitInvoice:deliverInvoice:list:redDashed'">
                <a @click="red(scoped)">红冲</a>
              </a-menu-item>
              <a-menu-item v-auth="'kitInvoice:deliverInvoice:list:invalid'">
    ...
    <EditInvoice ref="editInvoice" title="运费发票编辑" :columns="columnsInvoiceRelationContract" v-on:editOk="fetchData" type="2"/>
  </div>
</template>
  ...
  methods: {
    ...
    exportList() {
      ...
      API_GET_INVOICE_EXPORT({
        ...
        comDownload(res, undefined, "运费发票列表" + ".xls");
      }).finally(() => {
    ...
    add() {
      this.$router.push({
        path: "/center/admin/invoice/transport/add",
        query: {
          type: 2,
        },
      });
    ...
    formatePostData() {
      return {
        ...
        invoiceItemNameListStr:
          this.formResult.commissionItemNameListStr.join(","),
        ...
        hasAttachment: this.formResult.hasAttachment.length ? (this.formResult.hasAttachment.join(",") == '有附件' ? true : false) : "",
      };
    },
    fetchData() {
      this.loading = true;
