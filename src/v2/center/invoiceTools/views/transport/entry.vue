<template>
  <div>
    <p class="method-btn-wrap">
      <a-space>
        <div class="invoice-export-button" @click="exportList" :loading="loadingExport" v-auth="'kitInvoice:deliverInvoice:accountBook:export'">导出数据</div>
      </a-space>
    </p>
    <div class="search-form search-form-no-margin-left">
      <p class="search-form-tag-list">
        ...
        <ul class="selected-tag">
          ...
          >
            ...
            <a-tag closable @close="handleTagClose('createDate')" v-if="key === 'createDate' && formResult['createDate'].length && formResult['createDate'][0]">登记日期:{{formResult.createDate[0]}}~{{formResult.createDate[1]}}</a-tag>          </li>
        </ul>
      ...
      <FormMoreAndCheckboxItem class="trans-input-entry" ref="sellerNameListStr" v-on:change="handleItemChange" label="开票单位" title="sellerNameListStr" :list="sellerList" placeholder="请输入开票单位"/>
      ...
      <div class="filter-more-list" v-if="showMoreFilter">
        ...
        <FormSelectMonthItem class="trans-input-entry" ref="recordDateListStr" v-on:change="handleItemChange" label="入账月份" title="recordDateListStr" :list="monthList"/>
    ...
    <p>
      <a-button type="primary" :disabled="selectedRowKeys.length == 0" @click="entryList" v-auth="'kitInvoice:deliverInvoice:accountBook:batchAdd'">批量入账</a-button>
    </p>
    <a-table
    ...
    >
      <span slot="action" slot-scope="text, scoped">
        <a-space>
          <a @click="detail(scoped)" v-if="scoped.id" v-auth="'kitInvoice:deliverInvoice:accountBook:detail'">查看</a>
          <a @click="entry(scoped)" v-if="scoped.id" v-auth="'kitInvoice:deliverInvoice:accountBook:edit'">编辑</a>
          <a @click="entry(scoped)" v-if="!scoped.id" v-auth="'kitInvoice:deliverInvoice:accountBook:add'">入账</a>
          <a @click="cancelEntry(scoped)" v-if="scoped.id" v-auth="'kitInvoice:deliverInvoice:accountBook:delete'">取消入账</a>
        </a-space>
      </span>
    </a-table>
    ...
    <InvoiceEntry ref="invoiceEntry" :invoiceType="2" :columns="columnsEntryInvoiceEditList" v-on:entry="entrySuccess" />
  </div>
</template>

<script>
import moment from "moment";
import { columnsEntryTable, columnsEntryInvoiceEditList } from "./table.js";
  ...
  methods: {
    ...
    exportList() {
      ...
      API_GET_INVOICE_ENTRY_EXPORT({
        ...
        .then((res) => {
          comDownload(res, undefined, "运费发票入账" + ".xls");
        })
        .finally(() => {
    ...
    entry(scoped, type = "add") {
      this.$refs.invoiceEntry.showModal(scoped, type, 2);
    },
    ...
    detail(scoped) {
      this.$router.push({
        path: "/center/admin/invoice/transport/entry/detail",
        query: {
          id: scoped.id,
        },
      });
