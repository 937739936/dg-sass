		...
		>
			...
			<a-row>
				<div class="slTitleAssis">
					<span class="editor-required-icon">* </span>
					特殊约定
					<a-space :size="30">
						<a-button
							class="slBtn"
							type="primary"
							ghost
							@click.native="collectTemplate(50)"
						>
							收藏
						</a-button>
						<a-button
							...
							type="primary"
							ghost
							@click.native="selectTemplate(50)"
						>
							选择已有模板
						</a-button>
						<span class="tips">注：本条款为《保理信息表》中特殊约定内容，填写时请注意条款中甲乙方对应关系。</span>
					</a-space>
				</div>
				<a-form-item
					label=""
				...
				>
					<Editor
						placeholder="特殊约定"
						ref="specialAgreement"
						:maxLength="2000"
						v-decorator="[
							'specialAgreement',
							{
								rules: [
									{ required: true, message: `特殊约定必填`, whitespace: true },
									{ max: 2000, message: `特殊约定最长2000字符` }
								],
								validateTrigger: 'blur'
							}
						]"
...
import Editor from '@/v2/center/trade/views/contract/diy/components/Editor.vue';
import Template from '@/v2/center/trade/views/contract/diy/components/Template.vue';
...
export default {
	...
	methods: {
		// 收藏模版
		collectTemplate(type) {
			// type是模版数，editorContent是富文本编辑器的内容
			this.$refs.template.collect(type, this.financingApplyForm.getFieldValue('specialAgreement'));
		},
		// 选择模版
		selectTemplate(type) {
		...
		showContent(data) {
			this.$refs[`specialAgreement`]?.updateHtmlContent(data);
		},
