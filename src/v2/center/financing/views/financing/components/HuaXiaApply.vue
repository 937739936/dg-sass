						...
						<a-form-item label="融资还款账号">
							<a-select
								...
								v-decorator="[
									'acctAccountId',
									{
										rules: [{ required: true, message: `融资还款账号必填` }],
										validateTrigger: 'change'
									}
								]"
...
export default {
	...
	methods: {
		getBankAccount(companyUscc, type) {
			API_COMPANYACCOUNTLIST({ uscc: companyUscc, d: Date.now() }).then(res => {
				let data = res.data || [];
				let _array = [];
				data.forEach(item => {
					_array.push({
		...
		setFormData() {
			...
			this.getBankAccount(this.VUEX_ST_COMPANYSUER.companyUscc, 'h');
			this.setFangkuan();
		},
		// 根据accountNo设置放款账号
		setFangkuan() {
