...
export default {
	...
	mounted() {
		...
		this.getDetail();
		API_FinancingSignList({ financingApplyId: this.financingApplyId }).then(res => {
	...
	methods: {
		...
		autoSignature() {
			...
			API_CfcaFinAutoSignature({ financingApplyId: this.financingApplyId })
				.then(res => {
					if (res.success) {
						this.$message.success('签署完成').then(() => this.$router.push('/center/financing/financingList'));
		...
		step1(obj) {
			return API_FinancingGetSigList({
				financingApplyId: this.financingApplyId,
				cert: obj.cert
			});
		},
		step2() {
			return API_FinancingSignSave({
				financingApplyId: this.financingApplyId
			});
		},
		signApply() {
