	...
	<div>
		...
		>
			...
			<li>
				...
				<span>{{ data.bankProductName }}</span>
			</li>
			<li v-if="VUEX_ST_COMPANYSUER.companyType !== 'CORE_COMPANY'">
				<span class="label">合作企业</span>
				<TextOverFlow
					v-if="data.coreCompanyName"
					:content="data.coreCompanyName"
					:maxWidth="maxWidth"
				/>
				<span v-else>-</span>
			</li>
			...
			>
				...
				<span>{{ data.creditTypeDesc }}</span>
			</li>
			<li>
				<span class="label">授信额度（元）</span>
				<span>{{ displayAmountText(data.totalAmount) }}</span>
			</li>
			<li>
				<span class="label">剩余额度（元）</span>
				<span>{{ displayAmountText(data.availableAmount) }}</span>
			</li>
			<li>
				<span class="label">已用额度（元）</span>
				<span>{{ displayAmountText(data.usedAmount) }}</span>
			</li>
			<li>
				<span class="label">冻结额度（元）</span>
				<span>{{ displayAmountText(data.frozenAmount) }}</span>
			</li>
			<li>
