<template>
	<div class="LoanDetail">
		...
		<!-- <a-tab-pane key="1" tab="放还款详情" style="flex-direction: column;"> -->
		...
		<!-- </a-tab-pane> -->
		<!-- <a-tab-pane key="2" tab="还款申请记录" style="flex-direction: column;" v-if="checkLoanLog()">
                <div class="rz-content">

                    <a-row>
                        <a-button type="primary" @click="pushAndSyncLoan" style="margin-bottom:20px;"><div>同步</div></a-button>
                    </a-row>
                    <a-row>
                        <a-table
                            rowKey="id"
                            ...
                            :locale="{emptyText: '暂无数据'}"
                        >
                            <div slot="serialNo" slot-scope="text, record">
                                <a  href="javascript:;" @click="$router.push('loanApplyDetail?id=' + record.id + '')">{{text}}</a>
                            </div>
