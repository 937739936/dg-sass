...
import { API_GetLoanDetail, API_LoanApplySaveNS } from '@/v2/center/financing/api/index.js';
		...
		sumbitApply() {
			this.applyForm.validateFields(error => {
				...
				this.$confirm({
					...
					cancelText: '取消',
					onOk: () => {
						let amount = this.applyForm.getFieldValue('amount');
						if (amount > this.fangkuanData.unPayPrincipal) {
							this.$message.error('还款本金不能大于未还款金额');
							return;
						}

						API_LoanApplySaveNS({ loanId: this.loanId, amount: amount }).then(res => {
							if (res.data) {
			...
			API_GetLoanDetail({ loanId: this.loanId }).then(res => {
				if (res.success) {
					...
					this.applyForm.setFieldsValue({
						amount: this.fangkuanData.unPayPrincipal
					});
				}
			});
		}
