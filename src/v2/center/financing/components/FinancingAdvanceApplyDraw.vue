...
const searchListOn = [
	...
	// {
	//   decorator: ["serialNo"],
	//   addonBeforeTitle: "预付账款流水号",
	//   type: "input",
	//   placeholder: "请输入预付账款流水号",
	// },
];

const relationOrderListColumns = [
	{ title: '预付账款流水号', dataIndex: 'serialNo', fixed: 'left' },
	{ title: '卖方名称', dataIndex: 'sellerName' },
	{ title: '合同编号', dataIndex: 'contractNo' },
	{ title: '预付账款金额（元）', dataIndex: 'amount', customRender: t => formatMoney(t) },
	{ title: '付款类型', dataIndex: 'payTypeDesc' },

	{ title: '金融机构', dataIndex: 'bankName' },
	{ title: '拟融资金额（元）', dataIndex: 'planFinancingAmount', customRender: t => formatMoney(t) },

	{ title: '预付账款申请日期', dataIndex: 'requestTime' },
	{ title: '资产状态', dataIndex: 'statusText' }
];
...
export default {
	...
	methods: {
		...
		async getRelationOrderList(data) {
			let API = API_GetAdvancePayableList;
		...
		handleSubmit() {
			this.$store.commit('financing/updateAdvance', this.currentRecord);
			this.orderRelationModal = false;

			if (this.type == 'list') {
				this.$router.push('/center/financing/financingAdvanceApply');
			}
		}
	}
};
