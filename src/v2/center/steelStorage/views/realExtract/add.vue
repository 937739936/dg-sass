	...
	<div class="slMain">
		...
		>
			...
			<a-form
				...
				:style="[isAgain ? 'padding-top:48px' : '']"
			>
				<a-row>
					<a-col :span="8">
					...
					<a-col :span="8">
						<a-row
							...
							justify="center"
						>
							<a-form-item label="实际库存数量">
								<a-input
									placeholder="请输入实际库存数量"
									disabled
									v-decorator="[`actualQuantity`]"
								/>
							</a-form-item>
						</a-row>
					</a-col>
					<a-col :span="8">
						<a-row
							type="flex"
							:justify="isAgain ? 'end' : 'end'"
						>
							<a-form-item label="实际库存重量(吨)">
								<a-input
									placeholder="请输入数量"
									disabled
									v-decorator="[`actualWeight`]"
								/>
							</a-form-item>
						</a-row>
					</a-col>
					<a-col :span="8">
						<a-row
							type="flex"
							:justify="isAgain ? 'start' : 'start'"
						>
							<a-form-item label="实提总数量">
								<a-input
									placeholder="请输入实提总数量"
									:maxLength="40"
					...
					<a-col :span="8">
						<a-row
							type="flex"
							:justify="isAgain ? 'center' : 'center'"
						>
							<a-form-item label="实提总重量(吨)">
								<a-input-number
									:min="0.0001"
									placeholder="请输入实提总重量"
									:max="999999999.9999"
									:precision="4"
									v-decorator="[`weight`, { rules: [{ required: true, message: `请输入实提总重量` }] }]"
								/>
							</a-form-item>
						</a-row>
					</a-col>
				...
				<a-row type="flex">
					<a-form-item label="实提类型">
						<a-select
							:getPopupContainer="getPopupContainer"
							placeholder="请选择实提类型"
							:disabled="form.getFieldValue('cleanWarehouse') == 1"
							v-decorator="[
								`status`,
								{
									initialValue: form.getFieldValue('cleanWarehouse') == 1 ? 'ALL_EXTRACT' : undefined,
									rules: [{ required: true, message: `请选择实提类型` }]
								}
							]"
						>
							<a-select-option value="PART_EXTRACT"> 部分实提</a-select-option>
							<a-select-option value="ALL_EXTRACT"> 全部实提</a-select-option>
						</a-select>
					</a-form-item>
					<div
						class="tip"
...
export default {
	...
	computed: {
		...
		isAgain() {
			return !!this.$route.query.realId;
		}
	},
	mounted() {
		/** 再次实提 */
		if (this.$route.query.realId) {
	...
	methods: {
		...
		async getOutboundInventory() {
			...
			const obj = {
				actualQuantity: res.data.actualQuantity < 0 ? 0 : res.data.actualQuantity,
				actualWeight: res.data.actualWeight < 0 ? 0 : res.data.actualWeight
			};
			this.$nextTick(() => {
		...
		async getActualExtractDetail() {
			const params = {
				id: this.$route.query.realId || this.$route.query.id
			};
			...
			this.form.validateFields(async (err, values) => {
				if (!err) {
					...
					const params = {
						...
						outboundIdList: this.$route.query.idList && this.$route.query.idList.split(','),
						...values,
					...
					if (this.$route.query.id || this.$route.query.realId) {
						params.id = this.$route.query.id || this.$route.query.realId;
					}
					...
					try {
						...
						this.$router.push('/center/steelStorage/realExtract/list');
					} catch (error) {
						this.disabled = false;
					}
				}
