	...
	>
		<a-card :bordered="false">
			<div style="display: flex; justify-content: space-between">
				<span class="slTitle">入库记录查询</span>
				<div
					class="btn"
					@click="goAdd"
				>
			...
			>
				...
				>
					<div class="action">
						...
						<!-- 已入库 -->
						<template v-if="record.status == 'ENTERED'">
							<a-button
								type="link"
						...
						<!-- 已作废 -->
						<template v-if="record.status == 'INVALID'">
							<a-button
								type="link"
								@click="goDetail(record)"
								>查看</a-button
			...
			<TipModal
				ref="tipModal"
				tip="您确定作废当前入库记录么？作废后将会影响系统中的库存统计数据。"
...
import { getAllWarehouseList, invalidWarehouse, getInStorageList, deleteWarehouse } from '../../api';
...
const searchList = [
	...
	},
	{
		...
		type: 'select',
		placeholder: '请选择',
		options: filterSteelsCodeByKey('warehouseInStatus')
	}
];
...
export default {
	...
	methods: {
		...
		goEdit(item) {
			this.$router.push({
				path: '/center/steelStorage/inStorage/add',
				query: {
					id: item.id
				}
			});
