			...
			<div class="slMain mapCon">
				...
				>
					<a-form-item :label="`${typeName}名称`">
						...
						<a-auto-complete
							:placeholder="`请输入${typeName}名称`"
							:data-source="portNameList"
							:maxLength="200"
							:filterOption="filterOption"
							v-decorator="['name', formRules.name]"
							@blur="handlePortNameBlur"
						>
							</a-auto-complete>
					</a-form-item>
...
export default {
	...
	computed: {
		...
		portNameHistoryList() {
			...
			let { type } = this.circle;
			if (type == 'origin') {
				list = [...this.originPortNameList];
			} else if (type == 'destination') {
				list = [...this.destinationPortNameList];
			}
			return list;
		}
	},
	...
	methods: {
		...
		handlePortNameBlur(value) {
			let list = [...this.portNameHistoryList]
			if (!value) {
				this.portNameList = [...list];
				return;
			}
			if (!this.portNameList.includes(value) ) {
