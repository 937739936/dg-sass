<template>
	<div id="shipInfo">
		...
		>
						...
						>
							<a-space>
								<a-input-number
									...
									@blur="handleSubmit"
									@change="e => handleDeliverQuantityChange(e, record, index)"
								/>
							</a-space>
						</a-form-model-item>
						<span v-else>
					...
					>
						...
						>
							...
							<a-auto-complete
								style="width: 200px"
								placeholder="船舶始发港口名称"
								:data-source="originPortNameList"
								:maxLength="200"
					...
					>
						...
						>
							<a-date-picker
								...
								:disabled="!Boolean(record.originPortName)"
			...
			<CheckPort
				ref="checkPort"
				:originPortNameList="originPortNameList"
				:destinationPortNameList="destinationPortNameList"
...
export default {
	...
	watch: {
		...
		portNameHistoryInfo: {
			...
			immediate: true,
			handler(val) {
					this.originPortNameHistory = val.originPort || [];
					this.destinationPortNameHistory = val.destinationPort || [];
			}
		},
	},
	computed: {
		// 始发港列表
		originPortNameList: function () {
			let list = [...this.originPortNameHistory];
			this.form.tableDataSource.forEach(item => {
				if (item.originPortName && !list.includes(item.originPortName)) {
					list.push(item.originPortName);
				}
			});
			return list;
		},
		// 目的港列表
		destinationPortNameList: function () {
			let list = [...this.destinationPortNameHistory];
			this.form.tableDataSource.forEach(item => {
				if (item.destinationPortName && !list.includes(item.destinationPortName)) {
					list.push(item.destinationPortName);
				}
			});
			return list;
		},
	...
	data() {
		return {
			...
			firstDeliverQuantity: undefined,
			originPortNameHistory: [],
			destinationPortNameHistory: [],
			getPopupContainer,
		};
	},
	mounted() {
	...
	methods: {
		...
		addShipInfo() {
			var deliverQuantity = undefined;
			if (this.form.tableDataSource.length == 0) {
				deliverQuantity = this.firstDeliverQuantity;
			}
			let obj = {
		...
		setFirstDeliverQuantity(val) {
			this.firstDeliverQuantity = val;
			if (this.form.tableDataSource.length == 0) {
				return;
			}
			this.form.tableDataSource[0].deliverQuantity = val;
		},
		handleDeliverQuantityChange(e, record, index) {
			if (index == 0) {
				this.$emit('changeFirstDeliverQuantity', e);
			}
		},
