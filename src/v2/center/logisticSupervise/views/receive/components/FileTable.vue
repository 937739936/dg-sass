<template>
	<div>
		<div>
			<div class="sub-title">
				<span style="margin-right: 30px">附件信息</span>
				<span
...
<script>
import { API_UPLOAD_FILE, API_GetDownloadRAR, API_DOWNLPREVIEWTE } from '@/v2/api/upload';
				...
				let fileData = fileList
					...
					.map(item => {
						...
						if (!item?.response?.success){
							this.$message.error(item?.response?.error?.message || '上传失败');
