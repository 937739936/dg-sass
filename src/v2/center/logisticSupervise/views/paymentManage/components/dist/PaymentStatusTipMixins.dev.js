...
var _paymentManage = require('@/v2/center/logisticSupervise/api/paymentManage');
...
var _default = {
	...
	computed: {
		// 需要提示的列表付款状态
		listNeedTipStatus: function listNeedTipStatus() {
			return [
		...
		// 需要提示的步骤付款状态
		stepNeedTipStatus: function stepNeedTipStatus() {
			var list = this.listNeedTipStatus;
			list.push('CANCEL'); // 资产作废

			return list;
		},
		// 需要提示的操作步骤
		stepNeedTipOperations: function stepNeedTipOperations() {
			return [
				'PLATFORM_AUDITING_REJECT', // 平台运营驳回
				'RISK_CONTROL_REJECT', // 平台风控驳回
				'OA_REJECT', // OA驳回
	...
	methods: {
		// 是否需要提示
		currentIsNeedTip: function currentIsNeedTip(query) {
			var businessOperation = query.businessOperation,
				paymentStatus = query.paymentStatus,
				isSteps = query.isSteps;
		...
		// 获取状态提示
		getPaymentStatusTip: function getPaymentStatusTip(visible, query) {
			var _this = this;
			...
			(0, _paymentManage.API_GetPaymentStatusTip)({
				paymentNo: paymentNo,
				...
				.then(function (res) {
					if (res.success) {
						var tipInfo = res.data;
						var tip = '';

						if (businessOperation) {
							tip = _this.formatOperationTip(tipInfo, businessOperation);
						} else {
							tip = _this.formatStatusTip(tipInfo, paymentStatus);
						}

						_this.statusTipInfo = _objectSpread({}, _this.statusTipInfo, {
							statusTip: tip
						});
					} else {
						_this.$set(_this.statusTipInfo, 'statusTip', '获取状态提示失败');
					}
				})
				['catch'](function () {
					_this.$set(_this.statusTipInfo, 'statusTip', '获取状态提示失败');
				...
				['finally'](function () {
					_this.$set(_this.statusTipInfo, 'isTipLoading', false);
				});
		},
		formatOperationTip: function formatOperationTip(tipInfo, operator) {
			var tip = '';
			var result = tipInfo.result || '';
			var reason = tipInfo.reason || '';
			var platformName = tipInfo.platformName || '';
			var operateTime = tipInfo.operateTime || '';
			var personName = tipInfo.personName || '';
			var assetSerialNo = tipInfo.assetSerialNo || '';
		...
		// 设置状态提示
		formatStatusTip: function formatStatusTip(tipInfo, status) {
			/**
      	{ 
      		result: 审核结果、确认结果
      ...
      */
			...
			var operateTime = tipInfo.operateTime || '';
			var personName = tipInfo.personName || '';
			var assetSerialNo = tipInfo.assetSerialNo || ''; // 平台运营驳回 或平台风控驳回
		...
		// 资产状态提示语
		receivableStatusTip: function receivableStatusTip(status, statusDes, assetStatusTipVO) {
			var statusTip = AssetStatusTooltip[status] || '';
			var data = assetStatusTipVO || {};
			statusTip = statusTip
		...
		finStatusTip: function finStatusTip(status, statusDes, finStatusTipVO) {
			var statusTip = FinStatusTooltip[status] || '';
			var data = finStatusTipVO || {};
			statusTip = statusTip
				...
				.replace('【作废原因】', data.cancelReason || ''); // let pre = true;
			// if (pre) {
			// 	// 预融资
			// 	statusTip = statusTip
