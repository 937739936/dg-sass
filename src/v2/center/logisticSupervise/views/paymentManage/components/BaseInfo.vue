						...
						>
							...
							<template v-for="(items, index) in paymentTypeList">
								<a-radio
									...
									:key="index"
									:value="items.value"
									v-if="['SETTLEMENT', 'PRE_SETTLEMENT'].includes(items.value)"
									>{{ items.text }}</a-radio
								>
							</template>
						</a-radio-group>
	...
	methods: {
		...
		async paymentTypeChange(e) {
			return new Promise(async resolve => {
				...
				this.$emit('getSettleInfo', this.paymentType === 'SETTLEMENT');
				this.$nextTick(() => {
					this.payApplyForm.setFieldsValue({
						payType: undefined
					});
		...
		async invoiceChange(e) {
			...
			if (this.hasInvoice === 'INVOICE') {
				...
				if (
					!(
						this.invoiceInfo &&
						((this.invoiceInfo.storeInvoiceRecordVOList && this.invoiceInfo.storeInvoiceRecordVOList.length) ||
					...
					this.$emit('getInvoiceInfo', true);
				}
			} else {
				this.$emit('getInvoiceInfo', false);
			}
		},
		async getBankInfo() {
