		...
		>
			...
			<div class="table-box">
				<a-form :form="form">
					...
					<a-table
						...
						:scroll="{ x: true }"
					>
						<span slot="sellerName">{{ $route.query.invoiceType == 'INPUT' ? '仓储方' : '承运人' }}</span>
						<span slot="buyerName">{{ $route.query.invoiceType == 'INPUT' ? '承租方' : '托运人' }}</span>
				...
				<div class="tip">
					<span>价税合计总额</span>
					<span class="money">￥{{ fillDecimal((+detailData.invoiceVO.totalAmount || 0.0).toLocaleString()) }}</span>
					<!-- <span style="margin-left:5px">已拆分金额</span> <span class="money">￥{{ fillDecimal(splitedAmount).toLocaleString() }}</span>  -->
					<span style="margin-left: 5px">剩余拆分金额</span>
					<span class="money">￥{{ fillDecimal(notSplitAmount).toLocaleString() }}</span>
					<p v-if="detailData.invoiceVO.stampTaxFlag == 2 && type == 'DELIVER'">
						<span style="margin-left: 5px">含印花税合计总额</span>
						<span class="money">￥{{ fillDecimal(detailData.invoiceVO.stampTaxFlagTotalAmount || 0.0).toLocaleString() }}</span>
					</p>
			...
			<div v-if="!isEdit">
				...
				>
					...
					/>
					{{ detailData.invoiceVO.attachmentName }}
				</div>
...
export default {
	...
	methods: {
		...
		async confirmSave() {
			...
			await saveEdit({ ...this.editParams, ...params });
		...
		async getDetail() {
			...
			this.$nextTick(() => {
				if (this.detailData.invoiceVO.attachment) {
					// 处理附件名称
					let front_url = this.detailData.invoiceVO.attachment.split('?')[0];
					const arr = front_url.split('/');
