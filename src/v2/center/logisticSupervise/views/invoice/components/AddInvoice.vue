	...
	methods: {
		...
		next(item) {
			...
			const path =
				this.industryType == 'STEEL'
					? '/center/steels/invoice/LinkContract'
					: `/center/logisticSupervise/invoice/${invoiceType}/link/contract`;
			this.$router.replace({
				path,
				query
			});
		...
		async handleUpload() {
			...
			if (this.invoiceType == 'INPUT') {
				formData.append('productCode', 'WAREHOUSE_RENT');
			}
			if (this.invoiceType == 'DELIVER') {
				formData.append('productCode', 'LOGIC_DELIVER');
			}
