    ...
    <div slot="closeIcon"><i  class="icon-danchuang-closeicon iconfont" style="fontSize:25px"></i></div>
    	<SlFormNew
         ...
         class="search-wrap"
				@change="changeSearch"
				@resetFunc="resetFunc"
        :class="$route.query.invoiceType"
			></SlFormNew>
    <!-- <a-form  layout="inline" :form="form" class="search-wrap">
        <a-form-item label="" class="order-wrap col" :colon="false">
          <SlInput :addonBeforeTitle='$route.query.invoiceType =="INPUT" ? "仓储方" : "承运人"' @blur="getList(1)" v-model="params.sellerName"  placeholder="请输入"></SlInput>
        </a-form-item>
        <a-form-item label=""  class="order-wrap col" :colon="false">
          <SlInput :addonBeforeTitle='$route.query.invoiceType =="INPUT" ? "承租方" : "托运人"' @blur="getList(1)" v-model="params.buyerName"  placeholder="请输入"></SlInput>
        </a-form-item>
        <a-form-item label=""  class="order-wrap col" :colon="false">
...
<script>
  ...
  const columns = [
    	{ title: '合同编号', key: 'contractNo', dataIndex: 'contractNo' },
      { title: '托运人', key: 'buyerName', dataIndex: 'buyerName' },
      { title: '承运人', key: 'sellerName', dataIndex: 'sellerName' },
      { title: '运输方式', dataIndex: 'transportMode', width: 100, customRender: t => {
        return t.replace('AUTOMOBILE', '汽运')
                .replace('TRAIN', '火运')
                 .replace('SHIP', '船运')
      } }
...
// 仓储合同
  const columns2 = [
    { title: '仓储合同编号', key: 'contractNo', dataIndex: 'contractNo' },
		{
      title: '仓库名称',
