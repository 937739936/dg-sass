	...
	<div class="slMain">
        ...
        <div class="btn-box-title">
          <div
            class="btn"
            @click="add"
						v-auth="'logicDeliverMonitor:contractManager:warehouse:add'"
          >
            <img
              class="icon"
              ...
              >
                <a-col :span="24">
                  <p>
                    ...
                    <TextOverFlow
											v-if="items.contractDynamicsFields"
                      :content="items.contractDynamicsFields.warehouseName || '-'"
                      :maxWidth="maxWidth"
                    />
                  </p>
                  <p>
...
import { getStorageContractAction } from '@/v2/center/logisticSupervise/views/contract/action';
...
export default {
	...
	methods: {
		...
		handleChange(data) {
			...
			if (data.warehouseId) {
				...
				delete this.searchParams.warehouseId
			}
			this.getNum();
		...
		actionDiv(items) {
			let t = this;
			let actionList = getStorageContractAction(
				items,
			...
			if (dropdownList.length) {
				actionVnode.push(
					h(
						...
						[
							h('a', [h('span', {}, '更多'), h('em', { attrs: { class: 'right-arrow' } })]),
    ...
    add() {
      this.$router.push('/center/logisticSupervise/contract/storage/add')
    },
		...
		eidtContract(items) {
			this.$router.push({
				path: '/center/logisticSupervise/contract/storage/add',
				query: {
					id: items.id
				}
			})
