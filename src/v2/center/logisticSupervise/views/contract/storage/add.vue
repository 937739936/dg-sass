<template>
	<div class="slMain">
		<Breadcrumb />
    <a-card
			:bordered="false"
			style="padding-bottom: 12px"
		>
      <div class="methods-wrap">
        <div
          slot="title"
          class="slTitle"
        >
          <span>{{ $route.query.id ? '编辑仓储合同' : '新增仓储合同' }}</span>
        </div>
      </div>
      <div
        class="slTitleAssis"
      >
        合同信息
      </div>
      <StorageContractInfo ref="contractInfo" @getStorageCompanyName="getStorageCompanyName"></StorageContractInfo>
      <div
        class="slTitleAssis"
      >
        签署信息
      </div>
      <SignInfo ref="signInfo"></SignInfo>
      <div
        class="slTitleAssis"
      >
        附件信息
      </div>
      <Attachment ref="attachment" :list='attachmentList'></Attachment>
      <div class="submit-btn">
        <a-space :size="30">
          <a-button
            type="primary"
            ghost
            @click="cancel"
            >取消</a-button
          >
          <a-button
            type="primary"
            @click="save(false)"
            :loading="loadingSave"
            >保存</a-button
          >
          <a-button
            type="primary"
            @click="save(true)"
            :loading="loadingSubmit"
            >提交</a-button
          >
        </a-space>
      </div>
    </a-card>
  </div>
</template> 
<script>
  import Breadcrumb from '@/v2/components/breadcrumb/index';
  import StorageContractInfo from '@/v2/center/logisticSupervise/views/contract/components/StorageContractInfo';
  import SignInfo from '@/v2/center/logisticSupervise/views/contract/components/SignInfo';
  import Attachment from '@/v2/center/logisticSupervise/views/contract/components/Attachment';
  import { 
    API_contractDetail,
    API_Save
  } from '@/v2/center/trade/api/transportContract';
  import { mapGetters } from 'vuex';
  export default {
    data() {
      return {
        transferFlag: false,
        attachmentList: [
          {
            key:"OFFLINE_CONTRACT",
            label: '仓储合同',
            accept: '.jpg,.png,.pdf,.jpeg,.JPEG,.PNG,.JPG,.PDF, .bmp',
            tip: '可支持格式为jpg，jpeg，png，bmp，pdf的附件，单个附件大小不得超过100M的文件。',
            required: true,
            isShowBtn: false,
          },
        ],
        loadingSave: false,
        loadingSubmit: false,
      }
    },
    components: {
      Breadcrumb,
      StorageContractInfo,
      SignInfo,
      Attachment
    },
    computed: {
      ...mapGetters('user', {
        VUEX_ST_COMPANYSUER: 'VUEX_ST_COMPANYSUER'
      }),
    },
    mounted() {
      if (this.$route.query.id) {
        this.getDetailsData()
      }else {
        if (this.$refs.contractInfo) {
          this.$refs.contractInfo.initFormData()
        }
      }
    },
    methods: {
      getStorageCompanyName(data) { // 获取仓储方名称并赋值
        if (data) {
          this.$refs.signInfo.setSellerName(data)
        }
      },
      getDetailsData() {
        API_contractDetail({
          id: this.$route.query.id
        }).then(res => {
          if (res.success) {
            this.detailsData = res.data;
            this.$nextTick(() => {
              this.$refs.contractInfo.initFormData(this.detailsData)
              this.$refs.signInfo.initFormData(this.detailsData)
              this.$refs.attachment.init(this.detailsData.contractAttachment)
            });
          }
        });
      },
      cancel() {
        this.$router.push('/center/logisticSupervise/contract/storage/list')
      },
      async save(type) {
        const contractInfo = await this.$refs.contractInfo.handleSubmit()
        if (!contractInfo) return
        const signInfo = await this.$refs.signInfo.handleSubmit()
        if (!signInfo) return
        const filesInfo = await this.$refs.attachment.save()
        if (!filesInfo) return
        const bodyObj = {
          id: this.$route.query.id,
          submit: type,
          productCode: 'WAREHOUSE_RENT',
          ...contractInfo,
          ...signInfo,
          attachmentReq: filesInfo,
          contractDynamicsFields: {
            warehouseId: contractInfo.warehouseId,
            payCompanyName: signInfo.payCompanyName,
            warehouseName: contractInfo.warehouseName
          }
        }
        console.log('bodyObj', bodyObj)
        API_Save(bodyObj).then(res=>{
          if (res.success && res.data) {
            this.$message.success(type?'提交成功':'保存成功')
            this.$router.push('/center/logisticSupervise/contract/storage/list')
          }
        })
      },
    }
  }
</script>  
<style lang="less" scoped>
  .slTitleAssis {
    margin: 30px 0 20px 0;
  }
  .submit-btn {
		position: sticky;
		bottom: 0;
		padding: 20px;
		background: #ffffff;
		text-align: center;
		.ant-btn {
			margin: 0 15px;
			padding: 0 30px;
			border-radius: 6px;
			border: 1px solid @primary-color;
		}
	}
</style>
   