...
export default {
  ...
  methods: {
    ...
    // 关联收货编号 关联发货编号
    goGoods(info, i) {
      const receiveIdList = info.receiveId.split(',')
      const receiveId = receiveIdList[i]
       let path = `/center/logisticSupervise/receive/send/detail?deliverId=${receiveId}`
      if( this.type == 'IN') {
        path = `/center/logisticSupervise/receive/send/detail?receiveId=${receiveId}`
      }
