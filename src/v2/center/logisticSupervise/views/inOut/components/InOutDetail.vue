		...
		>
			...
			<a-row class="supple-info">
				...
				>
					...
					<span class="omit" v-if="detailInfo.receivingCompanyName">
						<a-tooltip>
							<template slot="title">
				...
				<div class="titleInfoItem">
					<span class="label">{{ titleStr }}库类型：</span>
					<span class="c8">{{ detailInfo.storageTypeText }}</span>
				</div>
			</a-row>
		</a-card>
		<a-card
					...
					<div>
						...
						<div>
							<ul class="grid-wrap transportBox">
								...
								<li v-if="detailInfo.storageType !== 'LOSS_OUT'">
                ...
                <li v-if="detailInfo.actualReceivingUnit">
                  ...
                  <a-tooltip>
                    ...
                    <span>{{detailInfo.actualReceivingUnit || '-'}}</span>
								...
								<li :style="{width: detailInfo.storageType === 'LOSS_OUT' ? '100%' : '66.6%'}">
...
import InOutParticulars from '../../../../../../../submodules/src/logisticsPlatform/InOutParticulars.vue';
		...
		detailInfo(info) {
			...
			if (info.transportMode == 'AUTOMOBILE') {
				this.filesList = [
					...
					{ type: 'OTHER_PROFIT_LOSS_TICKET', title: '超亏吨凭证', require: false, list: [] },
