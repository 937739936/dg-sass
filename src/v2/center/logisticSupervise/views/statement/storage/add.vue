		...
		>
			...
			<ul class="grid-wrap">
				...
				<li>
					...
					<span>{{ contractData.sellerName }}</span>
				</li>
				<li>
				...
				<li style="width:66.6%">
					...
					<span>{{ contractData.contractDynamicsFields.warehouseName }}</span>
				</li>
			</ul>
			...
			<Attachment :list='attachmentList' ref="attachment" style="margin-top:20px"></Attachment>
...
export default {
	...
	methods: {
		...
		getContractInfo() {
			// 如果是从业务动态监控的补录结算单入口进来，则合同信息直接反显，禁止修改
			API_JumpFromMonotoring({ contractNo: this.$route.query.contractNo,productCode: this.defaultParams.productCode, }).then(res => {
				if (res.success) {
		...
		getDetail() {
			API_transport_settle_details({
				...
				this.$nextTick(() => {
					...
					this.$refs.attachment.init(res.data.terminalStatementVO.attachmentList)
				})
			});
		},
		//选择合同弹框
		...
		toSubmit(type) {
          ...
          if(!list) return
					this.settleForm.attachmentList = list;
			...
			API_transport_settle_save({
				...
				.then(res => {
					if (res.success) {
						...
						this.$router.push('/center/logisticSupervise/statement/storage');
					}
				})
				.finally(() => {
