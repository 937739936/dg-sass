		...
		<a-card :bordered="false">
			<div class="s-title-btn">
				...
				<a-button
					type="primary"
					@click="settleAdd"
					v-auth="'logicDeliverMonitor:statementManager:warehouse:add'"
							...
							<template>
								<a-space>
									...
									<a
										...
										v-auth="'logicDeliverMonitor:statementManager:warehouse:edit'"
										>编辑</a
									>
									...
									<a
										...
										v-auth="'logicDeliverMonitor:statementManager:warehouse:delete'"
										>删除</a
									>
								</a-space>
							</template>
...
import ContractList from '../components/ContractListStorage';
...
export default {
	...
	methods: {
		...
		// 获取仓库列表
		async getTransferWarehouseList() {
			const res = await getTransferWarehouseList();
			this.searchList[2].options = res.data.map(el => {
				return {
					...el,
					label: el.name,
					value: el.id
