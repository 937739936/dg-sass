		...
		<a-row>
          ...
          合同已完善--当前用户不是下游负责人，只可查看-->
				...
				<div class="tabs-bar">
					...
					>
						...
						<div class="tabs-text">
							...
							<span
								v-if="
									dynamicMonitoringDetail[['contractSignDate', 'downstreamContractSignDate'][contractType]] && item.value === 0
								"
							>
								签订日期：{{ dynamicMonitoringDetail[['contractSignDate', 'downstreamContractSignDate'][contractType]] }}
							</span>
							<span
								v-if="
			...
			<a-col span="18">
				...
				<template
					v-if="
						(contractType === 0 && curUpstream.upOrderNo) ||
						(contractType === 1 && dynamicMonitoringDetail.downOrderNo) ||
						contractType === 2
					"
				>
					...
					<template v-if="contractContentActiveIndex === 2">
						...
						<DownStreamSupplementCapitalFlow
							v-if="
								(businessLineType === 'OFFLINE' || businessLineType === 'DOWN') &&
								((contractType === 2 && $route.query.activedCoverIndex != 0) || contractType === 1)
							"
							:orderNo="orderNo"
							:contractNo="contractNo"
							:businessLineNo="dynamicMonitoringDetail.businessLineNo"
							:contractType="contractType"
							:dynamicMonitoringDetail="dynamicMonitoringDetail"
							:downOrderId="dynamicMonitoringDetail.downOrderId || downOrderId"
							:downOrderNo="dynamicMonitoringDetail.downOrderNo"
							:upOrderNo="(curUpstream && curUpstream.upOrderNo) || upOrderNo"
							@refresh="refreshDetail"
						/>
						<CapitalFlow
							v-else
