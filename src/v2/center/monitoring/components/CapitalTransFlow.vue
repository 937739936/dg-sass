<template>
	<div>
		...
		>
			...
			>
				<a
					v-if="!isFinanceCompanyAndOnlineContract && record.paymentAttachmentInfo && record.paymentAttachmentInfo.length > 0"
...
import { getTransPayList, getTransPayStatistics } from '../api/transportBusiness.js';
...
export default {
	...
	methods: {
		...
		getUpstreamPaymentStatistics() {
			...
			getTransPayStatistics(params).then(res => {
				if (res.success) {
