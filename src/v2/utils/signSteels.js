 ...
 * @Date: 2022-07-02 22:13:43
 * @LastEditors: liupengbo liupengbo@shdatalink.com
 * @LastEditTime: 2022-07-02 22:16:49
 * @FilePath: /dg-trade-frontend/src/untils/signSteels.js
...
export function sign(step1, step2, completedRoute, isOrderModule = false, companyUscc='') {
        ...
        if(window.FAKE_SIGN==='ON')
        {
            ...
            step2().then(res=>{
                this.$message.success('模拟盖章完成！').then(()=>{
                    ...
                    } else { // 终止合同盖章completedRoute为undefined
                        this.$router.go(-1)
                    }
                });
            });
            ...
            this.$message.info('当前为开发模式。可在浏览器console或控制台输入 window.FAKE_SIGN="ON" 开启模拟盖章。页面重新加载后自动关闭');
        }
    }
    const currentCompanyUscc = companyUscc;
    var that = this;
    var defer = $.Deferred();
    defer.resolve();
    defer = defer.then(function () {
        ...
        step1({
            ...
            $.each(toSigList, function (i, e) {
                defer = defer.then(function (data) {
