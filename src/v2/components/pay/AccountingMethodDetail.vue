<template>
  <div class="accounting-method-detail">
    <div class="accounting-wrapper">
      ...
      <div class="accounting-content" v-html="detail.contractPriceMethod"/>
    </div>
    <div class="method-wrapper">
      <div class="com-title">
        ...
        <div class="power-plant">
          <label>电厂名称：</label>
          <a-input v-model="detail.powerName" :disabled="true"/>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      let checkedList = this.selected.map(item => {return item.type})
      this.checkItemList.forEach(item => {
        this.$set(item, 'isChecked', checkedList.indexOf(item.type)>-1)
      })

      let checkedOtherList = this.selectedOther.map(item => {return item.type})
