<template>
  <div class="total-amount-detail">
    <!--加权批次化验列表---start-->
    <div class="table-wrapper" v-if="goodsValueDetailList1.length > 0">
      <table class="custom-table">
        ...
        <tr class="header-th">
          ...
          <th><div>数量(吨)</div></th>
          <th v-for="item in evaluationIndexList" :key="item.key+'_1'"><div>{{item.text}}</div></th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="(row, index) in goodsValueDetailList1" :key="'add_'+index">
          <td>{{row.receiveNo}}</td>
          <td>{{row.weight}}</td>
          <td
    ...
    <!--加权指标奖罚---start-->
    <div class="table-wrapper" v-if="goodsValueDetailList2.length > 0">
      <table class="custom-table">
        <thead>
        <tr class="font-weight">
        ...
        <tbody>
        <tr v-for="(row, index) in goodsValueDetailList2" :key="'even_'+index">
          <th>
          ...
          <td>
            ...
            <span v-if="index===1">{{row.averagePrice}}</span></td>
        </tr>
        </tbody>
      </table>
    </div>
    ...
    <!--单批次化验数据及奖罚---start-->
    <div class="table-wrapper" v-if="goodsValueDetailList3.length > 0">
      <table class="custom-table">
        <thead>
        <tr class="font-weight">
        ...
        <tbody v-for="(row, index) in goodsValueDetailList3" :key="'row_'+index">
        <tr
          ...
          <td v-if="_inner===0" rowspan="2">{{innerRow.receiveNo}}</td>
          <td>
    ...
    <div class="tip-wrapper">
      总货值 = 加权货值+单批次总货值+调整总金额+额外扣减 = {{amount.goodsValueWeight||0}}+{{amount.singleBatchGoodsValue||0}}{{Number(amount.adjustTotalAmount) > 0 ? '+' : ''}}{{amount.adjustTotalAmount||'+0'}}+{{amount.extraChange||0}}   元
    </div>
  </div>
</template>
<script>
  export default{
    ...
    methods:{
      initData(val){
        ...
        arr.forEach(item => {
          switch (item.type+'') {
            case '1': this.goodsValueDetailList1 = item.goodsValueDetailList || []; break
            case '2': this.goodsValueDetailList2 = item.goodsValueDetailList || []; break
            case '3':
            {
              ...
              arr1.forEach(item => {
                let obj = arr2.find(inner => {return inner.receiveNo === item.receiveNo}) || {}
