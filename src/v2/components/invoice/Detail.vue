<template>
  <div class="invoice-detail">
    ...
    <a-form-model class="invoice-info-form" ref="invoiceForm2" :labelCol="{ span: 8 }" :wrapperCol="{ span: 13 }" :colns="false">
      <a-row>
        <a-col :span="12">
          <a-form-model-item label="发票代码" prop="code">
            <a-input v-model="invoiceForm.code" disabled></a-input>
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item label="发票号码" prop="no">
            <a-input v-model="invoiceForm.no" disabled></a-input>
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item label="发票金额(不含税)" prop="taxExcludeAmount">
            <a-input v-model="invoiceForm.taxExcludedAmount" disabled></a-input>
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item label="开票日期" prop="issuedDate">
            <a-input v-model="invoiceForm.issuedDate" disabled></a-input>
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item label="发票校验码" prop="checkCode">
            <a-input v-model="invoiceForm.checkCode" disabled></a-input>
          </a-form-model-item>
        </a-col>
        <a-col :span="12">
          <a-form-model-item label="开票类型" prop="invoiceType">
            <a-input v-model="invoiceForm.invoiceTypeDesc" disabled></a-input>
          </a-form-model-item>
        </a-col>
        <!-- <a-col :span="12">
    ...
    <template v-if="this.invoiceData">
      ...
      <div class="other-info">
        ...
        <div class="upload-wrap">
          ...
          <a-button style="margin-top: 10px" type="primary" ghost v-if="fileList.length" @click="viewPic">查看</a-button>
        </div>
        ...
        <div class="link-order">
          <a-row type="flex" justify="space-between">
            ...
            <a-col v-if="isModify"
              ><a-button type="primary" @click="addLinkOrder"><a-icon type="plus"></a-icon> 新增关联订单</a-button></a-col
            >
          </a-row>
          <a-table id="link-order-table" :scroll="{ x: true }" style="margin-top: 16px" :columns="orderColumns" :dataSource="linkOrders" rowKey="orderId">
            <span slot="money" slot-scope="text, record">
              <a-input v-if="isModify" placeholder="请输入大于零的金额" :disabled="record.hasPayment" class="edit-input" v-model="record.splitAmount"></a-input>
              <p v-else>{{ (+text).toLocaleString() }}</p>
            </span>
            <span slot="action" slot-scope="text, record, index">
              <a-space>
                <a href="javascript:;" v-if="isModify && linkOrders.length > 1" @click="deleteLink(record, index)">删除关联</a>
                <a
                  href="javascript:;"
          ...
          <LinkOrder ref="linkOrder" @update="linkUpdate"></LinkOrder>
        </div>
      </div>
    </template>
    <div class="btn-wrap" v-if="!(hidePageFooter === true)">
      <a-button type="primary" ghost @click="submitInvoiceInfo" v-if="isModify">确定</a-button>
...
<script lang="jsx">
import { API_INVOICESAVE, API_UPLOAD, API_ORDER_HAS_PAYMENT } from "@/v2/api/common";
  ...
  methods: {
    toContractDetail(record) {
      let routerData = this.$router.resolve({
        path: '/center/contract/'+record.orderType.toLowerCase()+'/online/detail',
        query: {
          id:record.orderId,
