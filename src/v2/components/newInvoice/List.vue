  ...
  <div class="slMain">
    <a-card :bordered="false">
      ...
      <div :class="'table-box '+(pagination.total>10?'fixedBottom ':' ')">
        ...
        <span slot="invoiceStatusTitle">
          发票状态
          <a-tooltip>
            <template slot="title">
              <div style="white-space: pre-wrap">{{ invoiceStatusTip }}</div>
            </template>
            <i class="iconfont icon-liebiaobiaotou-shuoming" style="font-size: 12px"></i>
          </a-tooltip>
        </span>
          ...
          <div slot="action" slot-scope="text, record">
            <a-space>
              ...
              <!-- 判断是否是异常发票 -->
              <a href="javascript:;" v-if="record.state == 'NORMAL'" class="btn"
                 @click="goDetail('modify', record)">编辑</a>
              <!-- 异常发票判断 -->
              <a href="javascript:;" v-if="record.state != 'NORMAL'" class="btn" @click="del(record.id)">删除</a>
        ...
        <div :class="'pagination-checkbox '+(pagination.total<10?'fixedCheckbox ':' ')" v-if="selectedRows.length">
          <svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="18px" height="18px" viewBox="0 0 32.000000 32.000000" preserveAspectRatio="xMidYMid meet">
    ...
    <ConfirmModal ref="confirmModal" okText="去上传"></ConfirmModal>
  </div>
</template>
...
const pathSteelInfo = {
  INPUT: {
    ...
    uploadNegative: "/center/steels/invoice/upload/sellNegative",
  },
  OUTPUT: {
    detail: "/center/steels/invoice/selldetail",
    modify: "/center/steels/invoice/selledit",
    uploadNegative: "/center/steels/invoice/upload/buyNegative",
  },
  DELIVER: {
    detail: "/center/steels/invoice/Freightdetail",
    modify: "/center/steels/invoice/freightEdit",
    uploadNegative: "/center/steels/invoice/upload/freightNegative",
  },
};
const searchList = [
  {
...
export default {
  ...
  methods: {
    ...
    async spill(record) {
      // 红冲发票
      reverseInvoice({invoiceId: record.id}).then(res => {
        if (res.data) {
          this.$message.success("发票号码为" + record.no + "的发票，已红冲");
          this.getList(1, this.pageSize);
        } else {
          this.showRedAlert(record)
    ...
    showRedAlert(record) {
      const that = this
        ...
        this.$refs.confirmModal.showModal({
          modalTitle: '发票状态异常',
          modalText: '税局未查询到发票红冲，如需红冲发票，请先上传负数发票。',
          okText: "去上传",
          confirm: () => {
            that.$router.push({
              path: pathItem.uploadNegative,
              query: {
                id: record.id,
                no: record.no,
              },
    ...
    goAdd() {
      ...
      // 判断是钢材新增还是发票新增
      const path = this.industryType == "STEEL" ? `/center/steels/invoice/${steelStr[this.invoiceType]}` : `/center/invoice/${str[this.invoiceType]}`;
