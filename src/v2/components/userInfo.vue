<template>
  <div class="user-wrap">
    <a-popover placement="bottom" class="user-popover">
      <template slot="content">
        ...
        <ul class="company-list">
          <li v-for="item in conpanyList" :key="item.companyUscc" @click="changeCompany(item)">
            <span>{{ item.companyName }}</span>
            <a-icon type="right" />
          </li>
        </ul>
        <a-divider />
        <p class="logout" @click="logout">退出系统</p>
      </template>
      <div class="user-info">
        <div>
          <p class="user-name">
  ...
  methods: {
    ...
    getCompanyList() {
      ...
      API_COMPANYGETSWITCHCOMPANY({
        ...
        if (res.success) {
          this.conpanyList = (res.data || []).filter(
            (item) => item.companyUscc !== this.VUEX_ST_COMPANYSUER.companyUscc
          );
        }
      });
    },
