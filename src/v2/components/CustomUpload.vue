...
<script>
import { API_UPLOAD_FILE, API_GETCURRENTENV, API_DeleteAttach, API_GetDownloadRAR, API_UPLOAD_WATER_MARk } from '@/v2/api/upload';
...
export default {
	...
	props: {
		...
		defaultFormat: {
			type: Array,
			default: () => ['png', 'jpeg', 'jpg', 'gif', 'pdf', 'doc', 'docx', 'xlsx', 'xls', 'rar', 'zip']
		}
	},
	watch: {
		fileDataSource: {
	...
	data() {
		return {
			myallowFormat: this.defaultFormat, // 默认文件类型
	...
	mounted() {
		...
		switch (this.type) {
			...
			case 'offlineConsig':
				...
				break;
			default:
				this.myallowFormat = this.defaultFormat;
		}
	},
	computed: {
		...mapGetters('user', {
	...
	methods: {
		...
		handleFileTypeChange(value) {
			...
			if (this.type == 'receive') {
				this.myallowFormat = this.type == 'receive' ? this.receiveFormat : this.defaultFormat;
				if (
					value.key == 24 ||
			...
			if (value.key == 83) {
				this.myallowFormat = ['jpg', 'png', 'bmp', 'pdf', 'jpeg'];
			}
		},
			...
			if (this.isNeedRotate) {
				this.fileList.forEach(async (item) => {
					if (item.response && item.response.success) {
						const url = await this.$RsaDecrypt.generateFileUrl(item.response.data.fileUrl);
						this.$set(item, 'thumbUrl', url);
					}
				});
			}
			if (
