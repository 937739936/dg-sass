...
$(function () {
    ...
    let line = 5, divId = '#main-img_OP6DN';
    _get('/riskControl/screen/topic4', function (result) {
        var res = result;
        $(divId).empty();
        let arr = [[]];
        ...
        res.forEach((item, index) => {
            ...
            keyStr += `<div class="line2">${item.dc_name}</div>`
            keyStr += `<div class="line3">${item.data1}</div>`
            keyStr += `<div class="line4">${item.data2}</div>`
            keyStr += '</div>'
            arr[key].push(keyStr)
        })
        arr.map(item => {
